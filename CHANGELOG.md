...
### Added
- **Home-screen widgets on iPhone and iPad.** Two widgets, **Nearby Lifers** and **Media Targets**, named after the Map Explorer views they come from, list the nearest recent eBird reports, within 25 miles of where you are, of birds you still need: species not in your eBird backup, or, for Media Targets, species you have recorded but still lack a photo, audio or video of in your Macaulay Library export. Each comes in small (the nearest bird), medium (the nearest three) and large (the nearest eight), and has a **Time range** setting of Day, Week or 30 days. Media Targets also has a **Media** setting: Photo, Audio or Video lists the species missing that type, and Any, where a new widget starts, lists the species missing at least one and marks each bird with an icon for every type still missing. Each row shows the species, its distance in miles, how recently it was reported, and the place. A widget measures from your current location under the same permission SnowRaven uses for Use my location, never asks for permission itself or for Always access, and measures from your saved Default Location when location is off. It refreshes about every 30 minutes, as often as iOS allows; a refresh sends one nearby-sightings request to eBird with your own key and nothing to the developer, and when it cannot reach eBird it keeps its last list, marked Offline or eBird busy. Loading or clearing a data file, or saving or clearing your eBird key or your Default Location, updates placed widgets straight away. Tapping a widget opens Map Explorer on the matching view, with the widget's time range and media type selected, a 25 mile radius for the session, and a search from where you are, and while it loads, a line at the top of the map says what it is looking for. Tapping a bird in a widget then shows only that species, centered on the sighting the widget listed, and a Show all pill brings back every bird. Widgets need iOS or iPadOS 17 or later; on an older version the app works as before. Help and the privacy policy describe what the widgets send and what they keep on your device.

### Changed
- On iPhone and iPad, the location permission prompt now also says SnowRaven uses your location to measure distances in its home-screen widgets.
...
### Added
- **The weather record now sits on the bird's own page.** Statistics could already tell you the skies and temperatures you have a bird in, but only after you left the bird you were looking at, opened another tab, waited for it to compute, scrolled to the Weather section and typed the bird's name into a picker to re-select the species you had just been reading about. That is four steps to re-state something the app already knew, and the answer arrives on a page about everything except that bird. There is now a **Weather** card on **Species Detail**, sitting below Comments, for the species you already have selected. It has no picker of any kind, because you are already on a bird. It comes out of the same derivation the Statistics Weather section paints from, so for any bird both can offer, the two print the same numbers rather than merely agreeing today. It opens with one number held against two different wholes: how many of your weather-block checklists carry this bird, then, in a second sentence beginning "That is", the same number against the checklists you have the bird on. Nothing divides them and there is no coverage percentage for the bird, here or anywhere. Below that, two groups of rows, sky and temperature, every band present including the ones you have never birded, each bar the bird's own record scaled so its biggest band fills the track, each row reading as a count and its share of the bird's records, then in quieter type the same band's share of all your outings. Two shares of two different wholes, each named, and again nothing divides them. **A bird with fewer than 10 weather-block checklists gets a plain sentence and no chart at all**, and that is the ordinary case rather than an edge: on a real export four species in five land there. It is built as the full card with the chart not yet arrived, not as an empty box with an apology, so stepping through your list you read one continuous sentence per bird and the chart is a bonus that sometimes appears. It names the floor rather than pointing anywhere, because it cannot know whether filling in your backlog would reach this particular bird, and it will not promise something it cannot deliver. Every figure covers your whole export and counts every form as its parent species, so the county and date filters and all three toolbar switches change nothing on it, and it says so in one line whenever a filter is active or you have a form selected. It reads only the backup you already loaded, so it makes no lookup, works offline, and is correct on a device where you have never opened Statistics. Nothing is written anywhere and no new data leaves your machine.

### Fixed
- **Two layout defects in the weather rows, in the shipped Statistics Weather section as well as on the new card.** Both were found while checking the new card at a phone width with large text, then confirmed against the released 1.0.22 build, and both live in the row rules the two surfaces now share, so repairing them changes how the **Statistics** Weather section renders too. That is the point rather than a side effect: one set of rules, one place to fix. First, a row's label could paint straight over the figure beside it. In the narrow layout a label is allowed to wrap but was not allowed to break, so multi-word names like "Scattered clouds" wrapped cleanly while single-word ones could not break at all: "Thunderstorm" ran 49 pixels through the count next to it, in both browser engines, on the row every birder who has never birded a thunderstorm sees. Labels can now break, so a label always fits, wraps or breaks and never covers the number, at every supported window width and every text size. **That first repair reaches more of the Statistics tab than the bird-by-bird chart it was found on.** The same rule governs the labels in the four whole-export charts at the top of the Weather section, by sky, by temperature, by wind and by day versus night, and those had the same defect: at a phone width with the largest text, "Overcast" ran through the count beside it. They are fixed too. The change there is confined to the narrow widths at the largest text setting where the overlap happened, and the cost is that such a row can now be a line taller, because the label wraps instead of painting over the figure. Second, the bars could vanish for exactly the reader who enlarged their text. Whether a row keeps its four columns side by side or stacks was decided by the width of the window, which cannot answer a question about the width of a column: at 200% text on a desktop, rows kept the side-by-side layout with nothing left over, and several drew no bar at all while still taking a bar's height. That decision now tracks the row's own column measured against the size of its own text, so every bar keeps a usable width at every window size and every text size. That second repair is deliberately narrower than the first: it is scoped to the bird-by-bird charts alone, and the four whole-export charts above them are untouched by it at every width and every text size, which was measured rather than assumed. Both repairs were measured in Chromium and WebKit against a real build, on both surfaces, swept across the width bands where they actually failed, and re-run against a build with the fixes removed to confirm the checks can tell the difference.

## [1.0.22] - 2026-09-07

### Added
- **SnowRaven finally reads the weather blocks it has been writing.** Every time you look up a checklist's weather, SnowRaven hands you a block to paste into the comment, and that block comes home in your next backup. Until now the app did exactly two things with it: notice it was there, and hide it. There is now a **Weather** section on the Statistics tab, sitting just under Data Quality, that reads them. It appears only if you have at least one, it is built entirely from the backup you already loaded, and it makes no lookup at all, so it works offline and needs no key. It opens by telling you what it covers, before it draws anything: how many of your checklists carry a readable weather block, what fraction of your birding that is, and, if any blocks defeated it, how many. Every figure below is about those checklists and says so. **Your outings by weather** distributes them four ways, by the sky in the block, by temperature, by wind and by day versus night. Each axis states its own denominator, because a block can record a temperature and no wind, and one sentence explains why the totals differ rather than leaving you to wonder. A band you have never birded in is drawn as a zero rather than dropped, because dropping it would make the picture itself misleading. Three things it is careful about: it groups on the sky glyph in your own block rather than on the weather service's wording, since the glyph is a fixed set and the wording is not; a block records a temperature range because it covers every hour of the outing, so each checklist is counted once at the middle of its range and the section tells you how wide your ranges typically are; and wind is stored as a Beaufort description, so these are the app's own bands and not a speed it invented. **Species and time out, by weather** gives the average number of species per checklist in each band with the same bands' average time out beside it, on one chart with a Temperature/Sky toggle. That pairing is the whole point: weather changes how long you stay out, so a lower species count can be the weather or it can be the outing, and now you can see both. A band averages only once it has 8 checklists; below that it shows its count and refuses to average, because an average of seven outings reads as a finding and is noise. The time figure carries its own, smaller denominator, since not every checklist records a duration. **One species, its weather** lets you pick a bird and see the skies and temperatures you have it on. Each bar is that bird's own record, scaled so its biggest band fills the track, which is what makes a bird you have seen a handful of times readable at all rather than a sliver against a rail sized by hundreds of outings. Every row gives the count and its share of the bird's records, then, quieter beside it, the same band's share of all your outings; two shares of two different wholes, each named, and nothing divides one by the other. That second figure is doing real work: a chart of where a bird turned up also reflects when you were out, and having both on the row is what lets you see which is which. "0 and 0%" and "no outings" are drawn differently on purpose: the first means outings in that band and never this bird, which is interesting, and the second means you have never birded that band at all. Counts are checklists rather than sightings, so a bird listed three times on one checklist counts once. Two things it will not do. It never predicts, recommends or ranks: it does not tell you what weather is good for birding, there is no leaderboard of birds by rain, and there is no statistical inference anywhere in it. And it is honest about a number that does not line up: this section only counts a block carrying a SnowRaven or RainCrow credit, because a block with no credit is one whose wording it would be guessing at, while the Weather tab's own backlog uses a looser test. The two figures are therefore not complements of each other, the Help documents exactly that, and the section deliberately prints no count beside its link to the Weather tab so the two can never sit on screen disagreeing.
- **The Weather tab now says what filling in a gap is for.** The backlog section gained one line pointing at the Weather section on Statistics, so the two halves of the loop know about each other.

### Changed
- **RainCrow is spelled the way RainCrow spells it.** The Data Quality card and the Help said "Raincrow" while the blocks the app reads say "RainCrow". One word, on a chart label and in two sentences.

## [1.0.21] - 2026-09-06

### Added
- **Named Birds now shows you when you saw each bird, not just how many times.** A named bird is a relationship that runs over time, and until now the tab could only give you that as a list of dates, newest first. That is a good record and a poor picture: it does not show you that you saw a bird six times in one week in June and not since, or that another one turns up every few months without fail. Open a named bird's card and there is now a strip of its sightings laid out in time, one mark for each date. Below the whole list there is a second strip covering every named bird at once, one labelled lane each, on a shared axis, so you can see at a glance which birds you were following when and where they overlapped. Both strips answer to one control at the top of the tab, next to Sort, which chooses what the timeline runs to: **Last sighting**, which is where it starts, or **Today**. On Last sighting a strip ends at the day you last saw the bird. On Today it runs to now, so the gap since you last saw it becomes the empty stretch at the right hand end, and a bird you have not seen in months looks different at a glance from one you saw yesterday. The same control moves the elapsed figure on every card, so the number and the picture can never disagree with each other. The marks are yours to press. Press anywhere along a strip and it selects the nearest sighting; the two lines underneath name its date and the place you were, and say which of the bird's dates it is, so a cluster of marks sitting on top of each other at phone width is still countable. Arrow keys step through them one at a time, Home and End go to the ends, and Escape clears the selection. A bird you have seen once has no strip, because a picture of a single fact is not worth the room. It all works out of your own backup, so it makes no network request, saves nothing, and works offline.

### Fixed
- **The elapsed time on a named bird's card was wrong, and it never said which two dates it meant.** It was wrong in two separate ways. It threw away the remainder instead of rounding, so anything from two months to one day short of three months read as `2 mos.`, and a span of a full year less one day read as `11 mos.`; a bird followed for nearly three months was reported as two. And its month arithmetic assumed every month is thirty days long, which disagreed with the real number of days for about a fifth of all short spans and by as much as two days. February was wrong by two every time: the 27th of February to the 1st of March is two days, and the card said `4 days`. Both are fixed. The figure is now counted in real days and the months and years are derived from that count, so it is exact, and a same day first and last sighting reads `Same day` rather than the `1 day` it used to claim. The other half of the problem was that the card never told you what it was measuring. It measured the first sighting to the last one, while the website described it as how long you have followed the bird, which for a bird you are still seeing reads as first sighting to today. Each card now names its own endpoints in words, right after the figure, and the control described above lets you switch between the two readings.

## [1.0.20] - 2026-09-06

### Added
- **Search: press Cmd-K and go straight to the thing you were thinking of.** SnowRaven has a tab for every way of looking at your data, and a well-travelled backup holds close to a thousand birds. Until now the only way to reach any of them was to know where it lived: the right tab, then the right control on it, then the name. Getting to one bird's page from the Calendar meant switching to Species Detail, waiting for it to come up, finding the species picker at the top of it, and typing there. Press **Cmd-K** or **Ctrl-K** anywhere in the app and a search box opens over whatever you were doing; type a few letters, press Enter, and you are there. Both combinations work on every platform, so one habit travels between a Mac and a PC, and there is a visible **Search** control in the navigation for anyone who would rather press something: at the top of the sidebar on a wide window, as a magnifier at the top of the icon column on a narrower one, and as the first row of the More sheet on a phone. It searches two kinds of thing. Destinations are the tabs you have visible, in the order you saved, with Settings last; a tab you have hidden in Settings does not appear, and with nothing typed the whole visible list is shown, so Search doubles as a quick jump between tabs. Species are the birds in your own eBird backup, matched on the common name and the scientific name at once, so *calypte* finds Anna's Hummingbird, and choosing one opens Species Detail on it even when you have that tab hidden. The arrow keys move through the results and stop at the ends rather than wrapping, Enter opens the highlighted one, and Escape closes; so does the Close button, a press on the dimmed area, choosing a result, and a second press of the key combination. Whichever way you close it, the keyboard goes back to the control you opened it from, or to the main content when that control has gone away. It makes no network request and saves nothing, so it works offline and needs no API key. About the species half it says which of these is true rather than showing you an empty list: your backup is still being read, no backup is saved yet, a saved one will not load, or nothing matches what you typed. Destinations are there the moment it opens and are never held up by any of that, and replacing or clearing your backup in Settings reaches an open search straight away, so it never offers birds from a file that is gone. Where the main way you point at the screen is touch, an iPhone or iPad included, no key combination is shown at all, because there is none to press; attach a keyboard and both still work. Two limits are worth saying out loud. Search finds destinations and species, not counties, hotspots, locations or checklists, because none of those has a page of its own to land on yet. And on a phone, while a map is expanded to fill the screen, the navigation is not on screen and neither is the Search control, so leave fullscreen first or use the key combination if you have a keyboard.

### Changed
- **Statistics does its arithmetic beside the screen instead of on it.** The tab derives everything it shows from your backup in one long run of sums: the life list, the totals, the accumulation curve, the temporal and duration breakdowns, the geography, the effort and quality figures, the breeding stats and the rest. All of that used to run on the same thread that draws the window, so for as long as it ran the window could not respond to anything, and it ran twice over: once on your first visit to the tab, and again in full every time you pressed **Count all forms**. That work now happens on a thread of its own and the results arrive together when they are ready. Nothing on screen changes. The same "Computing your statistics" line covers the first visit, a toggle still flips instantly and updates the figures a beat later, and every figure, list and chart is identical to 1.0.19 for the same backup, checked in Chromium and in Apple's WebKit. What changes is what the app can do while it thinks. Measured in Chromium, which is the one engine that can report this, on a reference backup with the processor slowed down to stand in for a phone or a Raspberry Pi, the longest unbroken block of unresponsiveness on a first visit fell from about 620 to 670 milliseconds down to about 380 to 430, and a press of Count all forms fell from about 180 to 200 milliseconds down to about 86 to 91. Three honest notes come with that. On a fast Mac at full speed, pressing Count all forms did not visibly stall beforehand either, so there is nothing there to feel; this is for the slower machines. Choosing Weekly, Monthly, Yearly or Total costs the screen what it always did, because that press never recomputed the whole chain to begin with. And the tab is not free of pauses now: drawing the page itself and loading the map on it are both still substantial pieces of work and neither is touched by this, so what is true is that the sums no longer add to them. One thing happens out of sight: the tab keeps a second copy of your observations beside the screen while you are working with it, and lets go of it about half a minute after you stop, so returning after a long pause pays a small handover again rather than holding that memory for the whole session.
- **A large Macaulay Library export no longer locks up the window while it loads.** Turning your ML export into a list of species ran on the same thread that draws the window, so a big export froze the app for as long as it took, on whichever tab was the first to need it. It is now read on a thread of its own, the way your eBird backup has been since 1.0.14, and the window stays responsive throughout. A read that goes wrong is noticed rather than left hanging: the tab says it could not load your ML export instead of waiting for an answer that is not coming, within a time limit worked out from the size of your own file.
- **The Calendar's day details box and the Map Explorer's filters sidebar hold the keyboard in place by construction.** Both used to decide whether the keyboard had wandered out of them by comparing where it was against a list they built of the controls inside, which is a prediction about the order the browser will visit those controls in. When 1.0.16 gave every control in the day details box an explicit place in the tab order, that prediction started being right, and the box has held the keyboard correctly ever since; the prediction itself was never repaired. Both now watch for the keyboard landing anywhere outside them and pull it straight back when it does, which is what the app's other overlays have done since 1.0.15, so containment no longer rests on an order agreeing with a guess. The filters sidebar does that only while it is a cover over the map, which on a phone is what it is. Pressing Tab in either one behaves exactly as it did. The same shared machinery is now behind the first-run welcome screen and the in-app help overlay as well, with no change to how either behaves.

### Fixed
- **A file that is not the export for the slot, or one that is too large, is turned away when you upload it.** The two Default Files slots in Settings checked only that the name of the file ended in `.csv`. `MyEBirdData.csv` ends in `.csv`, so dropping it into the ML Export slot stored it happily and the mistake surfaced later on Multimedia, as an export that could not be turned into a species list, with nothing to say which file was wrong or which slot it belonged in. Each slot now looks at what is actually in the file before saving it, and one that is not the export for that slot is refused on the spot, with a line under the row saying so and naming the slot it does belong in. A file larger than 50 MB is refused the same way. In both cases whatever was already saved in that slot is left exactly as it was. On the web and Raspberry Pi versions this also closes a straight untruth: the server has always refused an upload over 50 MB, and the app threw that answer away and reported the upload as saved, over a slot that still held the old file. It now says what happened. The desktop, iPhone and iPad versions never had a size limit at all, and now have the same one.
- **The Map Explorer's filters sidebar stops holding the Tab key once your phone is sideways.** Open **Filters** on a phone and the sidebar covers the map, so the Tab key is deliberately held inside it. On a cover that is exactly right. Rotate to landscape with it still open and it stops covering anything: it becomes a column beside the map, with its own close button not drawn at that width, and yet Tab went on cycling around the controls inside it. Nothing on the rest of the page could be reached with Tab until you pressed Escape to close the sidebar or clicked somewhere else, neither of which you had any reason to think was needed, since the map and the page were plainly right there beside it. The sidebar now stands down the moment the screen is wide enough that it is no longer a cover, and Tab moves through the page as usual. Nothing you can see moves, because the sidebar's contents are on screen at that width either way, and the keyboard is left on whatever control you were already using rather than being sent to a Filters button that is not drawn at that width. Turn the phone upright again and the Filters button is back in the corner where it belongs.
...
### Fixed
- **The popups on the maps can now be closed from the keyboard in the Mac, iPhone and iPad apps.** Open a breeding-atlas block, a county, a Media Targets spot, a Nearby Lifers spot, or a pin on the Sighting Locations map, and the little x in the popup's corner was drawn by the map library rather than by SnowRaven. Apple's WebKit engine, which those three apps run on, does not put an ordinary button in the tab order unless the page asks it to, and the library never asked, so on those three the x could not be reached at all: no amount of pressing Tab arrived at it, and Escape does nothing to a map popup. On the atlas, county and sighting popups there was no other way to close them, so a keyboard user opened one, read it, and moved on with it left open. On the Media Targets and Nearby Lifers popups the only way out was to leave the popup, find the matching row in the sidebar list and press it again, which on a phone means going behind the Filters panel. All five popups now draw their own close button, in the same corner, at the same size, with the same larger tap target on a touch screen, so Tab reaches it and Enter or Space closes it. Two smaller things change everywhere, the web and Windows included. The button now has a name: the map library gave its own button no name at all, so a screen reader read out the times sign, and each of the five now says which popup it closes. And the keyboard is likely to start in a different place when a popup opens, on the close button rather than on the first link inside, because the library moves the keyboard to the first thing it can find and the close button is now earlier in the popup than it used to be. That second one is read from how the library works rather than measured in each browser. Nothing changes for a mouse anywhere: the button looks and behaves exactly as it did, and a click on the map still does what it always did on each popup.
- **The marks beside a bird's name stay visible when their icons cannot load.** Every bird name in the app is followed by two small marks, one linking to that species on eBird and one to its Birds of the World account, and each mark is the site's own icon, fetched from the site. When an icon could not be fetched, because you were offline, or a content blocker refused it, or the site was slow to answer, the mark simply vanished and left a gap beside the name that you could still click without being able to see it. With no connection that happened to every mark on every screen, including a first-ever launch, where the rest of the name is drawn from the app's own copy of the eBird taxonomy and comes up fine. The app now draws a stand-in symbol in the same spot instead, a globe for eBird and a shelf of books for Birds of the World, at the same size and in the same ink as the text beside it, so the link is there to see and to press. Nothing moves, both marks still name where they go for a screen reader, and an icon that arrives late takes its place back. Each mark answers for itself, so one site's icon beside the other's stand-in is an ordinary sight: the two sites let a browser keep their icons for different lengths of time, so they do not go at the same moment.
