...
- **The "open this checklist on eBird" affordance is the shared `components/ChecklistLink.tsx`** — one visual signature + one accessible-name formula (`Open checklist {id} on eBird (opens in a new tab)`), and it keeps the `SUBMISSION_ID_RE` guard (the standing eBird-id shape-validation rule below applies — junk id renders as plain text, never a styled 404 link).
