...
## Overlapping saves to one shared document are a lost update -- desktop writes now take turns, the erased ledger is honestly unrecoverable, and the store leg is an OPEN question -- 2026-08-30 (v1.0.9)
...
**The App Store leg is an OPEN question -- not the standing hold, and not yet a decision.** 1.0.9 likely clears the replacement bar: the same unserialized seam ships in the in-review 1.0.4 build (`b52fdc55`), so the race exists there. But the user has twice chosen to let 1.0.4 finish review when a build qualified (1.0.5, 1.0.7), and has not yet chosen for 1.0.9. Per the 1.0.5 entry's instruction the disposition stays with the user -- state that it qualifies, name the re-queue cost, ask -- so this entry records the question as open, never as decided by omission. Everything else shipped: macOS (notarized, stapled) and Windows (CI green at the tag) with `latest.json` live, the website, and iOS 1.0.9 build 1 on TestFlight.
