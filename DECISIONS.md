...
## A published mock is a behavioural claim, a liveness gate must OBSERVE rather than predict, a gate that skips silently reports "not run" as "verified", and the App Store leg was missing for a THIRD release -- 2026-09-06 (Spool bundle, v1.0.20)

**Decision: shipped** as a six-build Spool bundle -- `f8779ae` (command palette, a New Feature with a design pass), `ebaa8ea` (ML export hardening), `8441dac` (Playwright gate), `5b89e39` (shared control primitives, DECLINED at scoping with no code -- its own entry below), `2a27adc` (focusable-selector single source), `d757255` (Statistics compute off-thread -- its own entry below), squashed at `e9bbdf7`, release prep at `49886ae` where tag `v1.0.20` points, iOS stamp at `ab71cd2`. Notarized universal macOS DMG (submission `36e105c1-8dc8-4d3c-bda4-6bde4a245c13`, Accepted and stapled), signed Windows installer, updater bundle and a `latest.json` carrying all three platform keys (`darwin-aarch64`, `darwin-x86_64`, `windows-x86_64`). iOS build `1.0.20.1` VALID on TestFlight, Delivery UUID `3a074a02-9344-40e0-9a11-77688323e8b1`, validated with `altool --validate-app` before upload. A stale 1.0.19 archive was found sitting in `src-tauri/gen/apple/build/` and moved aside before the build; the fresh archive was confirmed by its stamped `CFBundleVersion`, per the standing rule. Security PASSED WITH NOTES on all five built runs, no Critical or High.

**A PUBLISHED MOCK OF AN INTERACTIVE SURFACE IS A BEHAVIOURAL CLAIM, AND IT IS CHECKED BY RE-DERIVING IT THROUGH THE REAL CODE.** `website/index.html`'s Search figure showed a query and a result list the shipped filter cannot produce: QA fed the shipped default layout and the mock's three species to the real `buildPaletteRows` and the published `warb` query yielded `destinationCount === 0`, against a mock showing a destination. The replacement was not eyeballed either -- the inputs were fed in a deliberately WRONG order, the returned rows (one destination, then Anna's Hummingbird, California Quail, California Scrub-Jay in `compareSpeciesName` order) were matched element-for-element against rows parsed back out of the published HTML. `aria-hidden` removes a mock from the accessibility tree, not from the reader's eye. **Two prose rules earned beside it.** Three of four published files independently reached "the search overlay" and the fourth reached for the component's name, which is v1.0.16's never-name-a-surface-from-a-component rule one level up, on a surface that has no tab id at all; the mechanical check is to grep the diff for every identifier in the feature's own filenames. And `docs/HELP.md` got the destination-population claim right ("every tab you have visible") while `README.md` and the website got it wrong in the same change: **when one prose file has a claim right and another has it wrong, that is a sweep failure, not a wording difference** -- compare the files against each other, not only each against the source. The guard that now holds both, `frontend/src/lib/palettePublishedClaims.test.ts`, is the shape worth copying: it scopes itself to each file's Search passage rather than the whole document (so HELP's true "Every tab is reachable at every width" is not collateral), builds the cap regex from the imported `SPECIES_CAP`, and carries a non-vacuity leg per file so DELETING the sentence goes red -- which is the move an author under time pressure actually reaches for, and the one a naive "must contain" guard rewards.

**A LIVENESS GATE THAT DECIDES WHERE FOCUS GOES READS `document.activeElement` BACK; IT DOES NOT PREDICT WHICH ELEMENTS ACCEPT FOCUS.** The palette captures its opener eagerly, and `document.activeElement` is `<body>` whenever nothing in the page holds focus -- which passes every predicate an enumeration reaches for (in the document, not disabled, not inert) while accepting no focus at all, so the restore reported success with focus sitting exactly where the spec forbids it. Measured in Chromium and WebKit. `lib/paletteFocus.ts` now rejects `<body>` and `<html>` outright AND reads `activeElement` back after focusing, so a candidate that refuses focus for any other reason falls through. This is the v1.0.15 focus-trap rule (contain on `focusin`, never predict the engine's order) in its second location: enumerating an engine's refusals is a prediction, and this repo has now paid for that twice.

**THE FOUR COPIES OF `FOCUSABLE_SELECTOR` ARE GONE, AND TWO OF THE BUILD'S OWN PREMISES WERE MEASURED FALSE ON THE WAY.** `WelcomeScreen.tsx`, `Calendar.tsx`'s day dialog, `MapExplorer.tsx`'s filters sidebar and `HelpDocs.tsx` each hand-rolled a keydown-only trap; all four now use `lib/useFocusTrap.ts`, and a source sweep finds zero `activeElement === first/last` comparisons left in production. The one permitted copy is `website/tools/verify/verify-palette.mjs`, which runs inside `page.evaluate` where there is no module graph to import from, and it is compared to the export character-for-character rather than by eye. **WelcomeScreen must NOT opt into containment**, against the brief: it renders the control that opens HelpDocs, App.tsx mounts HelpDocs as a later sibling at the same z-index, and the welcome stays mounted, so an armed `focusin` arm answers Help's own opening `.focus()` by yanking focus back to the welcome's "Go to Settings" button -- measured twice, independently. **And HelpDocs is not an F061 site**, against the brief's other premise: by the time an effect cleanup runs React has already detached the ref, so the arm returns at its `if (!root) return` guard. That was proved with a discriminator rather than a story -- two otherwise-identical panels differing only in restore timing, the synchronous-inside-the-handler one landing on `<body>` and the effect-cleanup one landing on the opener -- so the deciding variable is restore timing relative to the unmount commit, and "the restore is declared before the trap" is not by itself a finding. HelpDocs still stays at the default, but the deferral now rests on "no measured need" rather than on a mechanism.

**AN OPT-IN TO CONTAINMENT IS ARMED ON THE CONDITION THAT MAKES THE SURFACE MODAL, NEVER ON A PROXY FOR IT -- and this build's own Medium was that rule's first instance.** The filters sidebar armed on `sidebarOpen`, plain state with no width awareness, while the thing that makes it modal is a CSS tier. Open Filters on a phone at 640px or less and rotate to landscape (an iPhone reaches about 844px): the sidebar loses `position:absolute`, the backdrop reverts to `display:none`, the Close button reverts to `display:none` so the trap's own visibility filter drops it from the list -- and the trap stayed armed on a now in-flow column beside a plainly visible map. The keydown cycle was pre-existing; what this build added was the `focusin` arm removing the click-outside release, turning an escapable cycle into a hard capture, and that increase is this build's. Three things ride with the fix. The invariant belongs in a **render-phase adjustment, not an effect**, because the trap arms in an effect: a render-phase clear is discarded before commit so the armed state never exists, while an effect commits it and disarms a tick later. The clearing path is chosen by **who caused the change** -- routing a viewport-driven clear through the user-driven close path would fire an opener-restore at a Filters button the new tier does not draw, dropping focus to `<body>`. And the flag cannot disagree with the CSS because `useIsPhone` reads the same `(max-width:640px)` the stylesheet block opens at.

**A GATE THAT SKIPS WHEN ITS DEPENDENCY IS MISSING REPORTS "NOT RUN" AS "VERIFIED", AND THE DEPENDENCY REPAIR WAS THE WHOLE BUILD.** `website/tools/package.json` and `package-lock.json` were gitignored and untracked, filed alongside `node_modules/`, `demo-data/` and `shots/`, so **Playwright was declared nowhere in this repository**: a fresh clone got the `.mjs` files and no way to run any of the fourteen harnesses, and the same was latently true of the screenshot pipeline. Both manifests are tracked now with a comment saying why, `playwright@1.62.1` resolves exactly, and three harnesses moved from `pipeline/<feature>/` to `website/tools/verify/` behind one runner wired into `pipeline.yml` after `npm run build`. The gate **fails when `CI` is set** and Playwright cannot be resolved, and skips with a loud banner otherwise. **Two of its own theses failed one level up and were fixed.** `run.mjs`'s header claimed it scanned the directory while its roster was hand-written, so a fourth harness dropped into `verify/` would have gone silently unrun under a summary reading `3/3 green` over a denominator that had quietly shrunk. And **the derivation's predicate inherits the full weight of the guarantee the list used to carry**: `readdirSync` returns directories and symlinks, and a name-only filter executed both -- a `verify-dir.mjs/` directory resolves through its `index.js` and reports PASS. `withFileTypes: true` plus `isFile()` is what makes the predicate test what an entry IS rather than what it is named. Separately, `detached: true` buys process-group teardown and **silently sells terminal signal delivery**: SIGHUP left the harness orphaned holding both browsers, precisely the leak the detach was added to prevent.

**THE UPLOAD REFUSALS ARE A REGISTRY, NOT A DISCIPLINE, AND THE WEB TRANSPORT WAS THROWING AWAY THE ONE ANSWER IT HAD.** The Default Files slots checked only that a name ended in `.csv`, so `MyEBirdData.csv` in the ML Export slot stored happily and surfaced later on Multimedia as an unparseable export with nothing to say which file was wrong. `lib/uploadGuard.ts` exports `refuseByFilename` and `refuseByContent` as a pair so an import path added later gets all of them or visibly none, `lib/detectExportType.ts` classifies from the header line alone (deliberately looser than both parsers, proved by running them, so a refusal can never turn away a usable file), and `lib/firstLine.ts` bounds the header read at 8,192 characters. **That bound was a retention defect, not a CPU stall, and the security review corrected its own round-one severity reasoning in writing:** on the shipped unbounded spelling a 200 MB input with no line break retained 200 MB, and one line followed by a newline retained **6.4 GB** through the char-by-char cons-string copy -- an OOM kill, not a slow load on a Pi. Separately, `WebStorage.writeFile` discarded the `fetch` response, so the one platform that has always enforced a 50 MB cap was the only one that could not report it: an over-cap upload read as a completed save over an unchanged slot. `deleteFile` gained the same check with `404` treated as done, because raising it would put "Delete failed. Please try again." over a row that is already empty.

**A BOUND STATED IN CHARACTERS IS TESTED AT THAT BOUND UNDER EVERY LINE TERMINATOR THE FUNCTION ACCEPTS.** The header bound shipped a CRLF off-by-one that a LF-only edge row could not see, in a function whose own comment asserted the edge held -- and the row named "the edge holds" checked LF at the bound and CRLF at bound-minus-one, so it never touched the failing combination. The comment was repaired to be true for both endings rather than narrowed to name an exception. **And a bound whose enforcement point differs from its scan window states which of the two is the control and pins both, in opposite directions:** `MAX_SCAN_READS = MAX_HEADER_CHARS + 2` is deliberately wider than the bound, which reads as a defect until you see enforcement is on the resulting line's LENGTH; narrowing it broke correctness and widening it broke only the work bound, so the pair of mutations is what proves the design and a single "is it bounded" test would have accepted either error.

**THE APP STORE LEG WAS MISSING AGAIN, FOR A THIRD RELEASE, AFTER A ROADMAP ITEM AND A CLAUDE.md RULE BOTH NAMED THE DEBT.** Queried at ship, App Store version records exist for **1.0.19, 1.0.17, 1.0.14, 1.0.13 and 1.0.4**, while TestFlight holds VALID builds for **1.0.15.1, 1.0.16.1, 1.0.17.1, 1.0.18.1 and 1.0.19.1**. So **1.0.18 joins 1.0.15 and 1.0.16 as a release that shipped a VALID TestFlight build and never received a store version record.** Nothing was lost to users -- 1.0.17 carried 1.0.15 and 1.0.16's changes to the store, 1.0.19 carried 1.0.18's -- but that is three silent skips, and the second and third happened AFTER a roadmap item naming the debt existed, which is the same failure v1.0.17's entry recorded when the item sat at number 1 in Up Next while 1.0.16 shipped past it. Writing the rule into CLAUDE.md did not prevent the third either. **What actually caught it, all three times, is the same thing: querying App Store Connect at ship.** A TestFlight build is not a submission, and the changelog, the tags and the repo look identical whether a version record exists or not, so nothing local can tell you. 1.0.20 has version record `d90eed84-df1f-4f68-8b92-b81fbf313263`, review submission `b9971e91-edc2-4b43-8bb4-b915d5dda3fc`, **WAITING_FOR_REVIEW**, release type AFTER_APPROVAL, with build 1.0.20.1. **The pattern is recorded rather than the intention:** a rule in a file and an item on a roadmap have each now failed to stop this, so the only thing to rely on is the ASC query itself, listed for the last few releases at every ship, with any deferral written down as a decision with a reason.

---

## A worker may HOLD its input when the memory is bounded and the fallback is the shipped behaviour; the fix for a bad delimiter is to stop DECODING rather than pick a better one; and a surviving mutant can be a fact about the design -- 2026-09-06 (statistics-compute-off-thread, v1.0.20)

**Decision: shipped** (`d757255`). The whole Statistics derivation moved into `lib/statsBundle.ts` as one pure `computeStatsBundle`, called from a worker (`lib/statsWorker.ts`), behind a settle contract (`lib/statsOffThread.ts`) and a component seam (`lib/useStatsBundle.ts`). About fifteen chained `useMemo`s left `BirdingStats.tsx`; both `useDeferredValue` calls are gone, because the bundle IS the deferred value and carries the `includeSpuh` and `granularity` it was computed with, so the accumulation chart reads the request its data came from rather than approximating it with a separately-deferred control. Nothing on screen changes: page text is byte-identical to 1.0.19 in Chromium and WebKit, on first visit and after the toggle. Measured in Chromium with the CPU throttled to stand in for a phone or a Pi, worst long task on a first visit fell from 622-674 ms to 381-432 ms at 4x and 974-1007 ms to 576-588 ms at 6x; Count all forms fell from 178-200 ms to 86-91 ms at 4x. Three qualifications are stated rather than buried: at 1x the toggle produced no long task on either build, so there is nothing to feel on a fast Mac; the granularity press is unchanged, because it never recomputed the whole chain; and the page still has long tasks (its own render, MapLibre's mount), so what is true is that the compute chain no longer contributes one.

**A WORKER MAY HOLD ITS INPUT RATHER THAN RECEIVE IT PER REQUEST, AND THE TRADE IS MEASURED BEFORE IT IS TAKEN.** Both shapes were measured on the reference export (21,856 rows, 7.18 Mchar) before choosing. Re-posting the observations with every request costs a `structuredClone` round trip of **34.6 ms, about 17 ms of it unbreakable main-thread serialization**, to move **47.9 ms** of compute off -- a third of the win handed straight back, scaling with the device exactly as the compute does, so it is worth least on the machines the change exists for. Holding the copy makes every later request three small values at **0.003 ms** out and about 4 ms back. The price is **19.2 MB of worker heap** (2.67 MB per Mchar) beside the 19.3 MB the main thread already holds -- **larger than the 13.2 MB of raw CSV text this repo has already decided once was not worth retaining**, when `observationsCache` stopped holding it to answer a boolean. So the copy is bounded rather than assumed acceptable: `STATS_WORKER_IDLE_MS = 30_000` tears the worker down after thirty seconds with nothing to do and the next request respawns it and re-pays the hand-over, which is a smaller win after a long pause and never a loss (17 ms against 48 ms). The bound exists because the tab is never unmounted -- tabs are hidden with `display:none` -- so without it one visit would hold the copy for the rest of the session.

**THE SETTLE CONTRACT'S THIRD INSTANCE IS THE FIRST THAT IS NOT ONE-SHOT: teardown and settle are different events.** A worker outlives a request here, so each request owns an idempotent settle (clear its watchdog, drop it from `pending`, resolve or reject once, re-arm the idle timer) while teardown is separate and is triggered by the fatal paths, the idle timer and `dispose`. Four things are new at this instance. **Termination assertions become two-sided** -- zero on the happy path, exactly one per fatal path -- where the one-shot precedents could assert one everywhere. **A `{ ok: false }` reply rejects the request without killing the worker**, because a compute that threw says nothing about the worker's health and the alternative is discarding a 19 MB hand-over over one answer. **A memory bound is part of the contract**, per the paragraph above. And **the contract is not total until the PAYLOAD is checked as well as the timing**: a reply whose id matched but whose bundle was absent resolved with `undefined`, and the render gate `!computed || !bundle` then parks the tab on its spinner permanently for that export -- the settle contract's own failure shape arriving one layer above it. `isStatsBundle` now checks all sixteen fields behind `BUNDLE_FIELDS: Record<keyof StatsBundle, true>`, so adding a field and forgetting the guard is a build error rather than a silently narrower check; it tests `undefined`/`null` rather than truthiness, because a `!b[field]` check would have rejected the empty bundle (`filteredCount: 0`, `includeSpuh: false`) and produced the exact hang it exists to prevent. Forgery was cleared in one line only because both ends use the per-worker `onmessage` rather than a global `message` listener; a future worker reaching for `addEventListener('message')` would look identical at the call site and be a genuine hole, so the module doc names the property.

**THE FAILURE SIGNAL DELIBERATELY INVERTS ITS v1.0.14 PRECEDENT, AND THE REASON WAS CHECKED RATHER THAN INHERITED.** v1.0.14 resolved `null` rather than throwing and refused a main-thread retry; here `compute` REJECTS and `computeStatsWithFallback` catches it and runs the identical chain synchronously. Both halves of the precedent's reasoning were tested against this case and neither transfers. A failed parse must not be retried on the main thread because that re-runs the identical multi-megabyte allocation that just failed, on the thread that paints, and there is no other way to get the answer; **here the input array is already resident on this thread -- it is the array we cloned FROM** -- so the fallback allocates nothing the worker allocated (measured: the whole bundle retains 1.3 MB against 19.3 MB already in the heap). And the fallback is not a novel risk: **it is the shipped 1.0.19 behaviour**, which ran this exact chain on this exact thread on every render, so refusing it would replace a working tab with a blank one. None of the nine internal reason codes reaches the DOM. The totality claim is bounded at the definition site rather than left implied: this settles for every outcome the session can produce, and it does NOT close non-settlement, which is closed one level down by the watchdog because that is the only place it can be.

**AN IDLE TEARDOWN RESPAWNS AND EVERY FATAL PATH DOES NOT, AND THAT ASYMMETRY IS NAMED AT THE DEFINITION SITE RATHER THAN LEFT TO BE INFERRED FROM TWO FLAGS.** An idle teardown says nothing went wrong, so the next request spawns again. **A fatal event is a verdict about this device and this export, and every one of them is a verdict that would repeat:** a worker OOM-killed on a 19 MB hand-over is killed again by the same hand-over, a `postMessage` that could not clone the export out cannot clone it out a moment later, a constructor that refused is a platform saying no, and silence past the budget means either the worker is gone or the device took a thousand times the measured cost. Retrying on each toggle would re-run the allocation that just killed a thread, pay the spawn and the serialize for it, and fall back anyway. The stated cost is bounded, and it is why this is the right side to err on: **one transient death that would not have recurred leaves the tab computing on the main thread for the rest of the export's life, which is exactly how 1.0.19 behaves.** `dead` and "no worker" are two flags rather than one; collapsing them loses this.

**A BUDGET RE-KEYED ONTO A DIFFERENT UNIT FROM ITS PRECEDENT NEEDS AN ABSOLUTE CEILING, AND THE CEILING IS ANCHORED TO A CEILING THAT ALREADY EXISTS.** The per-row anchor was measured over a 15x range and taken at the slowest reading, 5.58 us/row at the smallest input where fixed overheads dominate, giving 30 s + 1.5 ms/row, about 269x the anchor -- a hair wider than the ~250x both parse budgets carry, which is right because this one covers a clone leg they do not. But the 50 MB upload cap is a hard ceiling for a budget keyed on BYTES and is not one for a budget keyed on ROWS: `entryFromRow` needs only a non-empty Common Name, so at two-byte rows 50 MB is about 25M rows and the budget would have been about ten hours, during which a silently OOM-killed worker (an OOM kill dispatches no `error` event, which is the case the watchdog exists for) leaves the tab on its spinner. The shipped `STATS_BUDGET_CEILING_MS = 240_000` is better than the round five minutes first recommended, and the reason is the invariant rather than the number: **the parse budget's effective ceiling at the same cap is 230 s, so anything that survived its own parse always has at least as long to compute.** When two budgets guard successive stages of one pipeline, state their relationship, not just their values.

**THE ONE REAL REGRESSION THIS BUNDLE INTRODUCED, AND THE FIX WAS TO REMOVE THE ROUND TRIP RATHER THAN CHANGE THE JOINER.** The refactor encoded the escapee set by joining the sorted names on a newline and splitting them back out, on the stated premise that an eBird common name cannot contain one, so `{"Mallard\nX"}` and `{"Mallard", "X"}` shared a key. **Nothing enforces that premise:** `streamCsvRows` is a character-level reader whose own header says it handles quoted fields with embedded newlines, `trim()` and `normalizeSpeciesName` strip ends only, and the taxonomy lookup resolves on scientific name first, so such a name reaches the excluded set intact. Two consequences: an escapee silently stops being excluded, and the key is the recompute key, so two different sets collide and one reuses the other's bundle. **This was new** -- at 1.0.19 the Set was passed by identity with no encoding step -- which is why it is reported as a regression rather than filed under robustness. The Auditor offered a NUL joiner; the Engineer declined it, and the verification records the decline as correct: **any joiner is a claim about what a CSV field cannot contain, and this build had just been wrong about exactly such a claim, so a "safer" character keeps the same claim alive in a quieter form and re-arms it for the next reader.** `excludedNamesKey` is now `JSON.stringify(sortedNames)`, injective over any string array for any content whatsoever, and the names travel to the request as the array they already are; **nothing decodes the key, and the definition site says so, so a future decoder is visibly a new decision rather than a small edit.** No identity churn was reintroduced: the effect keys on the string, so a Set rebuilt with identical content yields a new-but-equal key and no re-run -- the property the original design was reaching for, now had without an encoding. **The visible symptom is quieter than the fixture suggests, which makes the guard more valuable rather than less:** on the real export both colliding sets drop exactly one species, so both headline counts read **282** and only the accumulation series differ. The guard, `lib/statsExcludedKey.test.ts`, is the shape worth copying -- it asserts the OLD joiner really did collide on each pair before asserting the shipped key does not, sweeps ten characters a future change might reach for so the property is about all content rather than the one character that was found, and carries a comment-stripped source scan because the file's own explanation of the banned operations would otherwise match it.

**A SURVIVING MUTANT CAN BE A FACT ABOUT THE DESIGN RATHER THAN A COVERAGE GAP, AND THE WAY TO SAY SO IS THE MEASUREMENT AT THE DEFINITION SITE.** Two guards in the settle path are each half of a redundant pair: `if (settled) return` pairs with `pending.delete(id)` (the delete is what makes a second delivery unreachable at all, the flag is what would stop it if it were not), and the watchdog callback's `if (!settled)` pairs with `clearTimeout(watchdog)`. Deleting either half alone leaves the suite green; deleting a pair turns it red, and each partner is independently killed by "a settled request leaves no watchdog armed behind it". So **neither survivor is reachable while its partner stands**, and both are kept: each is one line, each closes a hazard the other closes by a different route, and a settle contract is exactly the place to pay a line for a second route. The measurement is written at both definition sites and in the test file's header, **so a future reader deleting one as dead code meets the measurement instead of a green suite.** Their provenance is worth keeping too: they began as three uncovered mutants, because `FakeWorker`'s `late-reply` behaviour was byte-identical to `silent` -- it returned without delivering anything, so the test named for a late event never delivered one and asserted the mechanism instead. **A settle-contract guard over a long-lived worker must DELIVER the late event, not merely assert that the handlers were detached.**

**A STALE PREMISE SURVIVED IN THE VERY MODULE THE FIX WAS ABOUT, ONE ROUND AFTER v1.0.17's RULE NAMING THAT SHAPE.** `statsBundle.ts`'s `StatsRequest` doc block still explained that `excludedNames` is sorted so `excludedNames.join(...)` can be a content signature -- the premise just struck for being unsafe, stated as live rationale, in the file a reader arrives at first. `useStatsBundle.ts` still pointed at "the signature note below" over a note that no longer used the word. The risk is precise: the next person to touch the escapee key reads that line, concludes a joiner is the sanctioned design, and re-introduces what the new guard exists to prevent -- a decoder the source scan would catch, but whose REASONING the comment would have supplied. It was caught by the Auditor's confirmation round, not by the source sweep that ran in the same build and correctly removed a different stale justification from `useStatsBundle.ts`. **v1.0.17's rule is that a claim which reached published prose from the source is swept starting at the source, and its own worked example was a struck premise surviving in a doc comment in the file the fix was about. The rule did not prevent its own recurrence one release later**, which is worth recording plainly: the sweep is only as wide as the file set the sweeper thinks the claim lives in, and a premise stated as a TYPE's rationale lives one module away from the code that changed.

**A GUARANTEE THAT HOLDS ONLY BECAUSE OF A CALLER'S INCIDENTAL BEHAVIOUR IS NOT GUARDED.** Removing `observations` from the session effect's dependencies left both component-level tests green, because `BirdingStats` happens to cycle `active` on every new export and tears the worker down that way regardless -- so a caller that swapped exports without cycling `active` would keep answering from a copy of a file the user has replaced. The invariant is now pinned at the hook with no component in the way. Whenever a component test appears to cover a hook invariant, mutate the hook and run the CALLER's tests to find out whether the component is doing the work.

---

## Shared control primitives are DECLINED a second time, and the v1.0.19 deciding fact is REPLACED: the app's shared button contract is a CSS class register, not a React component -- 2026-09-06 (Spool bundle, v1.0.20)

**Decision: not built, again.** Reached Stage 1 only (Evaluator scoping, no code) as build 4 of a six-build Spool spin; `5b89e39` is one file, the change brief. Artifacts in `pipeline/shared-control-primitives/`. **This entry replaces the reasoning of the v1.0.19 decline above rather than adding to it.**

**THE v1.0.19 DECIDING FACT IS WITHDRAWN.** That entry rested on this: the guard's population is intrinsic tags, so every migrated site is a site `tabOrderCoverage.test.ts` stops watching, and a full migration would delete the guard by attrition. Re-measured at HEAD, **that is not decisive.** It is a solvable change to the guard -- count primitive call sites as population -- and what it actually yields is an **ordering constraint, not a bar**: the guard must be made primitive-aware BEFORE the intrinsic population shrinks, never after, or the guarantee lapses mid-migration. One caveat on this record, stated because the brief's own wording invites over-reading: the brief calls it "a twenty-line change, measured below" and its body does not discharge that forward reference. What is actually recorded is the assertion that it is small, the ordering rule, and the re-derived population; the twenty lines are the brief's estimate, not a measurement.

**THE DECLINE NOW STANDS ON THE OBJECTION v1.0.19 LISTED FIRST AND TREATED AS SECONDARY, and fresh measurement makes it stronger in a way that changes the shape of the answer: the app already HAS its shared button contract, and it is a CSS class register rather than a React component.** Re-measured at HEAD after three builds had landed since the v1.0.19 tag, **181 of 243 control sites carry a bespoke inline `style`** (the total grew by 8 from v1.0.19's 235 while the inline-styled count did not move). So a primitive over today's code is a passthrough whose only content is a `tabIndex` default -- and the register that IS the contract is named by its classes, `sr-btn-quiet` and `sr-btn-accent` in `globals.css`, which have no component owner. That fixes the sequencing rather than merely deferring the work: **the design-system register pass is not a nice-to-have that would make primitives better, it is the thing that would give a primitive anything to own**, and it remains a separate, unscheduled build.

**TWO FURTHER CORRECTIONS, both of which shrink the case rather than restate it.** The `<Link>` half is **redundant, not merely costly**: `OutboundLink` already is that primitive, and sets `tabIndex` before its `{...rest}` spread so a caller can still override. And v1.0.16's clause that "primitives would need an override prop for the roving groups, which is the same class of mistake one level up" is **withdrawn outright** -- a primitive needs no override at all. The v1.0.19 entry's other findings survive intact: the tab-stop default the primitive would buy is already enforced loudly by the guard, and the exclusion roster is four rows of three different kinds (two roving groups, one non-roving redundant chevron, one base-map button the platform removes via native `disabled`, which is not a tab-order decision at all) rather than the three roving groups the idea recorded.

**WHAT WOULD CHANGE THE ANSWER, so a third scoping is not paid for from scratch:** a design-system register pass giving `sr-btn-quiet` and `sr-btn-accent` a component owner, with the guard made primitive-aware first; the inline-style population falling well below today's 181 of 243, so a primitive would absorb something; or a second app or surface needing the same controls, which is what usually makes a passthrough worth its layer. **Absent one of those, a re-queue is answered with this brief.**

**THE IDEA WAS NOT RE-FILED, AND THAT IS THE DELIBERATE DIFFERENCE FROM THE FIRST DECLINE** -- but the consequence is that the original inbox row still carries the pre-decline text, naming a 218-button refactor across 79 files and an override prop for three roving groups, both of which are now explicitly withdrawn. Whoever next meets that row at a lane fork should read this entry rather than it.

---

## The 1.0.19 screenshots were held on a premise nobody checked twice, and the App Store submission was withdrawn and resubmitted to carry the correction -- 2026-09-05 (v1.0.19)

**Decision: recaptured and resubmitted**, hours after the bundle shipped, on the user's challenge. `fab7d9c` carries all twelve App Store PNGs and the two Statistics website shots; the App Store version record's two sets were replaced whole and in order; version `917a20fd-6ea2-427e-aed8-aa020c08e47d` was withdrawn from review (`DELETE /v1/appStoreVersionSubmissions/{versionId}`, which moves it to `DEVELOPER_REJECTED`) and resubmitted as `c9e26fd6-1142-40bf-b251-c343324bcd75`, `WAITING_FOR_REVIEW`, with build 1.0.19.1, the same review notes and the same release notes.

**THE FALSE PREMISE, which is the durable part.** The 1.0.19 build deferred five screenshot sets because "this rig has no eBird or OpenWeather key", a claim three separate agents made after reading ONE location, `backend/.env`, which is empty. Both keys are in the desktop app's own store at `~/Library/Application Support/com.snowraven/data/api-keys.json`, and both verify against the live APIs with One Call 3.0 subscribed. A stated blocker went unchallenged through a QA pass, a security review, a deployment record and a closeout, because each reader inherited it from the last rather than re-deriving it. **A resource claimed absent is checked in every place it could live before it is allowed to defer work**, and the app's own storage is the FIRST place to look for a key the user's app plainly uses.

**A SECOND TRAP, and the reason a naive retry would still have failed.** Exporting `EBIRD_API_KEY` and `OPENWEATHER_API_KEY` into the backend process is not sufficient. `backend/routers/apikeys.py` answers `GET /settings/keys` with `dotenv.get_key` against the FILE, not the environment, and that response is what `BirdingStats.tsx` reads to set `hasEbirdKey`; the service layer reads the env. So a capture with env vars alone renders "No eBird key" while every outbound call would have worked. The capture ran uvicorn against a scratchpad ASGI wrapper that exports both vars, writes a 0600 temp env file OUTSIDE the repo and repoints `routers.apikeys.ENV_FILE` at it. `backend/.env` is still zero bytes and the repo was never modified. Anything that needs keys at capture time uses that shape; do not write the user's keys into a tracked file.

**WHY THE STORE LEG COULD NOT WAIT FOR THE NEXT RELEASE.** The staleness was not cosmetic. Every App Store shot shows the navigation, and the iPhone `01-map-explorer.png` showed a bottom bar of Weather, Statistics, Calendar, Species Detail while displaying the Map Explorer, a tab reached through More. That is precisely the arrangement 1.0.19 changed, so the store would have advertised the old behaviour using a picture of the screen this release moved. Withdrawing cost about half an hour of queue position because review had not started; the cost rises to a wasted review cycle once it has, which is the reason to decide this immediately rather than at the next ship.

**`weather.webp` re-rendered BYTE-IDENTICAL**, retiring the standalone "regenerate it on a keyed rig" roadmap item open since v1.0.4 by proving the committed frame was always a genuine live capture. Its checklist is dated Jun 7 2026, so the timemachine and tide-prediction results are deterministic. Nine other website shots also re-rendered byte-identical, which is a useful determinism signal for the capture tooling.

**One accepted state, recorded so it is not mistaken for a regression.** The Statistics frames photograph the app's FALLBACK glyphs (a Globe and a SquareLibrary in app ink) rather than the two site favicons, because those contexts require the escapee-provenance route stub and registering any Playwright route cancels every cross-origin image load in the context. Before 1.0.19 those slots photographed EMPTY, so this is strictly better than what shipped. The honest fix is capture-side, serving the stub from the backend instead of from a Playwright route, and stays on the roadmap.

---

## A route registered anywhere on a browser context cancels every cross-origin image in it; a default's migration reaches only the default it replaced; and a property claim is still false when its POPULATION is wrong -- 2026-09-05 (Spool bundle, v1.0.19)

**Decision: shipped** as a five-build Spool bundle -- `ecedd55` (capture route scope), `4489d5d` (Map Explorer before Calendar, which carried the four-file version stamp), `a470cea` (species-link glyph fallback, with a design pass), `b842849` (shared Button/Link primitives, DECLINED at scoping with no code -- its own entry below), `14b314b` (map popup keyboard close), released at `75d3bb6` with tag `v1.0.19` and the iOS stamp at `e5d6264`. macOS universal notarized and stapled, the Windows installer from GH Actions run `33972202471` (the stale 1.0.18 run correctly rejected), the website at 1.0.19, and iOS build `1.0.19.1` VALID on TestFlight. Security PASSED WITH NOTES on all four built runs, no Critical or High.

**A registered Playwright route cancels every cross-origin `<img>` load on its context, whatever the route's pattern.** Measured on Playwright 1.62.1 / Chromium 1234: with no route both favicons load (`naturalWidth` 48); with the real `**/checklists/**` stub both die with `net::ERR_ABORTED canceled=true`; with a deliberately non-matching `**/zz-never-matches-zz/**` the result is identical. No CORS error, no blocked reason, eager or `loading="lazy"`. Same-origin traffic and fetch-initiated cross-origin loads are unaffected -- it is specific to `<img>` element loads, which is why the app rendered, `/taxonomy/codes` resolved and the map tiles drew on an intercepted context while every species mark went missing. **So a capture-time stub belongs in a per-shot hook scoped to the contexts whose frame depends on it, never in a shared page factory**: `capture.mjs` went from registering the stub on 10 contexts to 3. The cost already paid was four published screenshots with empty 14px slots where an online visitor sees the eBird and Birds of the World marks, surviving three recaptures (`43d5593`, `6790e74`, `a836c57`) because an eye check at page scale cannot see a 14px hole -- magnify a crop and compare against the previous committed image. Accepted and stated: the three Statistics contexts keep the stub, because without it the escapee pass 404s and the tab photographs "eBird could not be reached", so the one glyph in their frame stays absent.

**A `DEFAULT_TAB_ORDER` change ships with a read-time migration of the IMMEDIATELY PREVIOUS default and nothing older.** The previous default is kept as a named constant so the check is one equality rather than a heuristic; the comparison runs AFTER the existing unknown-id drop and missing-tab append; the hidden set is preserved; nothing is written back, so hydration still never persists. This narrows v0.5.41 and v0.5.42's "saved layouts preserved" to **"saved CUSTOM layouts preserved"** -- an order identical to the previous default is not a custom layout, and a pre-Calendar (0.5.42) layout deliberately does not migrate. The migration is in scope at all, rather than a follow-on, for a reason worth reusing: the tab layout is device-local (iCloud syncs the two data files only, never settings), so a default that reaches only fresh installs would never reach the phone the idea was about. Left open and pre-existing, both in `tabLayout.ts:113-120`: the persisted order has no length bound, and duplicate ids are preserved.

**A build may narrow its own scope freely; it may not widen into a shipped design spec or a published statement.** The species-links Evaluator's first proposal was to remove the favicons entirely, which reverses `pipeline/species-links/design-spec.md`'s States table and rewrites `PRIVACY_POLICY.md:74` and its two mirrors, so it was not a call the build could make: it went back to the user as a separate idea and the build did what the queued idea asked. **The idea's other half, a durable favicon cache, was declined as REDUNDANT rather than merely out of scope** -- the cache existed to make the marks work offline, and the fallback delivers exactly that, because offline is precisely when the glyph renders. (`countyCompletenessCache`'s lineage is a JSON-document pattern with a code-unit budget, structurally wrong for image bytes.) What settled the glyph's token was measurement: at the mark's shipped 0.75 resting opacity the muted tokens fail the 3:1 non-text floor in light theme (`--sr-text-muted` 2.93:1, `--sr-text-gray` 2.77:1), while `--sr-text` clears at 7.96:1 light and 7.63:1 dark in the worst case. Cost of the two bundled glyphs: **+226 bytes** on the entry chunk, raw and gzipped.

**The app-owned popup close button is the house pattern, and it closes an accessible-name gap as well as a tab-order one.** maplibre-gl 5.24.0's `_createCloseButton()` sets exactly two things -- `type = "button"` and `innerHTML = "&#215;"` -- and **no `aria-label`**, so the library's own button announced as the times sign on every platform. Five popups now take `closeButton={false}` plus an app `<button tabIndex={0} className="maplibregl-popup-close-button">` named for the popup it closes, and a sixth (`BirdingStats.tsx`) was renamed off "Close popup" so `ACCESSIBILITY.md`'s naming clause became true of all seven rather than being narrowed to fit. Nothing is stamped onto library DOM imperatively, which is what keeps the claim inside the source guard's reach. One cross-platform side effect is stated rather than hidden: maplibre's `addTo()` calls `_focusFirstElement()` and the app button is appended first, so focus on open now lands on the close button rather than the first link. **Escape stayed out of scope deliberately** -- no popup in the app has ever had one, maplibre 5.24.0 has no popup Escape handling anywhere, and no map popup traps focus, so this is WCAG 2.1.1 operability and dismissibility and not a 2.1.2 keyboard trap; grading it as 2.1.2 would overstate it by a full grade.

**A property claim is still false when its POPULATION is wrong, and both QA hand-backs in this spin were false claims in prose rather than code defects.** `ACCESSIBILITY.md:19` published "Every popup any of these maps opens carries its own close button" -- falsified by the two popups that have none, and already contradicted by `:95` in the same file. That is v1.0.16's PUBLISH-THE-PROPERTY rule failing on its other axis: the sentence stated a property rather than a count and was false anyway, because five of seven is not "every". So **check the population as carefully as the property**, and repair at paragraph scope; the repair here is a conditional ("Where a popup on these maps has a close button...") naming the exceptions. The second hand-back was a struck design premise surviving in `website/tools/capture.mjs:89-98`, a third source file no brief named, whose accepted-cost sentence contradicted the very build that changed it. **A source sweep is scoped by the CLAIM, not by the file list a brief names**: grep the premise's distinctive wording across live source, and treat a stale accepted-cost note as the tell. Three more struck premises were swept the same way (`MapExplorer.tsx:985-987`, `map/SharePopup.tsx:189`, `map/SharePopup.test.tsx:101`). Between the two map-popup rounds no code, test, changelog or version file changed, checksums re-verified.

**A test stub must never supply a control the app is supposed to own.** `SightingsMap.test.tsx` carried a comment asserting maplibre rendered `aria-label="Close popup"`, which is false of 5.24.0 and is how a real keyboard gap sat behind a green suite: the test believed in a control the library never drew.

**The App Store position, in writing, because a deferral is a decision and never a silence.** 1.0.17 is READY_FOR_SALE and live; 1.0.14, 1.0.13 and 1.0.4 are also READY_FOR_SALE. **1.0.18 never received a version record and never will** -- it was held at ship pending 1.0.17's approval, and it is now deliberately SUPERSEDED by 1.0.19 rather than skipped by omission, since everything it added ships inside this release. **1.0.19** has version record `917a20fd-6ea2-427e-aed8-aa020c08e47d`, **WAITING_FOR_REVIEW** with build `1.0.19.1`, submission `a72ae906-2464-47c2-8146-5ad15c013f6d`. Recorded here rather than on the roadmap because v1.0.17's lesson was precisely that a roadmap item naming the debt was not enough: it was item 1 in Up Next while 1.0.16 shipped past it.

---

## Shared Button/Link primitives are DECLINED on measurement, and the deciding fact is that migrating a control REMOVES it from the guard that enforces its tab stop -- 2026-09-05 (Spool bundle, v1.0.19; DECIDING FACT SUPERSEDED 2026-09-06, see the v1.0.20 entry above)
...
**(3) SUPERSEDED -- what this entry called the deciding fact: the guard's population is INTRINSIC TAGS, so every migrated site is a site the guard stops watching.** *(Re-measured at HEAD on 2026-09-06 and withdrawn as decisive: making the guard count primitive call sites as population is a small change, and what remains is an ORDERING constraint -- primitive-aware guard first, migration second -- rather than a bar. The decline stands on finding (1) instead, sharpened; read the v1.0.20 entry above before acting on this paragraph.)* Its own non-vacuity floors (more than 150 buttons, more than 10 anchors) leave headroom of 71 buttons and 3 anchors before it goes red on emptiness -- so even a "bounded subset" migration is bounded by the guard rather than by coherence, and a full migration would delete the guard by attrition. A migrate-nothing variant that failed a NEW intrinsic control would have to fail a correct `tabIndex={0}` control, the pattern 235 shipped sites use and `.claude/rules/ui.md` mandates, or carry a 235-row baseline roster -- the blanket-pardon shape v1.0.16 closed with cardinality.

**The idea's recorded size is stale in both directions and its override premise is wrong.** *(Sharpened 2026-09-06: the override premise is not merely wrong about the roving groups, it is withdrawn entirely -- a primitive needs no override prop at all -- and the `<Link>` half is redundant rather than costly, because `OutboundLink` already is that primitive.)* Measured by the guard's own AST population: **235 sites (221 `<button>` plus 14 `<a href>`) across 55 of 80 shipped `.tsx`**, against the idea's 218 buttons across 79 files. The four non-`{0}` sites are not "three roving groups": they are three different kinds -- two roving groups (the navigation's vertical tablist over 11 destinations, and Settings' `RadioGroup` over three groups), one non-roving redundant chevron, and one base-map button the platform removes via native `disabled`, which is not a tab-order decision at all. A single boolean override prop cannot model three kinds, so the trap the idea names is worse than it recorded.
...
**THE 2026-08-12 DECISION "A decline REVERSED on a measurement" IS MODIFIED IN ONE SENTENCE AND RELIED ON IN EVERY OTHER.** Its scope sentence, "Surfaces that list species rather than headline a count are untouched", now carries one named exception, Species Detail, by user direction from the idea inbox: the Species Detail species count did not match Statistics. Measured, not assumed: both tabs load the same parsed backup and derive the same species set (demo export, 149 and 149, identical), and the one mechanism by which they differ is `countableLifeList(lifeList, excludedNames)` on Statistics, which Species Detail had no counterpart for; on the reference export that is 267 against 264 (Graylag Goose, Swan Goose, Muscovy Duck). The exception is carried behind an off-by-default `Show escapees` reveal switch, so at both tabs' defaults the two figures agree and one press restores the rows and the number. Everything else in v0.5.87 is extended and relied on rather than touched: Statistics remains the ONLY initiator of a provenance pass (the tab imports `useProvenanceLookup` and never `useExoticProvenance`, and `exoticProvenanceGraph.test.ts` pins it); the tab is a passive reader WITH the confirmation step, so a published name whose carrying checklist is not in the ledger stays visible; an unresolved species counts; and no offline heuristic stands in for provenance. The 2026-08-09 decline's standing conclusions still bind (no provenance column in the export; `category === 'domestic'` forbidden under any label), and v0.5.89 decision 2 (a tab's rows and its own count agree by construction) extends to the escapee half: the count is the length of the display list, so the figure and the selector cannot disagree. With an empty cache every number and option on the tab is byte-identical to 1.0.17. The switch is still rendered then, because a control that appears only after a Statistics visit is a layout shift and a discoverability gap.

**THREE DESIGN DECISIONS, ALL CONFIRMED BY THE USER AT THE DESIGN GATE.** (1) **The label is `Show escapees`.** This tab's verb family is "Show" (`countabilityCopy.ts` records that Multimedia and Species Detail move ROWS while Statistics and the Calendar move a NUMBER), the noun is `Count escapees`'s so the two tabs read as one rule in two verbs (the exact `Count all forms` / `Show all forms` pattern), it is positive and non-enumerating, and it carries no attribution, which is made once in the Statistics rule line. Rejected: `Show eBird escapees` (attribution the neighbours do not carry, and 19 characters where every character is a line at 320px), `Include escapees` (a count verb in disguise), and `Count escapees` here (the wrong verb for a control whose first effect is rows). (2) **No rule note and no `ExoticProvenanceAccount` panel on this tab** -- v0.5.89 decision 4 applied: the toggle is the interactive account. The Calendar's note exists because the Calendar has no escapee control of its own; here the control is on the row, its label names the class it governs, and the evidence lives on Statistics one tab away, whose list links each name back here. **Accepted cost, named:** a birder who has never visited Statistics sees a switch that changes nothing, and nothing on this tab says why. (3) **An app-wide hover state on the boxed `ToggleSwitch`** (border to `--sr-border-medium`, fill to `--sr-surface-subtle`, 120ms ease-out), the run's one deviation from shipped chrome, because the doctrine asks for hover and focus set on purpose and the boxed variant had none. It lands on every boxed switch (Species Detail, Multimedia, the Statistics Counties switch, the Checklists weather switch) and is gated off `disabled` and `aria-disabled`, so an inert switch never lights up as operable. **The mechanism required moving THREE inline declarations, not the two the design named:** `border`, `background` AND `transition`. An inline `transition: opacity 150ms` is specificity 1,0,0 and would have beaten the class rule's `border-color 120ms`, so the hover colours would have landed with no motion and nothing would have gone red; the whole chrome moved to the `.sr-toggle` class and the inert opacity fade rides along unchanged. Recorded in `.claude/rules/ui.md`.

**THE PREFERENCE IS SEPARATE AND SESSION-ONLY, and the reason is what a shared store would DO.** The idea's requirement is that the DEFAULTS agree, which both switches defaulting to off satisfies. A store shared with Statistics' `includeEscapees` would let a press on Statistics silently reshape the Species Detail selector, and possibly deselect the open species, while that tab sits hidden but mounted (tabs are hidden with `display: none`, never unmounted). No toggle in this family persists -- `includeEscapees` and `includeSpuh` on Statistics, `showSpuh` here -- and the one persisted-toggle precedent (`hotspotTierRings`, v1.0.2) was a logged deviation for a vision aid. So `showEscapees` is its own `useState(false)`, resetting on relaunch, and `docs/HELP.md` says all three switches are per-session.

**A REQUEST FOR A HIDDEN SPECIES REVEALS IT RATHER THAN DROPPING IT, and the fix closed a pre-existing silent drop with the same condition.** `openSpeciesInTab` returned silently when its target was absent from the display list, so a click on an escapee's name in the Statistics disclosure would have opened the tab with nothing selected. The shipped shape is one shared `revealAndSelect`: a target absent from the display list but present in the full key list turns on whichever switch hides it (`!countableKeys.has` for Show all forms, `escapeeNames.has(normalize)` for Show escapees), then selects. Because it is one condition, the identical drop that already existed for a name hidden by Show all forms (a hybrid clicked on Multimedia with that tab's Show all forms on) closed with it, and the Subspecies and forms control goes through the same helper because it lists every species with a form noted, which includes an escapee carrying a domestic-type form. A crafted name (`constructor`, `__proto__`, anything not in the export) fails both lookups and returns before any state moves; the suite pins it.

**THE APP STORE LEG IS HELD, VERIFIED AGAINST APP STORE CONNECT AND RECORDED IN WRITING, which is the check CLAUDE.md's release rhythm requires since v1.0.17.** 1.0.17 was submitted the morning of this ship and sits at `WAITING_FOR_REVIEW`; the standing rule that opt-in polish does not replace an in-review submission applies (precedent: the v1.0.2 hold and the v1.0.5 pass), because withdrawing 1.0.17 to swap in 1.0.18 would surrender the queue position for a change that is off by default and reads the same as 1.0.17 until the switch is pressed. The user decided HOLD at the deploy gate. Version records read from the ASC API before the ship and again after the TestFlight upload, identical both times: 1.0.17 `WAITING_FOR_REVIEW` (submitted 2026-09-04, unchanged by this ship); 1.0.14 `READY_FOR_SALE` (the live store version); 1.0.13 and 1.0.4 `READY_FOR_SALE` (historical); 1.0.18 no record, deliberately. 1.0.18 follows as an ordinary update after 1.0.17's approval, and because build `1.0.18.1` is already uploaded and VALID that later submission is a metadata action only (select the build, review notes verbatim from `appstore/REVIEW_NOTES.md`, the `reviewSubmissions` flow with the metadata key). Nothing was created or withdrawn in App Store Connect on this ship. One ASC fact promoted to the release skill: `GET /v1/apps/{id}/builds` rejects `sort`, so builds are listed with `GET /v1/builds?filter[app]={id}&sort=-uploadedDate`; the prior helper printed no build rows because of it.

**THE `Pipeline` WORKFLOW'S FRONTEND JOB IS RED ON EVERY PUSH TO MAIN, PRE-EXISTING, AND THE DECISION IS DEFERRED TO THE ROADMAP.** It has failed on every push since the 1.0.16 iOS stamp (`bc6e5c2`), with the failing identities VARYING between runs (`MapExplorerCenterShareFab`, `MapExplorerSearchThisArea`, `Settings.clear`, `Settings.icloud`), none in a file this build touched, while the same full suite is 4507 green on Hephaestus at first pass and the Backend job passes every time. It gated nothing that shipped (macOS and Windows come from `release.sh` and `windows-build.yml`, the site from `pages.yml`), so it was recorded as observed rather than chased inside a ship. What it needs is either a stabilisation pass on the ubuntu/Node 20 runner or an explicit decision to treat the workflow as advisory; until one is taken, a red `Pipeline` badge on main says nothing about the change under it.

**Two pre-existing Lows and one Informational, all recorded rather than fixed here.** The `browserslist <= 4.28.6` dev-only advisory is re-confirmed (`npm audit --omit=dev` clean; the shipped bundle is unaffected). `SpeciesDetail.tsx` and `SpeciesLinks.tsx` still read `taxonMap` / `formTaxonMap` / `taxonOrders` by bare index with a CSV-derived key and interpolate `speciesCode` into a fixed-host href with no shape gate -- untouched lines, joined to the existing `Object.hasOwn` sweep item rather than given a new one. And `docs/HELP.md`'s Statistics paragraph closes with "All of these places read the cached answer and never fetch anything themselves", which with Species Detail now in its antecedent is literally broader than the code (the tab issues its own pre-existing `/taxonomy/codes` request); HELP is compiled into the app, so it is a next-release edit, not a post-ship one.

---

## A bottom bar was rejected in 2026 on a count the app has since outgrown, so half of that decision is reversed and half is extended; a load-bearing justification outlives the thing it defended, SILENTLY; and the App Store leg was missing for two releases with nothing in the repo able to say so -- 2026-09-04 (v1.0.17)

**Decision: shipped** as an Improve run with a design pass -- `a836c57` (the change), `08f5a3e` (iOS 1.0.17 build 1 stamp), `0e8b0c5` (review notes), tag `v1.0.17`. Every leg shipped the same day: macOS universal notarized, the Windows installer signed with `latest.json`, the website and all 23 published screenshots, iOS build 1 on TestFlight, and the App Store submission at `WAITING_FOR_REVIEW`. Security **PASSED WITH NOTES**, no Critical/High/Medium: two Lows, both closed inside the run rather than deferred (the capture script's missing fail-closed demo-dataset guard, and a doc comment crediting a focus trap with an arm its call site does not enable), and one Informational accepted.

**THE 2026-05-27 DECISION "Responsive navigation: dropdown over bottom bar, overflow-driven collapse" IS PARTLY REVERSED, AND THE HALVES ARE WORTH SEPARATING, because one was invalidated by growth and the other was vindicated by it.**

*Reversed -- the dropdown, and the ground it stood on.* That decision rejected a bottom bar because "a bottom bar tops out at ~5 destinations and the app has 8." The premise was sound arithmetic and the conclusion did not survive two facts. First, **the mitigation it never considered**: a bottom bar does not have to hold every destination. Four favourites plus a More sheet holds any number, and it holds them in the app's *own saved order* -- the first four visible tabs -- so the reorder-and-hide setting that already existed chooses the favourites and nothing new is stored. The cap the decision treated as structural was a cap on one implementation of a bottom bar. Second, **the count went from 8 to 11**, which did not merely stress the dropdown, it made the dropdown the *usual* case: eleven destinations needed ~1409px of strip against a ~1457px collapse threshold, so an unmaximised laptop window, an iPad, a half-screen Mac window and every phone got a single scrolling list of eleven. A navigation that "scales to any count" scaled by degrading everyone to the same list. Its third clause is superseded outright: the note that a future mobile app should inherit the dropdown rather than invent a separate navigation was right about the *principle* -- one navigation, not two -- and wrong about which one, and the phone bar is now the shape the principle produces.

*Kept and extended -- collapse by measurement.* "Overflow-driven collapse rather than a fixed pixel breakpoint" is the half that got stronger. It now selects among three densities instead of two, from the same measured input, and the arithmetic is `available − navW − reserve ≥ 640` where `reserve` is published by any tab holding its own in-flow sidebar (only the Map Explorer today) and `navW` is resolved live from the root font size so it tracks `--sr-text-scale`. **The 640px floor is not a number of taste**: it is the app's own phone boundary, so the threshold expresses a rule the app already holds -- the sidebar may never squeeze the content column into the tier below it -- rather than introducing a constant someone must later defend. The Map Explorer case falls out of that arithmetic rather than being special-cased: a 2560px monitor keeps both sidebars, a 1024px window does not. **Hysteresis is new and was not optional** (collapse below the floor, restore only at floor + 48): a bare equality flips on every pixel of a window drag, and a density change is a visible layout change. Two hazards retired with the strip: the off-screen measurement probe that leaked page horizontal scroll on phones in v0.5.37 is gone, because the threshold reads the root font size instead of probing; and the roving-focus roster went from three groups to two, the tab-order exclusion roster from five rows to four, because the collapsed dropdown's `role="option"` listbox is **retired rather than rewritten** -- the More sheet uses plain trapped buttons at `tabIndex={0}`.

*Untouched, and deliberately so.* The two tab-layout persistence decisions are reused as-is -- same document, key, seam and normalizer -- and the collapse toggle is **session-only** precisely to keep it that way: a persisted density can be restored into a window where it is wrong, leaving the user to find a control to undo a state they do not remember setting. The toggle also narrows only, so the derived density is a ceiling and the two can never contradict each other.

**A LOAD-BEARING JUSTIFICATION OUTLIVES THE THING IT DEFENDED, AND IT DOES SO SILENTLY -- THIS IS THE ENTRY'S MOST TRANSFERABLE FINDING.** The website capture width was raised from 1440px to 1600px in v0.5.77 for one reason: to clear the strip's ~1457px collapse threshold so the tooling photographed a strip rather than a dropdown. **That threshold no longer exists, and nothing failed when it stopped existing.** The width is still right, for an entirely different reason -- it now decides which *density* is photographed, and the binding shot is the Map Explorer at ~1156px against ~856px for every other tab -- but a number kept for a dead reason is a number nobody can safely change. `website/tools/capture.mjs`'s header was rewritten with the new arithmetic during the build. **What the build missed, and this closeout found, is that the same dead justification was also written into `.claude/rules/docs-and-website.md`** -- the rule file that promoted it -- where it still said "adding a tab may require raising it," which is now the wrong trigger entirely (the trigger is a nav label growing or a tab gaining its own in-flow sidebar). Its neighbouring v0.5.84 clause was wrong in the other direction, warning that a probe at 1440px "finds no `[role="tab"]` strip at all" when the vertical tablist is now present at every non-phone width and absent only at phone widths. Both are corrected. **When a change removes a mechanism, grep for the things that were justified BY that mechanism**, not only for descriptions of it.

**A CLAIM THAT REACHED THE PUBLISHED PROSE *FROM* THE SOURCE MUST BE SWEPT STARTING AT THE SOURCE -- this cost two QA rounds and is the same failure as the paragraph-scope rule one level up.** The design pass opened on a layer that does not exist: it asserted that every tab already draws a house page header naming itself, and used that as the load-bearing answer for *where you are* at icon-rail density. Measured, **zero of the eleven destinations name themselves in a page heading and five render no heading whatsoever.** The premise was struck and the rail's identification rests instead on three layers that all ship (`aria-label` always present, tooltip on hover and `:focus-visible`, touch-hold), with the residual limit stated plainly rather than implied away. But the false claim had already reached `docs/HELP.md` and `README.md`, and the sweep that removed it covered `docs/`, `README.md`, `ACCESSIBILITY.md` and the website **and never `frontend/src/`** -- leaving it alive in a doc comment in the very file the fix was about. `.claude/rules/docs-and-website.md` already said the publish-the-property rule covers comments; what was missing was the sweep's *starting point*. The repair kept the struck claim written down **as a framed refutation rather than deleting it**, so the next reader cannot re-derive it from scratch, and CLAUDE.md's docs rule now names `frontend/src/` and `.claude/rules/` as in scope. The capture-width survivor above is the second instance in the same run, which is why this is a rule and not an anecdote.

**A DOM NODE A CHILD MEASURES OR OBSERVES IS PASSED AS STATE, NEVER AS A REF OBJECT.** An ancestor's ref is not attached when a descendant's layout effect runs, so the first measurement reads `null` and the `ResizeObserver` never attaches -- and nothing throws, nothing logs, and the component renders a plausible default forever. It is invisible in jsdom and invisible in review, because the code looks exactly like code that works. Both the shell measurement that picks the density and the bottom bar's own height hit it, in one change, which is what makes it structural: the shape recurs wherever measurement lives one level below the box being measured. `useLayoutEffect` does not save it -- child effects run before parent effects, which is the whole problem. Recorded in `.claude/rules/ui.md`.

**`inert` REMOVES A SUBTREE FROM FOCUS AND THE ACCESSIBILITY TREE, NEVER FROM THE PAINT -- and the instrument that told us otherwise was answering a different question.** A fixed element at the shared `z-index: 1200` overlay level, rendered later in the DOM than `.sr-map-fullscreen-panel`, paints over a fullscreen map no matter how inert it is; the phone nav bar is therefore **not rendered at all** in fullscreen, rather than merely made inert like the header and footer, which are in normal flow and genuinely covered. **`elementFromPoint` returned the map and pixel sampling returned the overlay, and the pixels were right**: `elementFromPoint` answers *what would receive this click* (hit-testing, which `pointer-events` and `inert` participate in), pixel sampling answers *what the user sees* (paint, which neither touches). Each is a convincing answer to the other's question. Name the question before choosing the instrument. Recorded in `.claude/rules/maps.md` and `.claude/rules/testing.md`.

**A COMMENT ABOUT A SHARED HOOK MUST DESCRIBE BEHAVIOUR AT *THAT CALL SITE*, AND THE FIX WAS THE COMMENT -- MEASURED, NOT ASSUMED.** `NavMoreSheet`'s doc comment credited its focus trap with a `focusin` containment arm the call site does not enable. The obvious repair -- enable `containOutsideFocus` so the comment becomes true -- was tried and **breaks both focus-return tests**: the `focusin` arm pulls focus back into a sheet that is unmounting, which is the F061 defect. So the comment was corrected to name the options this call site actually passes. A comment that overstates a shared helper is not a documentation bug; it is an invitation to a revert that will look like a fix.

**THE FLAKE THAT WAS NOT A FLAKE, AND THE COST OF "NOTHING ELSE COMPILING" NOT MEANING "QUIET".** A `mapCornerTabStops` failure -- one of the four identities already named in the v1.0.16 full-suite flake profile, which is exactly what made it convincing -- was reasoned about across two stages as though this change had caused it. It had not. **Six orphaned `/bin/zsh` busy-loops, PPID 1 and ten hours old, were pinning six of eight cores at load average 24**; the suite ran 51s instead of 32s. Nothing in the session had started them, and they are consistent in count, shell and shape with the "six deliberate CPU spinners" the v1.0.16 baseline itself ran -- so the second rule here is that **a deliberate load experiment owns its teardown**, or it taxes every session that follows it on that machine. Check load average and orphans before judging a timing-sensitive run; a green re-run after killing them is evidence, a re-run without checking is a coin toss you will read as a finding. Also from this run: **a pipeline reports the last command's exit code**, so `cmd | head` read `exit=0` over a command that exited 1, while verifying a security fix.

**THE APP STORE LEG WAS MISSING FOR TWO ENTIRE RELEASES, AND A ROADMAP ITEM NAMING THE DEBT WAS NOT ENOUGH TO CATCH IT.** 1.0.15 and 1.0.16 each shipped a `VALID` TestFlight build and **neither ever got an App Store version record**, so the store sat on 1.0.14 while two releases went out to every other platform. This was found by querying App Store Connect during this ship -- **nothing in the repo could have surfaced it**: a TestFlight build is not a submission, and the tags, the changelog and the release assets look identical either way. The deferral itself was correct when taken (1.0.14 was `IN_REVIEW` and replacing a build in review surrenders its queue position, which is the standing rule operating, not a miss); what failed is that a deferral with no verification step became a silence, and the silence then covered a second release taken under no such constraint. **The ROADMAP had it as item 1 in Up Next and 1.0.16 shipped straight past it**, which is the part worth recording: a roadmap item is a memory aid, not a gate, and a leg that can be silently skipped needs a check in the runbook rather than a note in a plan. CLAUDE.md's release rhythm now requires listing the last few releases' version records against ASC at every ship and recording any deferral **in writing, with its reason**. 1.0.17's release notes accordingly cover three releases.

**One icon changed, and it changed at the authoritative table.** Multimedia's lucide `List` was indistinguishable from Checklists' `ClipboardList` at rail size, so it became `Images` -- approved by the user at the design gate, and edited in the one glyph table so it reaches every density and the Settings reorder list from a single source. `TAB_LABELS` is untouched: the destination is still named **Multimedia**. One new token, `--sr-nav-bar-shadow`, the upward twin of `--sr-sticky-shadow`, defined in both themes before use.

---

## The WebKit default tab mode is a platform fact, not an accessibility nicety; a published claim states the PROPERTY and a repair is swept at PARAGRAPH scope; and a queued fix was REJECTED because permanent and transient failures want opposite treatments -- 2026-09-03 (v1.0.16)

**Decision: shipped** as a four-build Spool bundle on one version -- `2f06215` map-fab-keyboard-reachable (Fix), `37a49eb` webkit-tab-order-app-wide (Fix), `aeb6bbd` storage-read-throw-containment (Fix), `3e9c162` weather-backlog-honest-load-failure (Fix); bundle merge `2e92478` (tag `v1.0.16`), iOS stamp `bc6e5c2`. Two further commits sit inside the tag range (`83b952e`, `1ec0290`) and are **not** a fifth and sixth build: they are the real-engine tab-order premise harness and its path fix, test-harness work belonging to build 2. The bundle commit message says `6 build(s)` and is wrong; it was investigated before the deploy rather than after, precisely because if two builds had been real they would have shipped without Tester and Auditor sign-off, and `git diff --name-only v1.0.15..v1.0.16 -- pipeline/` lists exactly four build directories. All four carry PASSED QA and PASSED WITH NOTES security reports with no unresolved Critical or High. There was no separate release-prep commit: build 1 carried the version bump and the website pill, and builds 2 to 4 appended to the existing `## [1.0.16]` changelog section. Three legs shipped the same day: macOS universal (notarized, stapled, `lipo` confirmed `x86_64 arm64`) and the Windows installer re-signed locally with the real minisign key plus `latest.json`, the website, and iOS 1.0.16 build 1 on TestFlight (`VALID`). **The App Store leg is deliberately DEFERRED and is owed** -- 1.0.14's submission was still `IN_REVIEW`, replacing a build in review surrenders its queue position, and the user chose to keep it. 1.0.15 was never submitted and has no version record, so one submission of 1.0.16 discharges both.

**THE QUEUED FIX WAS REJECTED AND THE OPPOSITE SHIPPED, because a PERMANENT MEMOIZED failure and a TRANSIENT PER-CALL failure want opposite treatments and the 1.0.15 precedent covers only the second.** The queued item asked to move `await this.fs()` inside `TauriStorage.readFile`'s `try` (`lib/storage.ts`), by analogy with the v1.0.15 cache fix that moved a stored-file read inside `loadFresh`'s own `try`. The analogy fails on the property that decides the case. `loadFresh`'s failure is **transient and per-call** inside a layer that clears its memo, so `inflight` resets, the next mount retries, and quieting the throw to `null` is honest because "try again" is a true thing to offer. `fsModule` is assigned once with `??=` and never reset, and the adapter is a module singleton, so a failed dynamic import of `@tauri-apps/plugin-fs` **stays rejected for the whole session**: folding that into a `null` return converts a loud permanent failure into a silent permanent "you have no file" with nothing to retry, which is exactly the "report a failure as no data" the 1.0.14 family exists to remove. The move is also provably behaviour-neutral in every case reachable today, because reaching that call at all requires `readMeta` to have already awaited the same memoized promise successfully, and a fulfilled promise fulfils for every later await. So it is a no-op that would seal a real failure. The line stays where it is and gained a comment recording the whole derivation. **The queued item's two objections were both false as stated**, which is why this is recorded rather than merely done: the `fs()` failure is not "loud" anywhere today, and the throw/null distinction it wanted to protect is desktop-unreachable by construction. **And its citation of the v0.5.52 decision was a misread.** v0.5.52 declined to route Multimedia's ML load through `loadMLExport` because that helper swallows a bad parse to `null` and has no `detectFileType` gate; what it protects is the setup-required branch and the type gate, and it says nothing about distinguishing a read THROW from a `null`. Build 3 preserves both protected properties in full while collapsing the throw/null distinction that v0.5.52 was cited to defend. **A decision record cited in support of a change is re-read before it is cited**, because a citation that sounds right is how a protected decision gets extended past what it decided.

**THE REAL DEFECT WAS AT THE OTHER END OF THE SAME LINE, and it is the shape this family keeps taking: a failure rendered as a SUCCESS.** `readFile` already resolved `null` for a stored-but-unreadable file, and Multimedia rendered that as a successful load with an empty media list -- every photo, recording and video count missing, nothing on screen saying anything had failed, so a birder who has photographed hundreds of species saw the same screen as one who has photographed none. A parse throw was worse: it reached the outer catch and rendered "Macaulay Library Export Required" over an export that is plainly stored. All four routes (a read that rejects, a read that resolves falsy, a stored file `detectFileType` does not call an ML export, and a `parseMLExport` that throws) now land on the one existing `ML_EXPORT_LOAD_ERROR`; the absent case still shows the guidance panel, because that is the right answer to it. The precedence is the one the code already had -- the ML checks run ahead of the eBird guard -- rather than a new one invented for the fix, and the alternative would split the four routes across that guard so two are pre-empted by a simultaneous eBird failure and two are not. **The fixture repair this exposed is worth its own sentence:** `TabLoadErrorAlert.test.tsx` mocked an ML export with no Common Name column, which `parseMLExport` throws on; it had been invisible because the eBird guard returned first and the parse never ran, and under the new precedence three tests began reporting the ML message instead of the eBird one they are about. No assertion was weakened. A fixture that is wrong in a way the code path never reaches is a defect waiting for a precedence change to reveal it.

**THE SAME FAMILY ON THE WEATHER TAB, AND THE ONE SURFACE THAT NOW ANSWERS A REJECTING STATUS READ HONESTLY.** The checklist backlog built its list from `loadEbirdObservations()` alone and mapped every falsy answer to the setup panel, so a truncated backup, a wrong `.csv` in the eBird slot, or an unreachable backend on web and Pi produced "Load your eBird backup first" over a file Settings was still listing as saved. It was the one `loadEbirdObservations` caller rendering a claim about the backup with no upstream `getFilesStatus()` branch, and the collapse was **not structural**: the status read was simply never asked for. It asks now, and carries `EBIRD_BACKUP_LOAD_ERROR`. **The build went past the letter of its brief in one place, deliberately.** A `getFilesStatus()` that REJECTS says neither "stored" nor "absent", and it is ordinary on web and Pi where that call is a bare fetch at a backend that can be down -- exactly the state a user retrying is in. Falling through to the guidance would re-introduce the family's lie in the corner nobody enumerated, so a rejecting status read reports the load failure. **The consequence is a stated divergence: the app now gives two different answers to a rejecting status read**, because the eight tab loaders still map it to `setup-required` in their outer catch. That is an argument for doing the eight, not for undoing this one, and it is on the ROADMAP as such. The desktop is unaffected, for a reason worth keeping in view: `TauriStorage.readMeta`'s blanket catch swallows its own failures into `{ebird: null, ml: null}` before any loader sees them, and `WebStorage.getFilesStatus` does the same for any non-ok response. **Those three swallowing sites are the actual subject of the remaining work** -- no loader can tell "no file" from "could not ask" until one of them stops discarding the difference, and the eight outer catches are downstream of that. The family is therefore **not closed** by this bundle, and saying so is the point.

**PER-CALL-SITE OVER SHARED PRIMITIVES, BECAUSE THE CODEBASE HAD ALREADY VOTED.** WebKit's default tab mode -- Safari with macOS "Keyboard navigation" off, which is the default and what WKWebView follows -- visits only explicitly-`tabindex`ed elements, native form controls and `<summary>`, so a plain `<button>` and a plain `<a href>` are skipped entirely on the shipped Mac, iPhone and iPad apps. Build 1 marked six map corner controls; build 2 found the gap was app-wide and marked the remaining 45 sites plus the Calendar's `aria-disabled` switch. **The remedy is `tabIndex={0}` at each call site, and the three alternatives were each rejected on a stated ground.** Shared `<Button>` / `<Link>` primitives are the honest answer to "a future component will not inherit," but they are a refactor of every button in the app across 79 component files with bespoke inline styles, and they do not actually solve the discipline problem: a primitive that always sets `0` would break the roving-tabindex groups, so it needs an override prop, and remembering **not** to pass the override is the same class of mistake one level up. A custom lint rule is decisive-against on this repo's own terms, since `.claude/rules/testing.md` states outright that vitest and eslint are not the pre-push gate, so it would be advisory over a gap that had already shipped twice. A runtime sweep stamping the attribute on mount would stamp the roving widgets, is invisible to source review, and would put the published claim beyond the reach of any source guard, which is the one property this run needed most. **The overwhelming majority of the app's buttons already carried the attribute** -- 188 of 218 at the moment the decision was taken, a snapshot rather than a durable figure, since the population moved within this same bundle. The 30 were stragglers, not a design gap, and the guard is what carries the property forward now that the number cannot. Two anchor edits were component-level for free, because `OutboundLink` and `ChecklistLink` already own their `<a>`, and `tabIndex={0}` placed before their `{...rest}` spread stays caller-overridable -- two edits reaching 63 call sites. Primitives are on the ROADMAP as the durable answer, not as this build's answer.

**TWO GUARDS AT DIFFERENT ALTITUDES, AND NEITHER SUBSUMES THE OTHER, SO BOTH FILES SAY WHAT THEY CANNOT SEE.** `lib/tabOrderCoverage.test.ts` parses every shipped `.tsx` under `frontend/src` with the TypeScript AST and asserts that no intrinsic `<button>` or `<a href>` lacks a literal `tabIndex={0}` outside a counted roster. Source coverage is **broad and shallow**: it sees every file including the ones no test has ever mounted, so a brand-new component's unmarked button fails without anyone remembering to write a row, and it cannot see a `tabIndex` a component strips at render time behind its own conditional. `components/mapCornerTabStops.test.tsx` reads the rendered DOM of the map corner controls and is **narrow and deep**: it catches exactly the render-time case, and cannot see a file nobody mounted. Each file's header now states its own blind spot, which is what stops the next reader deleting one as redundant. **An AST walk is also comment-immune by construction**, which is not a hypothetical here: a `<button aria-label="Close popup">` written inside a JSX comment in `BirdingStats.tsx` is what made three scan passes disagree on the population, and a naive regex sweep over-counted unmarked buttons by 19, because the house style writes long explanatory comments inside JSX opening tags containing literal markup. That satisfies `.claude/rules/testing.md`'s comment-stripping requirement structurally rather than by filter. **What NEITHER can prove, stated in the file:** that WebKit's real tab order reaches these controls. jsdom has no tab order at all and the source guard does not even render. The attribute IS the property that makes the engine's order irrelevant, so the attribute is what is asserted, and the engine-level claim is a browser measurement -- which this bundle finally made, in a committed Playwright harness, rather than carrying forward from a prior roadmap note.

**AN EXCLUSION ROSTER MUST BIND EACH ROW TO ITS SITE, and this was proven by mutation rather than argued.** Keying a roster row on file-plus-attribute makes that row a blanket pardon for its whole file: a second unmarked control added beside an already-rostered one is absorbed silently. Each row now declares a per-row `count`, the guard asserts equality rather than existence, and the comparison is a counted multiset rather than a set -- which binds the row to its own site without pinning a line number an unrelated edit above would move, and names the offending row and its reason in the failure message. Mutation-checked in both directions: a sibling added to a rostered file, a sibling added to a *different* rostered file, and a rostered site removed, each going red with the row named. Nine mutations across two rounds, including mutations of the **analyser** itself, because a scanner that silently matches nothing passes everything. **The roster's five rows are not one kind of thing, and calling them one kind was false twice over.** Three are roving groups where the arrow keys move within a single tab stop, each re-derived to its actual arrow-key handler rather than assumed: the tab bar, its collapsed-tab-bar listbox, and the Settings choice rows. The fourth, the species selector's list-toggle chevron, is **not** roving -- the arrow keys drive an `aria-activedescendant` index on the input and never reach the chevron at all; it is excluded because the `role="combobox"` input beside it is a native form control whose `onFocus` opens the same list, which is a different reason needing its own sentence. The fifth is a base-map button paired with native `disabled`, which is not a tab-order decision at all, since the platform removes it regardless.

**PUBLISH THE PROPERTY, NEVER THE COUNT -- three defensible scan methods gave three different denominators.** Passes over the same tree returned 15, 17 and 21 links, and 30 versus 31 buttons, the extra button being one written inside a source comment. A number in published prose depends on a scan method and goes stale the moment a surface is added; a property cannot be wrong. So `ACCESSIBILITY.md` states that no button and no link in the app's own screens lacks an explicit place in the tab order apart from the named exceptions, and a test asserts exactly that on every build. The same discipline was applied inward in the same bundle: a "nine surfaces" count came out of `setupCopy.tsx`, the file that exists to stop this family's copy drifting and that this build made ten; a "Three findings / the same nine surfaces" opener came out of `honestLoadFailures.test.tsx`; and a "nine surfaces" count came out of `.claude/rules/ui.md`. **Where a claim quantifies over N things, check N things:** verifying one map popup and writing a sentence about all five understated a published accessibility gap by two surfaces, and the corrected entry says outright that an earlier revision got it wrong by checking one and assuming the rest.

**A PROSE REPAIR IS SWEPT AT PARAGRAPH SCOPE, NEVER SENTENCE SCOPE, and this bundle paid for that rule seven times and six failed QA rounds.** The pattern is always the same: a correction lands in one sentence and not its sibling. The fix for one over-claim wrote a closed roster into Known Exceptions; the fix for THAT wrote "every tab is a tab stop" into the sentence directly below one whose identical over-claim had just been removed in the same pass. Twice the defect was in text the previous pass had just touched, and once it sat untouched one sentence away, contradicting the new text. **A summary sentence saying "the same X" inherits every property of X, including the ones that do not hold** -- the atlas and county panels were published as giving "the same keyboard path" as the in-view lists, which, once those lists' toggle-to-close was published two sentences earlier, silently promised a dismissal that does not exist. The sweep that follows a correction is mechanical: grep the phrase you just replaced across every file you touched, in both directions, prose against data and data against prose. **And never name a user-facing surface from a component name** -- `TAB_LABELS` is authoritative, `life-list` is **Multimedia**, and "Life List" separately names a concept and a Statistics section, so the wrong name does not read as wrong; it reads as a different screen.

**`ACCESSIBILITY.md` WENT FROM AN APP-WIDE CLAIM, TO A NARROWED CLAIM WITH A KNOWN EXCEPTION, TO A CLOSED EXCEPTION, INSIDE ONE BUNDLE -- and the narrowing was the right move at the time it was made.** Build 1's brief found that fixing the six map controls would not make the published line true: sortable column headers, a category the line named outright, were unmarked in both tables, and every `<a href>` in the app was skipped on both Apple platforms. **The user's stated preference was to fix the code rather than qualify the prose, and that preference rested on the FABs being the only gap.** They were not, so the build did both: it shipped the fix and narrowed the claim to what was measured, recording the residual under Known Exceptions rather than implying it away. Build 2 then closed the exception. **Narrowing a published claim to the truth is not a retreat and is not a lesser outcome than fixing the code** -- an over-claim in a published accessibility statement is a false statement in the direction that flatters the product, and it costs more the longer it stands. The honest partial statement is a deliverable in its own right; what is never acceptable is an app-wide claim written over named residuals. Those residuals are now named plainly: the map library's own zoom buttons, which the `+` and `-` keys cover, and the close button on five popups, **three of which have no keyboard close at all** -- verified per popup, with the correction that the clearing path does exist on each and is simply unreachable, because the only button that fires it is library-drawn. No map popup traps focus, so this is an operability and dismissibility gap in the WCAG 2.1.1 family, not a 2.1.2 keyboard trap; grading it as 2.1.2 would overstate it by a full grade.

**A REPAIRED SYMPTOM OVER AN INTACT CAUSE IS RECORDED LOUDLY, OR IT IS FORGOTTEN.** The Calendar's day-details dialog hand-rolls its own focus trap with its own narrower copy of the focusable selector and **no `focusin` containment arm**, so it contains by comparing `activeElement` against the ends of a `querySelectorAll` list -- the tab-order PREDICTION the v1.0.15 decision forbids outright. On WebKit it previously held not one tab stop inside it, so the first Tab left the dialog entirely and landed on the page behind, which made a published `ACCESSIBILITY.md` claim false on all three Apple platforms. Marking its Close button and its checklist links repaired that: WebKit's real order now matches the predicted list. **That is containment by agreement, not by construction**, and the cause -- four copies of `FOCUSABLE_SELECTOR` outside `lib/useFocusTrap.ts`, each hand-rolling a prediction-based trap, against `.claude/rules/ui.md`'s "lives there and nowhere else" -- is untouched. It is on the ROADMAP in those words, because a fix that makes a defect look closed is exactly the one that gets forgotten. `ModalDialog` is the same shape and holds today only because its six call sites all happen to contain nothing but already-marked buttons: **when a decision record forbids a mechanism, sweep for the other live instances of that mechanism in the same pass.**

**IN AN ASYNC FUNCTION, `try { return v } catch` CLOSES NEITHER DOOR A THENABLE CAN LEAVE BY, and a totality claim is bounded at its definition site or it is false.** `weatherBacklogLoad.ts` declares that its promise never rejects, and its call site depends on that: `App.tsx` calls it with a `.then` and no `.catch`, and there is no global handler anywhere in `src`. An async function's `return v` performs promise resolution of `v` **after** the try block has exited, so a `buildRows` handing back a thenable escapes through two separate doors on one enclosed-looking statement: the `then` call, and the `Get(v, "then")` getter lookup performed during resolution. Both were measured rejecting on the shipped shape and fulfilling under `return await`, at a cost of one microtask tick on the synchronous array path. **A synchronous-today return type is not protection**, because the change that breaks the claim is a refactor nobody will connect to it, and this repo has form for exactly that move: the observations parse went off-thread for the same class of reason. On that day the `try` silently stops catching, the missing `.catch` becomes a live defect, the section parks on its spinner for the rest of the session, and **nothing goes red**. **`return await` closes rejection; it does NOT close non-settlement** -- a thenable that never settles leaves the promise pending forever, measured still pending at the probe ceiling, and no `try` closes that anywhere. The docstring names that boundary explicitly rather than saying "no stated exceptions", which it did, and which was wrong by two shapes. A totality claim with an unnamed exception is worse than an honest partial one.

**A TOTALITY CLAIM NEEDS A TOTAL TEST: derive the subjects from the INTERFACE, sweep the nth call, and assert the sweep's own bounds.** The guard's first spelling named three of the four dependencies in a literal array, which is a roster wearing a totality claim's clothes, and the one it did not name was the member with the unguarded call site. A designed mutation -- a fifth dependency called outside every `try` -- produced **zero** failures across all 51 rows. Rewritten to iterate `Object.keys(deps())`, the same mutation goes red and names the offender by member and call number. Three parts ride with it. **Derive, do not enumerate:** TypeScript already forces the fixture to carry every member for the call under test to compile, so a dependency added later brings its own rows whether or not anyone remembers the file. **Sweep the nth call, not the first:** a member called more than once has more than one site and they are not equally guarded, and the second `isCurrent()` call sat outside every `try` precisely because the first one sat inside the status read's -- invisible on inspection. A throw on a call that never happens leaves the healthy path intact, so each row asserts FULFILMENT and a legal value rather than always the failure one. **Assert non-vacuity and the ceiling:** a counter compared against `keys x N` so a partial key list fails loudly, and, still open, an assertion that the healthy path's real maximum call count stays below `NTH_MAX` -- today two against a ceiling of three, one call of headroom that nothing polices. **A throw-sweep is structurally blind to a dependency that RESOLVES BADLY**, which is why the thenable shapes above needed their own probes; a totality guard should carry at least one row returning a hostile value beside the rows that throw.

**`npm audit --offline` PRINTS "found 0 vulnerabilities" WITHOUT CONSULTING THE REGISTRY, and it was published as a Pass before a networked run contradicted it.** It does not fail; it manufactures a false clean, which is the worst shape a gate can take, because a green line then means *not run* and reads as *verified*. The networked run reported one High: `browserslist@4.28.2`, transitive through `eslint-plugin-react-hooks`, lint-only, absent from the bundle, introduced by nothing in this bundle. **Run `npm audit` with network, and run it twice** -- `--omit=dev` for the shipped scope, the full tree for the toolchain -- because the two answer different questions and reporting only one is how a dev-only High either blocks a release it should not or hides entirely. **An advisory's severity and a finding's severity are different numbers and a review owes both**: npm rates it High at CVSS 7.5, the reachable risk here is Low, and collapsing the two either smuggles a downgrade past the reader or blocks a ship on a package that cannot run. The remediation is deliberately not a ride-along inside a Fix run's diff: a separate dev-only commit, verified lockfile-only, with no version bump, no changelog entry, no tag and no release, per CLAUDE.md's dev-only rule. **The same shape is why a harness that skips when its dependency is missing is worse than no harness at all**, and it is the reason the two committed Playwright harnesses stay hand-run rather than being wired in with a silent skip. **DISCHARGED by the playwright-gate build (2026-09-05):** three harnesses now run from `npm run verify --prefix website/tools`, wired into `pipeline.yml` after `npm run build`, and the runner **exits 1 when `CI` is set** and Playwright cannot be resolved while skipping loudly when it is not -- so the skip that would have rotted is exactly the one the gate refuses to take. The blocking repair was upstream of the skip question and is the part worth carrying: `website/tools/package.json` and `package-lock.json` were themselves gitignored, so Playwright was declared **nowhere in this repository** and every harness -- and the screenshot pipeline beside them -- failed at `createRequire` on any machine but the one they were written on. A dependency that exists only in one developer's `node_modules` is the same false clean in a different costume: the harnesses looked committed and were unrunnable.

**THE ENGINE PREMISE WAS FINALLY MEASURED RATHER THAN CARRIED.** Both tab-order builds rest on a claim about WebKit that this repo had been carrying from a prior roadmap note, and whose only in-suite evidence is a jsdom test that structurally cannot see a tab order. A committed harness now launches real WebKit and Chromium, presses Tab across a fixture of plain and marked buttons, links, a `<summary>` and inputs, and reports what it observes -- **and it reports a null result as a null result**, not as a pass, which is the property that makes it evidence. A second harness reads the Weather backlog's live region out of a real accessibility tree in both engines and **discriminates**: four of its checks fail against the pre-fix build, which is what turns "the region is present" from an assertion into a measurement. Neither is wired into CI. Promoting them is on the ROADMAP with the dependency decision spelled out, because Playwright lives in `website/tools` rather than `frontend`, and because the promotion must single-source the deliberate-exceptions allowlist rather than write a third hand-maintained copy of it -- which would be this bundle's recurring defect in a new costume. **DISCHARGED and DEFERRED, playwright-gate (2026-09-05).** Both are wired in now, from `website/tools/verify/`, and the tab-premise harness's null-result property survived the promotion by design: its exit code separates *the apparatus broke* (1) from *the premise measured null* (0), so CI can never go red on a WebKit vendor default. Its apparatus check is Chromium as the known-clean control -- full keyboard access is on there by default, so it must reach all seven fixture controls or no reading from either engine is evidence. Measured at promotion: Chromium 7/7, WebKit skipping the plain button and link and reaching the `tabindex="0"` ones, so the premise is CONFIRMED on this machine rather than carried. The backlog harness's discrimination was re-measured the same day against a real pre-fix build (a worktree at the commit before the fix): **exactly 4 checks fail**, the figure this entry claims, and the pre-fix build shows the setup-shaped title the fix removed. **The allowlist half was deliberately NOT done**, and the reason is that it turned out not to be a gate change at all: none of the three promoted harnesses reads the exclusions roster, so single-sourcing it is a prose repair to `ACCESSIBILITY.md` and `lib/useFocusTrap.ts` against the roster `tabOrderCoverage.test.ts` already owns -- and it collides with the queued `FOCUSABLE_SELECTOR` build that owns `useFocusTrap.ts`. Deferring it is a decision with a reason rather than a silence; it stays on the ROADMAP, to be done in that build or its own pass. The third hand-maintained copy this entry warned about was never written, which is what the warning was for.

---

## A focus trap must contain on `focusin` rather than predict the engine's tab order, a live region must exist before its message (the same trap, in eight places including both of its own precedents), and a quieting change owes a roster of what each caller RENDERS -- 2026-09-03 (v1.0.15)

**Decision: shipped** as a four-build Spool bundle on one version -- `947285a` website-acknowledgements-tone (Improve), `2550936` map-fullscreen-toggle (New Feature), `cd8a3a9` cache-read-throw-containment (Fix), `519ad69` tab-error-panel-alerts (Improve); bundle merge `2f79c71` (tag `v1.0.15`), iOS stamp `e40c748`. **There was no separate release-prep commit:** build 2 carried the version bump and the website pill in its own commit, and builds 3 and 4 appended to the existing `## [1.0.15]` changelog section rather than opening one. Three legs shipped the same day: macOS universal and Windows signed installer plus `latest.json`, the website, and iOS 1.0.15 build 1 on TestFlight (VALID). **The App Store leg is deliberately DEFERRED, and that is the standing rule operating rather than a failure.** 1.0.14's submission was still `IN_REVIEW`, and replacing a build in review surrenders its queue position; the user chose to keep it and submit 1.0.15 once 1.0.14 resolves. This is the same posture as the v1.0.4 replacement decision read from the other side -- there the in-review build was replaced because the newer one carried the fix that mattered, here the in-review build is protected because it does. The rule is that the App Store leg never blocks the other platforms, not that it always ships on the same day. **One build jumped the queue on purpose:** the map fullscreen toggle was a saved idea taken ahead of the Up Next list as a Spool choice, not a change of strategy.

**A FOCUS TRAP MUST CONTAIN ON `focusin`, NEVER BY PREDICTING THE ENGINE'S TAB ORDER.** A keydown-only trap answers "is focus at the boundary?" by comparing `document.activeElement` against the first or last entry of a list built from a `querySelectorAll` selector -- which is to say, by predicting the engine's tab order. **That prediction is wrong in WebKit's default tab mode**, which is Safari with macOS Keyboard navigation off (the default) and is what WKWebView follows, so it is what both shipped Apple platforms get. WebKit there visits a smaller and DIFFERENT set: elements with an explicit `tabindex`, native form controls, and `<summary>`. Plain `<button>` and `<a href>` are skipped entirely. Measured inside the expanded Species Detail overlay: the trap's list held 22 entries ending at the fullscreen toggle, while WebKit's real forward order ran canvas, the attribution `<summary>`, the three base-map buttons (which carry an explicit `tabIndex={0}`, which is the only reason they are visited at all), the Trails checkbox, and then straight OUT of the overlay -- leaving five elements before the trap's `last`, so `activeEl === last` never became true and the end-wrap never fired. Focus escaped after 5 Tab presses on three mounts and after 1 on the fourth, and it was proven by typing `xyz` into a covered input behind the panel and reading the value back, not inferred from `activeElement`. The containment arm did fire, but on the NEXT Tab: one hop too late, with focus already resting on a control the opaque panel was covering. **`focusin` needs no prediction at all** -- it fires after focus has moved and before the user can type, so containment reacts to where focus ACTUALLY went instead of betting on where it was about to go, and it cannot recurse because the corrective `.focus()` fires a `focusin` whose target is inside the root. **The keydown arm stays, for the two things `focusin` cannot do:** wrap at the ends with no visible out-and-back, and act when focus is lost to `<body>`, for which engines do not reliably fire `focusin`. Verified at 640 keypresses (40 forward and 40 backward on each of four mounts in each of two engines), zero escapes, with every input outside the map snapshotted for changed values. **jsdom has no tab order, so a test that reproduced WebKit's would only re-assert the broken assumption**; `useMapFullscreen.test.tsx` instead pins the property that makes the engine's order irrelevant -- focus lands outside and is pulled back with no keydown at all. `lib/useFocusTrap.ts`'s header carries the whole derivation, because any fix that keeps guessing the tab order keeps the defect open.

**A LIVE REGION MUST EXIST BEFORE ITS MESSAGE -- and this repo had already written that post-mortem, then left two live instances of the trap for the next author to copy.** `role="alert"` announces a MUTATION observed on a node already in the accessibility tree; a region created carrying its text is a single insertion with nothing registered to observe it, so the sentence is simply never spoken. v0.5.83 recorded this after a `display: none` rule caused it by the other route (an element hidden from the tree is inserted along with its first message). It recurred in eight tab load-failure panels, six of which had no live region at all -- and **both of the two that did were wrong in exactly the documented way**, because each rendered `role="alert"` from an early `return` in a phase switch, which creates the node at the instant its text exists. Measured at HEAD in both engines: 0 alert regions while loading, 1 after the message. **Correcting the precedents was the point, not a bonus.** Shipping six more copies of a pattern known to be broken while leaving two behind is worse than either, because the two survivors are what the next author reads as the reference. Four rules ride out of the repair, which is structural rather than an attribute: (1) **the region and every element between it and the tab must be mounted before the message** -- `TabLoadErrorAlert` owns frame, optional inner column and region together, mounts at fragment index 0 of every branch that can reach the error phase so React reconciles it to the SAME DOM node, and `lib/tabLoadAlertCss.test.ts` guards all three class selectors, compared exactly rather than by `includes` (`.sr-tab-load-alert` is a strict prefix of the other two), at all depths, with a positive `display: flex` assertion so the scan cannot pass vacuously. (2) **A tab whose load effect does not reset to a loading phase can go `ready -> error` in one commit and needs the region in its READY render too** -- Statistics and the Map Explorer are the two, and the test states the property ("the region existed, empty, in the commit before the message"), never the mechanism. (3) **The region holds the sentence and nothing else**: both precedents put `role="alert"` on the outer panel, so "Go to Settings" was read as part of the failure; the button is now a sibling. (4) **When a region becomes always-mounted, every `no alert exists` proxy assertion in the repo becomes `no alert carries text`** -- `MapExplorerSearchThisArea.test.tsx` asserted `querySelector('[role="alert"]')` was null as a stand-in for "no error", which after this change would have been asserting the defect.

**Two of this build's own stated reasons were false, were measured, and were corrected before ship; a third was disproved and shipped anyway, and it is the one that will bite.** The change brief called four panels' missing `aria-hidden` on the error icon a defect on two grounds, and neither holds: an `<svg>` of paths contributes nothing to `textContent` (measured empty on all eight tabs in both engines), and **lucide-react 1.14 adds `aria-hidden="true"` itself to a childless icon with no a11y prop**, so all four were already hidden at runtime. The explicit prop shipped anyway, correctly, but as a guarantee this component owns rather than a bug fix -- and the test says out loud that it cannot discriminate the explicit prop. The third is worse: the shipped comment justifies keying the message node by the MESSAGE rather than a counter on the premise that "a counter would also have to be state", and the Auditor disproved it -- the existing `setPhase({ tag: 'error', message })` calls already run inside the load effect's async function after awaits and pass lint today, so a counter in that payload is available in principle. The design is still right, on three reasons that do hold: the two non-resetting tabs each construct exactly one error message, so `error -> error` is the same sentence by construction and a genuinely different one would change the key; on the realistic retry path the panel is `display: none` and Blink reports the subtree `ignored`, so a counter's remount would announce nothing either; and where the repeat does land on a visible tab, nothing observable changed, so re-reading it would tell a screen-reader user something no sighted user is being told. **A justification is audited alongside the outcome, and a better reason replaces the written one in the same change that ratifies the decision** -- because a recorded reason with a hole in it is a revert waiting to happen.

**A GUARD TEST THAT MOCKS A MODULE WHOLESALE STRUCTURALLY CANNOT VERIFY THAT MODULE.** `honestLoadFailures.test.tsx` mocks `lib/observationsCache` and `lib/mlExportCache` wholesale to prove the eight tab loaders branch correctly, and it was green before, during and after a bug living inside those very modules -- correctly, because it can only ever see the callers. A fix to a module some guard test mocks must ask which of the two shapes the new guard is, and the answer here was both: `lib/cacheReadThrowContainment.test.ts` covers the seam at module level with both caches on one roster (a third cache of this shape then reads as a missing row), and `components/cacheReadThrowTabs.test.tsx` is the **real-module twin** -- the cache is real, only `storage` is mocked, so the rejection starts exactly where it starts in production, inside `WebStorage.readFile`'s bare `fetch`, and travels the real seam into the tab. Mutation-checked: with the read moved back above the try, 10 of the 18 new rows go red and `honestLoadFailures.test.tsx` passes unchanged, which is the demonstration itself. The twin also states that its two tabs are a **sample, not a roster**, and names the file that owns the full eight.

**A QUIETING CHANGE (throw becomes `null`) OWES A CALLER ROSTER RECORDING WHAT EACH CALLER RENDERS, not merely whether its behavior changed -- and the callers with no upstream guard get reviewed FIRST.** The bug was two lines: `await storage.readFile(...)` sat above `loadFresh`'s own `try` in both parse caches while both docstrings promised `null` on failure, so a read rejection escaped, bypassed each tab's honest `!ebird` branch, landed in its outer catch and rendered the setup panel -- "eBird Backup Required" with the Download My Data steps, over a file `getFilesStatus` had reported as stored on the same load. Reachable on web and Pi only, and ordinary there rather than exotic: `WebStorage.readFile` is a bare `fetch` plus `res.text()`, so an unreachable backend rejects the fetch and a body truncated mid-download rejects the text, over a ~6 MB CSV served off a Pi. The fix moves the read inside the existing try and nothing else; the catch is widened by exactly one await, and the cache write stays outside it so a different class of failure cannot be swallowed. **The roster is 16 call sites, and the delta at fifteen of them was zero or an improvement -- which is precisely why a delta-only roster was not enough.** Every tab calls `storage.getFilesStatus()` first and branches on it, so "no file at all" is already decided upstream and a falsy cache result already means "stored but unloadable". **A caller WITHOUT that upstream branch genuinely collapses the two**, and whatever copy it defaults to is wrong for one of them. There is exactly one: `App.tsx`'s Weather Backlog effect, whose `null` renders "Load your eBird backup first" with a Go to Import button -- the same setup-shaped lie, at zero behavioral delta, invisible to a roster that records only whether behavior changed. Enumerate the unguarded callers first, not last. **Two riders.** The five caller-side `.catch(() => null)` guards stay, because the cost of being wrong is asymmetric, but their comments each said the read "sits outside its try" and were retargeted rather than left -- a comment that misdescribes its own guard is worse than no comment. And the v1.0.14 memoized-promise hazard was checked rather than assumed away: `.finally` runs on rejection too, so `inflight` always cleared and this poisoned the CALLER, never the session -- the distinction is between a promise that never settles and one that settles badly, and both properties are now asserted rather than argued.

**"Capture rather than assume" is only half a decision -- the captured value must be MEASURED once in a real browser, or the record around it goes wrong while the code stays right.** The fullscreen toggle releases each map's page-embedded gesture posture and restores it on collapse, capturing `scrollZoom.isEnabled()` and `cooperativeGestures.isEnabled()` per mount rather than hardcoding a pair. Three of the four mounts pass `scrollZoom={false} cooperativeGestures`; the Statistics map passes neither, and the PR, a test row's title and the source comment all read "passes neither" as "maplibre's defaults" and claimed that map ships with scroll zoom ON. It does not. `SnowMap` forwards both props unconditionally, so the map is constructed with `scrollZoom: undefined` -- an own key that shadows maplibre's `true` default through its own `Object.assign` -- and QA measured the live posture in both engines as scroll zoom OFF and cooperative gestures OFF. **The capture is what made the wrong premise cost nothing**, and a hardcoded restore would have silently changed whichever mount the hardcoded pair was wrong about, with the other test row still green. That is the case for capturing; it is not a case for leaving the value unmeasured, because three artifacts carried a false claim that would have outlived the build. The corrected test row now records what it is NOT, by name.

**An option-gated shared extraction owes a two-direction mutation check, and the reviewer runs it rather than citing it.** The Tab trap was lifted out of `ModalDialog.tsx` into `lib/useFocusTrap.ts` behind `containOutsideFocus`, defaulting to `false` -- the dialog's shipped behaviour byte for byte -- with the map overlay opting in, because the surface behind it is a live page in the same panel rather than a `display: none` sibling. **Gate ON removed** (drop the `focusin` listener) turns exactly 3 rows red; **gate OFF removed** (arm it for every consumer) turns exactly 1 row red and leaves 27 green. Both directions matter here for a reason worth generalizing: **`ModalDialog` has no observable surface to regress into.** All six of its call sites sit behind a Mac/iOS platform gate, so no dialog renders in a browser at all, and no test in the repo fires a Tab key at one. The "byte for byte unchanged" claim had nothing else holding it up, so the gate-off mutation is the only thing that makes it checkable rather than rhetorical. Escape deliberately did not move: the two consumers need different phases and different side effects, and Tab and Escape are disjoint by key, so splitting one listener into two changes neither.

**Two `react-map-gl` and React-compiler facts, both silent when they bite.** `MapRef` **lies about handler properties**: `createRef` copies only the map's FUNCTIONS onto the ref object, so `mapRef.scrollZoom` is `undefined` at runtime while the type says otherwise -- it typechecks and fails. Methods like `resize()` are proxied and fine; handler objects (`scrollZoom`, `cooperativeGestures`, `dragPan`) are reachable ONLY through `getMap()`. And **a hook's return value used in a `ref=` prop taints the whole object for `react-hooks/refs`**: passing `fs.registerToggle` to `ref=` makes the compiler treat `fs` as a ref value, so every other read of it during render trips the rule. Destructure the members up front and pass the local. Two lint constraints shaped the hook itself for the same family of reason: it holds one state object and DERIVES `expanded` from it, so a collapse happens in the same render rather than a cascading one, and it stores the toggle element as state rather than a ref because a ref read through a hook's return value is a ref read during the consumer's render.

**A feature can find a shipped defect by asking what its own state change would do to code it does not touch.** `SightingsMap` built `MapBoundsFitter`'s `coords` inline, a fresh array identity on every render, against an effect whose deps are `[map, coordinates]` and whose body calls `fitBounds(..., { duration: 0 })`. **That was already live in the shipped app**: clicking a pin to read its dates sets `selectedCoord` in that same component, re-renders it, and snapped the map back to its starting view, discarding whatever the user had panned to. The fullscreen work would have made it worse in a way that looked like the feature's fault, since the toggle's state lives in the host and re-renders the map on every expand and collapse. One `useMemo` (both call sites already memoize `markers`), and the guard is driven through the real `SightingsMap` and the real fitter, with a guard-the-guard asserting it STILL re-frames when the marker set genuinely changes.

**A path-gated conventions file cannot enforce a rule at the one moment it matters, and that is how a release-parity guard sat red on `main` for five commits.** The app version lives in FOUR files, not the two CLAUDE.md named: `frontend/package.json`, `src-tauri/tauri.conf.json`, `CHANGELOG.md`, and `website/index.html`'s version pill (visible text and `aria-label`) plus its footer line. One guard asserts all four -- and it lives in `icloudKeysPublishedClaims.test.ts`, a feature-named suite with two repo-wide checks inside it. The website leg was documented only in `.claude/rules/docs-and-website.md`, which is path-gated on `website/**` and `README.md`, so a release-prep session touching the two manifests and the changelog never loads it. 1.0.14's prep bumped three of four, and the suite stayed red through the iOS stamp, a docs commit, the context update and a commit that **edited `website/index.html` itself and still left the stale version** -- touching the file is not what repairs it; only the version-stamp step is. The rule now lives in CLAUDE.md's always-resident Versioning section. **Stated weakness carried forward:** the guard is a `toContain` over the whole file, so one correct string anywhere turns it green and a corrected pill with a stale footer passes.

**Published prose describes the app's BEHAVIOR, not the app's own on-screen COPY.** The website and README each narrated what the Settings Acknowledgments panel says, naming its two credits. That costs three ways: it tells a reader what a different screen already tells them, it makes a future third acknowledgee a three-file edit, and it dilutes the authority of the screen the v1.0.10 decision put the content on. The trim leaves the app panel as the only place the names live, `docs/HELP.md` as the documentation of record, the README naming the section without its credits, and the website not mentioning it at all -- nothing un-credited, only un-repeated. This is the v0.5.75 propagation rule from the other side: that one says write the formulation once and propagate it to every hand-maintained copy, this one says REDUCE the number of copies where the content is app-internal, because fewer restatements is fewer things to keep true. **The boundary is courtesy content versus provider attribution: you may un-repeat a thank-you, never an attribution.** The `## Attribution` section, the website footer fine print and `website/privacy.html`'s Cornell and eBird notices were untouched and stay discharged in text.

---

## A promise with no reject path hangs the session, symmetry in the code is not symmetry in the evidence, and a failure signal must match the branch its consumers already take -- 2026-09-02 (v1.0.14)

**Decision: shipped** as a three-build Spool bundle on one version -- `fcde120` observations-worker-hang, `c6d3d16` clear-means-clear, `a51e588` honest-load-failures; bundle merge `73e8f38`, release prep `e249051` (tag `v1.0.14`), iOS stamp `40196d0`. All three are Fix-lane. Every leg shipped the same day: macOS and Windows desktop plus `latest.json`, iOS 1.0.14 build 1 on TestFlight (VALID), and App Store review submission `6a827d19-8cfd-44ee-a11f-3369dae0910a` at `WAITING_FOR_REVIEW`. 1.0.13 was already `READY_FOR_SALE`, so a new version record was created rather than an editable one reused; the standing hold recorded through 1.0.12 stays gone.

**A worker-backed promise owes a SETTLE CONTRACT, not two handlers.** `parseOffThread` built its promise with a resolve-only executor: no `reject`, no timeout, no `onmessageerror`, no termination detection. Any worker death that does not dispatch `error` left it pending forever and left the worker un-terminated. It now has one idempotent `settle` covering five paths -- a reply, `onerror`, `onmessageerror` for a reply that cannot be cloned back, a `postMessage` that throws on the way out, and silence past a budget -- which clears the watchdog, detaches the handlers, terminates the worker and settles exactly once, so a late event after the watchdog can neither re-settle nor double-terminate. **The budget is size-scaled and comes from a recorded measurement, not a guess:** `parseEbirdObservations` runs at 57.8, 71.6 and 75.0 Mchar/s across a 20x size range (flat, so linear in the input), and `30 s + 4 s/MB` is roughly 250x slower than measured -- wide enough for a Pi browser or an older WKWebView under GC pressure, with the 30 s floor covering worker spawn, module evaluation and the request's structured clone, none of which scale with the file. **And a memoized promise inherits its producer's worst settle path:** the pending promise sat in `inflight`, whose `.finally(() => { inflight = null })` therefore never ran, so every later caller for the rest of the session joined the dead promise and all eight observations tabs spun until a Settings save or clear. Memoize a promise and you have made its non-settlement session-scoped.

**A failure signal must match the branch its consumers ALREADY take -- rejecting is not automatically the honest choice.** A bare rejection was the smaller diff and the wrong answer: all eight tab loaders map a *thrown* load to `setup-required`, which tells the user to upload a backup that is plainly loaded. Each already had an honest branch on a falsy result, so resolving `null` reached a terminal state naming the real problem in six of the eight with no edit at all, and the two that did not (Map Explorer's `MapPhase` gained an `error` variant; Life List stopped reading a falsy backup as "no backup" and silently dropping every species seen but not photographed) were corrected rather than the signal being bent to fit them. Read the consumers before choosing the signal. The third build generalizes the same claim to nine surfaces: what a tab says about a stored file must match what is true of it, so a failed Macaulay Library read stopped rendering "eBird Backup Required" over a backup that had loaded perfectly, and the terse error panel now names `MyEBirdData.csv` and the Settings path while staying deliberately distinct from the `setup-required` guidance panel (the 2026-05-22 split), with every copy row asserting the step list is ABSENT and every row having an absent case.

**Symmetry in the code is not symmetry in the evidence, and this bundle hit it four times in three builds.** Three of four durable stores had an in-flight race test and the fourth had none; two half-tests each mocked the other side with nothing on the composed seam; two clear paths were symmetric in code and the failure surfacing was guarded on only one; four loaders shared a cancel-guard shape. Each untested half looked safe precisely because its sibling was tested and the code beside it read the same. Three mechanisms, all of which earned their place here. **One roster from one template**, so the store list is itself the artifact and a fifth store with no row reads as a gap rather than as nothing at all -- reach for it at the FIRST instance, not the third. **Every row mutation-checked against its own guard**, one row each, no cross-talk. **And every row asserts the write path never RAN, not only that the disk stayed clean:** the county row was green with its own guard deleted, because a downstream guard kept the disk clean while the deleted one was the only thing protecting a module-level byte total from being permanently over-counted. Four green rows are not four working guards; a roster green for the wrong reason is the same failure as a roster with a store missing. Two riders: a test that forces an interleaving must prove the interleaving it claims (the first ordering test released the parked write too early, so an unordered store passed on timing alone), and a roster carries each row's own re-trigger rather than one assumed to be shared -- Statistics re-runs on the files epoch and has no `filesVersion` prop, so a prop-driven roster would have left its effect never re-running and its row passing by never reaching the code it exists to test.

**The capture that closes an in-flight race belongs at the FETCH CHOKEPOINT, before the request -- not at the write.** A purge that empties mirrors and deletes documents is not enough: it must supersede the store's own in-flight work, which is three separate mechanisms (cancel the debounced timer, stop the parked load from adopting the document it just read, refuse a write from work that began before the purge), and a purge missing any one silently un-does itself a few hundred milliseconds later. The sharp part is the third. Identity (`store !== _store`) is sufficient only where the writer captured its store before the await; a **public** entry point that calls `ensureLoaded()` for itself finds the fresh post-purge mirror perfectly valid and merges the deleted export's data into it, which identity structurally cannot see. That case needs a monotone purge generation, because the question is "did a Clear happen since this work began?" rather than "is this the current object?". And the generation must be captured where the REQUEST begins: a write path is entered only after the answer lands, by which time the counter has already moved, so a generation read inside the write reads the post-purge value and the guard is inert. Where the fetch chokepoint lives inside the store the capture is internal; where it lives outside it -- `transport.getReplayable` for the replay store -- **the store exports the capture and the chokepoint hands it back to the write.**

**Reach for the type system first; keep a source census only for what types cannot express.** `replayStore.put`'s pre-request generation shipped optional, defended by a source-level census of its call sites, and the security review overruled that correctly. A required parameter fails at `npm run build`, which this repo declares as its pre-push gate; a vitest census fails one gate later. And a guard asserting an exact allowlist of callers fails TOWARD WIDENING, because the obvious response to it going red is to add the new module to the list, which is the opposite of what the guard exists for. It cost nothing -- the one real call site already complied. The census survives only for the part a type cannot state: that the capture is read ABOVE the request rather than below it, and that the call-site count is still one. The same gate-ordering point landed twice more in one build: a `vi.mocked` typing error and a dropped third argument were both caught by `npm run build` and by neither `vitest` nor `eslint`.

**A durable store keyed on user-file content registers with the same teardown that deletes the file.** Four stores each missed this, and the reason is structural rather than careless: there were three clear paths, so wiring a new store meant remembering all three. **The rule that travels is the registry, not the discipline** -- one table, one entry point (`lib/clearDerived.ts` / `purgeDerivedOnClear`), and an inventory guard pairing each store's exported purge with a row that calls it. Two riders learned building it. The purge must be a **production export, never the `_reset*ForTests` seam**: those detach the in-memory mirror and leave the document on disk, so shipping one as the clear path looks correct in a test and clears nothing. And **a store is registered on where its KEYS come from, not where its VALUES come from**: `county-completeness-v1` holds only eBird's public payloads and still belongs, because a key exists there only for a county the user has birded, which makes the key set a list of where they have birded. Accepted, stated cost: after a clear and a later re-upload those county lookups re-fetch once each. The other half is that the teardown fires on CLEAR only and never on REPLACE -- it is a separate dependency from the controller's existing `invalidate`, which the synced arrival also uses, so the pull path physically cannot reach it, and `PRIVACY_POLICY.md`'s published "asks only about checklists that have not been answered yet" stays true.

**`data/replay.json` now has a write chain equivalent to `docChains` and still does not join it, and the structural reason lives at the chain's definition site because a future change would silently break it.** It is its own file with a single writing module, so it has no read-modify-write to clobber and joins no seam chain -- but one module is not one write at a time: two concurrent `setReplayStore` calls completed in whatever order the filesystem returned, and the purge's document is the smaller one, so it was the likelier to land first and be overwritten by a pre-purge flush already inside the seam, which neither the cancelled timer nor the identity check reaches. `replayStore` therefore has its own ordered writer of the same shape and the same two rules as `TauriStorage.chain`: the next write is not CALLED until the previous one settles, which orders the writes rather than narrowing the window. What it does NOT need is `docChains`' read-modify-write wrapping, and only because a write is reachable solely from a flush that requires a completed `ensureLoaded`, or from the purge, which awaits it. **A future re-load-after-purge would break that quietly**, so the reason is recorded at the chain rather than left in a commit message.

**A best-effort teardown must report what it could not do.** `Promise.allSettled` with the results discarded is how "clear everything" becomes "clear whatever happened to work", indistinguishable from success at the call site -- over a `WebStorage.deleteSetting` that never checked `res.ok`, which is now fixed for the same reason already written one method above it. Keep the best-effort loop, because one store's failure must not abandon the other three, and **return the failures** so the caller can decide. The message then names what actually happened: not "delete failed" when the file IS gone, and not "try again" when the row the user would press is now empty. A background path with no user waiting may decline to surface it -- the synced `delete-local` path does, since its only surface reads "Could not sync", which would be false -- but that is a stated decision at the call site, not a discarded value, and the comment states the narrower recovery honestly (a repeat synced clear short-circuits before the purge, so it takes an upload and then a clear).

**A hand-built static reproduction is evidence about ELEMENTS, never about the PAGE -- and the FRAMING decides whether the record outlives the build as a truth or a falsehood.** The first draft of this build's PR reported "a 47px page-scroll leak at 320px". **There is no such leak.** Page `scrollWidth` measures 320 in every configuration, patched and unpatched, with no clipping ancestor: the shell's own padding absorbs the overhang. The repro copied the DOM below the component faithfully and invented the DOM above it, which is exactly why every box width reproduced to the hundredth while the page number was an artifact of the author's own ancestor chain. The real defect is the shared error panel overhanging its content column by **23.28px per side at 200% text scale**, identical in Chromium and WebKit, zeroed by the fix: **layout correctness, not an accessibility repair, and the pre-fix state was NOT a WCAG 1.4.10 (Reflow) failure.** Both sentences describe the same diff and only one is true, and "47px of page scroll at 320px" would have entered these records as a Reflow failure that never happened -- so establish whether a geometric defect is visible to the USER before writing the sentence that will outlive the build. Any claim containing `scrollWidth`, "page" or "viewport" comes from the real built app; a repro that produces one has produced a question, not a finding. **The v0.5.85 note has its converse here, and it decides the fix rather than forbidding it:** `overflow-wrap: break-word` renders identically and does not change the box width at all, because it does not affect intrinsic sizing and so cannot lower a flex item's automatic minimum -- only `anywhere` can. Where the container is externally bounded, so `anywhere` cannot collapse it below its own content minimum, the house `.sr-wrap-anywhere` helper is the right and only answer.

**Fix a hazard where it is CREATED; when you cannot, say what you are trading and name the WHOLE hazard.** `loadMLExport` documents that it returns `null` on failure and does not deliver it: the `await storage.readFile('ml')` sits outside `loadFresh`'s own `try`, so a read throw rejects the caller's `Promise.all` and renders a setup panel over a stored file. It was fixed at four call sites, which is four chances for a fifth caller to forget. **The bug brief explicitly permitted the module as a target -- the deferral is TIMING, not permission or scope.** Moving that read inside the existing `try` changes a shared cache seam for every present and future caller, and this was the last build of a three-build bundle about to go into release prep, which is the wrong moment to alter a seam four tabs read through. `observationsCache.ts:193` carries the identical defect at `mlExportCache.ts:23`'s exact shape, measured rather than reasoned, and the eBird CSV is the larger file, so a mid-body read failure is likelier on that side than on the one that was fixed. The queued follow-up therefore **names both modules**: a note naming only the ML half would send the next run after half the hazard.
...
> **PARTLY REVERSED by v1.0.17 (top of this file); one half of it was VINDICATED.** REVERSED: the dropdown, and the bottom-bar rejection below, which rested on a bottom bar topping out at ~5 destinations — four favourites plus a More sheet does not, and the count has since grown from 8 to 11. Also superseded: the Implications' expectation that a future mobile app inherits this dropdown. **KEPT and extended to three density tiers: collapse by measured width rather than a fixed breakpoint.**
