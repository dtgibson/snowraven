...
**Other Statistics** — Nemesis Birds only. Powered by `GET /stats/nemesis`; calls eBird `data/obs/geo/recent` with `back: 30` using the lat/lng/dist from map defaults; groups by species, keeps most-recent observation date per species, and returns sorted by most recent date descending (newest first — NOT by frequency). Frontend filters against life list. Each name links to `ebird.org/species/{taxonCode}` — resolved from `mlTaxonMap` or a fire-and-forget `/taxonomy/codes` fetch; falls back to plain text when unresolvable. Dot colors: red = seen in past 7 days, amber = 8–14 days, grey = 15–30 days. Most Photographed / Most Recorded / Most Filmed have moved to the Media card.
