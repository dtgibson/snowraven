...
- **The check never outpaces the app's eBird pacing contract.** It runs through the same key-global gate as every other eBird lookup, at one enforcement point, so a rate limit anywhere still slows everything on the key; a 429 is surfaced as a 429 on the checklist path for the first time. Offline, missing-key, and provider-failure all degrade through the app's three honest states. The privacy policy's eBird bullet names the per-checklist project lookup.
