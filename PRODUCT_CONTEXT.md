...
- **Hybrids no longer count as species.** A `Mallard x American Black Duck` was still counting toward the headline species total on Statistics, toward per-county species counts on the Map Explorer, and on Calendar, while "sp." and slash forms were correctly excluded. All three now use one countable rule, so the county Species and Completeness metrics finally agree with each other, and Statistics' switch reads "Count spuh, slash & hybrids" like the Calendar tab's. Intraspecific intergrades such as `Yellow-rumped Warbler (Myrtle x Audubon's)` still count, which is what keeps the repair from erasing a species a birder has recorded only in that form. A user with no hybrid rows sees no change; a user with them sees a genuine downward correction.
