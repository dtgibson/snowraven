...
On a phone at large text sizes, a control and the label beside it now grow together. The rule that keeps text above the size iOS zooms in on was replacing each control's own size instead of raising it, so a control asking to be larger rendered *smaller* at 200% than it declared: the command palette's search lost 8px, the checklist lookup 4px, the weather forecast fields 3px. It now only ever raises. The section labels beside those controls, which had been left out of the rule entirely, are sized in relation to their own control, so a label and its control cross from floored to scaling at the same text size rather than drifting apart.

The Weather tab's Planner takes the bounds for everything it draws from the plan document itself, so a malformed or absurd forecast cannot make the tab hang while it draws. No plan a real forecast produces is affected.

### Five-build Spool bundle: Current tide works on the Apple apps again, and a weather or tide reading the app cannot trust says so instead of inventing one (complete -- September 2026, v1.0.32)

Settings opens on API Keys, with Default Files -- and iCloud Sync beneath it -- directly below, so the two sections the tab exists for are the first ones on screen; everything under them keeps its previous order. The segmented sort controls on List Comparer, Multimedia and Species Detail now say which option is in force to a screen reader, as the one on Checklists already did, so the highlight is no longer the only thing carrying that state.

**Current tide on the Mac, iPhone and iPad apps had been showing as unavailable rather than fetching the tide for where you are, silently, since v0.5.34, and it works again.** Those apps asked the tide provider for a moment spelled as a single space, which it refused; a source comment asserted the opposite behaviour, and the published claim in the help and the README had been false on those platforms for the whole period.

A weather reading the provider sends back malformed is now refused identically everywhere instead of being filled in: a missing temperature used to read `0°F` beside a fabricated "Clear sky" on the web and Raspberry Pi versions and `NaN°F` on the Apple ones, and either could reach the copy-ready block a user pastes into a public eBird checklist. Both now say the weather is unavailable. Two honest readings come back with it -- a forecast at polar latitudes, which the web and Pi versions used to refuse outright, and a lookup spoiled by one bad hour out of forty-eight. A date and time neither runtime can read is refused politely by both the tide and weather lookups before any request leaves the machine, rather than reporting an error, answering about a different moment, or claiming the device is offline while it is online. And a tide timestamp the app cannot place now drops that high or low from the reading instead of being treated as 1970, which had produced a plausible but wrong water level, a "Previous high ... at" line with no time after it, and an invented turning point on the Weather/tide Planner that persisted into the offline replay.

**Key modules:** `backend/services/wall_clock.py` + `frontend/src/lib/wallClock.ts` (the shared wall-clock predicate both runtimes refuse an unreadable date and time with, and the "now in the location's timezone" fallback); `backend/services/tide_instant.py` + `frontend/src/lib/tideInstant.ts` (the shared predicate that places a NOAA timestamp on the epoch axis, or reports that it cannot); `frontend/src/lib/tauri/locationZone.ts` (the single chokepoint for the native timezone lookup, so a zone name that does not resolve is handled in one place).
...
Statistics opens by saying what fraction of the user's checklists it describes and how many blocks were found but could not be read, so the coverage figure is visible before any chart is. With no readable blocks the section is not there at all; with only a few it says so and points at the Weather Backlog rather than charting anything. Above that floor it shows their outings distributed across the eleven sky conditions, seven temperature bands, nine wind bands and day versus night, then species recorded per checklist by band with the same bands' average time out beside it, because weather changes how long a person stays out and a species count that ignores that is measuring the birder rather than the birds. A picker there gives one species its own record.

The Species Detail card needs no picker, because the page already knows which bird it is about. It shows the sky conditions and temperature bands that bird has turned up in, each row scaled against the bird's own largest band, with the same band's share of all the user's outings printed beside it in words, and it opens on one numerator against two wholes with nothing dividing them. A bird with fewer than ten checklists carrying a readable block gets the whole card with its chart not yet arrived, said in a plain sentence that names both the count it has and the number it needs. That is the ordinary outcome rather than a failure -- four birds in five -- which is why it points nowhere and offers nothing to press.

Nothing is predicted and nothing is ranked. A band with too few checklists to average says so and keeps its count, a band with none takes a visibly different shape from a thin one, every figure names the checklists behind it, and no figure divides one share by another. Blocks written by RainCrow count exactly as SnowRaven's own do.

### Named Birds sightings in time, and an elapsed figure that is finally true (complete -- September 2026, v1.0.21)

Each named bird's card now shows that bird's sightings as a strip in time, and below the list a second strip gives every named bird a labelled lane on one shared axis, so a glance says which individuals overlap, which have gaps, and which have gone quiet. One control beside Sort chooses what both strips and every card's elapsed figure run to -- the bird's own last sighting, which is the default, or today -- so the number and the picture cannot disagree. The marks are selectable rather than decorative: pressing anywhere along a track picks the nearest sighting and a fixed two-line readout names its date, its place, and which of that bird's dates it is, with arrow keys stepping, Home and End jumping to the ends, and Escape clearing. Each strip is a single stop in the tab order however many birds or sightings it holds, so the tab gains exactly two, and a bird seen on one date only says so in a sentence rather than drawing an axis with nothing to show. The strips are drawn in one ink instead of a colour per bird, because the palette holds four distinguishable colours and a birder can name more than four birds. Nothing is fetched and nothing is written to disk; the range choice lasts as long as the visit.

The elapsed figure on every card was wrong two ways and is rebuilt. It truncated where it claimed to round, so ninety days read as "2 mos." and a day short of a year read as "11 mos.", and its fixed thirty-day months disagreed with the calendar on 15,013 of 67,500 date pairs less than 45 days apart -- the last day of February to the first of March is two days and it printed "4 days". Months and years now derive from the true number of days between the two dates the figure names: exact to the day out to sixty days, then months plus the exact days remaining, then years plus rounded months, and a bird first and last seen on one date reads "Same day" rather than "1 day". The figure also names its own endpoints in words, so "50 days · first to last sighting" says what it measured instead of leaving the reader to work it out.

### Six-build Spool bundle: Search from anywhere, Statistics computes beside the screen, and the export slots turn away the wrong file (complete -- September 2026, v1.0.20)

Pressing Cmd-K or Ctrl-K anywhere in the app opens Search over whatever is on screen, and both combinations work on every platform so one habit travels between a Mac and a PC. There is a visible Search control in the navigation for anyone who would rather press something: at the top of the sidebar on a wide window, as a magnifier at the top of the icon column on a narrower one, and as the first row of the More sheet on a phone. It reaches two kinds of thing. Destinations are the tabs the user has visible, in their saved order with Settings last, so a hidden tab does not appear and an empty query lists them all, which makes Search a quick jump between tabs as well. Species are the birds in the user's own eBird backup, matched on the common name and the scientific name at once, so calypte finds Anna's Hummingbird, and choosing one opens Species Detail on it even when that tab is hidden. Arrow keys move and stop at the ends, Enter opens, and Escape closes, as do the Close button, a press on the dimmed area, choosing a result, and a second press of the combination; whichever way it closes, the keyboard returns to the control it was opened from, or to the main content when that control has gone away. It makes no network request and saves nothing, so it works offline and needs no API key, and about the species half it says which of four things is true rather than showing an empty list: the backup is still being read, none is saved, a saved one will not load, or nothing matches. Destinations are there the moment it opens and are never held up by any of that, and replacing or clearing the backup in Settings reaches an open search straight away. Where the main pointer is touch, no key combination is shown, because there is none to press; attach a keyboard and both still work. It finds destinations and species, not counties, hotspots, locations or checklists, none of which has a page of its own to land on; and on a phone, while a map fills the screen, the navigation is not on screen and neither is the Search control.

Statistics does its arithmetic beside the screen instead of on it. The whole derivation, from the life list and the totals through the accumulation curve, the temporal and duration breakdowns, the geography, the effort and quality figures and the breeding stats, now runs on a thread of its own and arrives in one piece, so the window stays responsive while it thinks. Nothing on screen changes: the same "Computing your statistics" line covers the first visit, a toggle still flips instantly and updates the figures a beat later, and every figure, list and chart is identical for the same backup. On a slowed processor standing in for a phone or a Raspberry Pi, the longest unbroken block of unresponsiveness on a first visit fell from roughly 620 to 670 milliseconds down to roughly 380 to 430, and a press of Count all forms from roughly 180 to 200 down to roughly 86 to 91. Drawing the page and loading its map are both still substantial and neither is touched, so what is true is that the sums no longer add to them. The tab keeps a second copy of the observations beside the screen while it is in use and lets go of it about half a minute after the user stops. A large Macaulay Library export is read the same way, on a thread of its own as the eBird backup has been since 1.0.14, so a big export no longer freezes the window on whichever tab first needs it, and a read that goes wrong says the export could not be loaded instead of waiting for an answer that is not coming.

The two Default Files slots in Settings look at what is in a file before saving it. They used to check only that the name ended in .csv, so dropping MyEBirdData.csv into the ML Export slot stored it happily and the mistake surfaced later on Multimedia as an export that could not be turned into a species list, with nothing to say which file was wrong or which slot it belonged in. A file that is not the export for its slot is now refused on the spot, with a line under the row naming the slot it does belong in, and so is a file larger than 50 MB; in both cases whatever was already saved in that slot is left exactly as it was. The size named in each refusal is formatted from the same limit that enforces it, and each row keeps its empty alert region mounted before a refusal and replaces the message inside it on every attempt, so assistive technology receives a fresh announcement even when the same file is rejected twice. On the web and Raspberry Pi versions this also closes a straight untruth, since the server has always refused an upload over 50 MB and the app threw that answer away and reported the upload as saved. The desktop, iPhone and iPad versions never had a size limit at all and now have the same one.

The Calendar's day details box, the Map Explorer's filters sidebar, the first-run welcome screen, and the in-app help overlay hold the keyboard in place through one shared focus trap that reacts to where focus actually moved rather than predicting the browser's tab order. Traps are ordered by activation, so when Search, Help, or another modal opens above a contained surface, the lower trap yields for both focus movement and Tab, then resumes as soon as the higher modal closes; the welcome screen can therefore contain the live app behind it without stealing focus back from Help or Search. The filters sidebar also stops holding the Tab key once a phone is turned sideways: rotating to landscape with it open turns it from a cover over the map into a column beside it, so containment stands down while the map and page are visible alongside.

The sixth build in the bundle, shared control primitives, was declined at scoping for the second time with no code written.
...
The popups on the maps can be closed from the keyboard on the Mac, iPhone and iPad. Five of them -- a breeding-atlas block, a county, a Media Targets spot, a Nearby Lifers spot, and a pin on the Sighting Locations map -- relied on a close button the map library drew for itself, which Apple's WebKit engine does not put in the tab order and which no screen reader could name beyond the times sign. Each now draws its own, in the same corner at the same size and with the same larger target on a touch screen, named for the popup it closes; seven such buttons across the app now say which popup they close, on every platform. Two Map Explorer popups still have no close button at all, and Escape still does nothing to a map popup. Four published website screenshots also regained the link marks beside their species names, which a capture-time network stub had been silently cancelling.

### Species Detail counts species the way Statistics does (complete -- September 2026, v1.0.18)

Species Detail and Statistics read the same backup and used to disagree about how many species it held once Statistics had checked exotic status, because only Statistics left out a species that eBird tags Exotic: Escapee on every checklist it was recorded on. A third switch in the Species Detail toolbar, Show escapees, now carries that rule: off by default, so the tab's "N species" figure reads the same as the Statistics Species headline and those birds are out of the species selector, and one press puts the rows and the number back. The tab asks eBird nothing of its own. It reads the answer Statistics already saved on this device and confirms each name against the checklists it can see, so where Statistics has never run the check nothing changes, a species stays listed while any checklist carrying it is unchecked, and a domestic form of an escapee hides with its parent under Show subspecies. Clicking an escapee's name on Statistics, or picking one in the Subspecies and forms control, turns the switch on and opens the species rather than opening the tab with nothing selected, and a form hidden by Show all forms is revealed the same way. The switch is per-session and separate from the Count escapees checkbox on Statistics; only their defaults agree. With it, every boxed switch across the app responds to the pointer, stepping its border and fill to the hover tones in both themes, while a switch that cannot be operated stays at rest.

### One responsive navigation at three densities (complete -- September 2026, v1.0.17)

Eleven destinations had outgrown the strip across the top of the window. The strip swapped itself for a single dropdown the moment it would not fit, and it stopped fitting at roughly 1,457px -- wider than most real windows -- so the dropdown was what people usually got: all eleven destinations in one scrolling list, on every unmaximised laptop window, every iPad and every phone. There is now one navigation that changes shape with the room it has: a labelled list down the side of a wide window, a narrow strip of icons when there is less room, and on a phone a bar along the bottom holding the first four destinations with a More sheet for the rest. Which one appears follows the width actually available, measured against the app's own 640px content floor, never a device check -- so a half-screen Mac window and an iPad in portrait get the same treatment for the same reason, and on the Map Explorer the navigation steps down to icons by itself once its own sidebar would leave the map too little room, then steps back when that sidebar closes. A Collapse control at the foot of the side list narrows it sooner; it lasts for the session only, so it can never be restored into a window it would be wrong for.

The saved tab order is reused unchanged -- same document, key, seam and normalizer -- and the phone bar's four favourites are simply the first four destinations the user has chosen to show, so the existing reorder-and-hide controls already decide them and nothing new is stored. Moving the brand block and tagline out of the page header into the navigation column returns roughly 150px of height to every tab on every wide window: the map is taller, Statistics starts higher, the Breeding Codes table shows more rows. Multimedia's glyph changed from a list to a picture frame, which at rail size the Checklists clipboard could no longer be told apart from; the destination is still named Multimedia. Every control the navigation draws is reachable by Tab on all six platforms, the More sheet keeps the keyboard inside it and closes on Escape, and at large text scales the phone bar drops its labels rather than clipping them. The app is left with **two** roving-focus groups rather than three and a tab-order exclusion roster of **four** rows rather than five, because the collapsed dropdown's listbox is retired rather than rewritten.

**Key files:**
- `frontend/src/components/TabNav.tsx` — the single nav entry: sidebar, rail, bottom bar and More sheet
- `frontend/src/lib/navDensity.ts` — the pure threshold arithmetic, with hysteresis (collapse below the floor, restore at floor + 48) so a window drag cannot flip it every pixel
- `frontend/src/lib/tabIcons.tsx` — the one authoritative glyph table, at four nav scales
- `frontend/src/lib/mapPanelChrome.ts` — gains a `fixedBelow` term, because the fixed bottom bar is not in `<main>`'s sibling flow at phone density

### Keyboard reach on the Apple platforms, and two more honest load failures (complete -- September 2026, v1.0.16)

The app is operable from the keyboard alone on the Mac, iPhone and iPad, which it was not. Those three apps run inside Apple's WebKit engine, whose own default gives no place in the tab order to a plain button or a plain link unless the page asks for one, and almost nothing in the app asked. The effect was not subtle: the buttons in every map's corner could not be reached at all, including the fullscreen button whose only job is to enter fullscreen and the "Atlas blocks in view" panel that is the only keyboard route to a breeding block; on Statistics the jump links, the four groups of chart toggles and every link out to eBird and the Macaulay Library were skipped; on Multimedia and Breeding Codes no sortable column heading could be reached, so neither table could be sorted from the keyboard at all; nothing inside the Weather tab's checklist backlog could be reached; and every link in the app was skipped, the "Skip to main content" link that is meant to come first of everything included. Every app-owned button and href link now renders through one of two thin native primitives that provide the WebKit-safe tab stop by default while forwarding the same props, refs, styles, button types and semantics, so the same controls are reached by Tab, in the order they are read, on the web, on Windows, and on the Mac, iPhone and iPad, without depending on a system setting; Enter presses them and Space does as well on the ones that are buttons. Intentional non-default behavior remains explicit only for roving groups, redundant controls and native-disabled states, with exact membership owned by an executable roster rather than copied into product prose. Two Calendar repairs came with it. Opening a day used to put the keyboard on the day box's Close button and then lose it to the page behind on the very first press of Tab, because there was nothing inside the box the keyboard could reach for the box to hold on to; the box now keeps the keyboard inside it until it closes. And the "Count all forms" switch stays reachable when the chosen metric makes it inapplicable, with the one-line note that explains why attached to the switch rather than sitting out of reach beside it. What is left over is the small number of controls the map library draws for itself rather than the app drawing them: its zoom buttons, which the + and - keys already cover, and the close button on the map popups, which v1.0.19 took over on all five that had one, leaving two Map Explorer popups with no close button at all and written up as the remaining gap. The accessibility statement now says something a reader can check, and a test asserts exactly that across every screen on every build, so it cannot quietly lapse as the app grows.

Every stored-file surface now preserves the difference between "no file saved" and "could not find out whether a file is saved." An absent metadata record or a successful empty status still shows setup guidance, while an unreadable Tauri record, a failed web or Pi status response, or a rejected status lookup reaches the existing load-failure panel on Statistics, Calendar, Checklists, Breeding Codes, Named Birds, Species Detail, Map Explorer, Multimedia, Weather Backlog, and Search. A saved Macaulay Library export that cannot be read or parsed likewise reports that failure instead of rendering an empty library, so no storage failure is presented as proof that the user's file is absent.

### Fullscreen on the embedded maps, and load failures that are honest out loud (complete -- September 2026, v1.0.15)

Three of the maps that live in a box on a scrolling page expand to fill the window from a button in the same bottom-right corner, and with the same icon, the Map Explorer has always had: Species Detail's Sighting Locations map in both its Pins and Heatmap views, the Statistics Geographic Stats map, and the per-individual map on a Named Birds card. It is the same map rather than a new one -- the same pins, base map, centre and zoom, an open popup, a dropped share pin and the county shading all survive the round trip in both directions, with nothing re-downloaded and no jump back to the starting view; Escape closes it and returns the keyboard to the button that opened it, the Tab key stays inside the expanded map, and expanded the map takes the scroll wheel and a one-finger pan because there is no page behind it to scroll. On iPhone and iPad the controls clear the status bar, the Dynamic Island and the home indicator in both rotations. The controls that sit beside a map rather than on it stay on the page, and the Map Explorer and the Weather tab's location picker are unchanged. Opening a pin on the Species Detail and Named Birds maps also stopped throwing away where the user had scrolled to -- a shipped defect that had been snapping those maps back to their starting view on every popup. And a saved file that cannot be READ is now reported as what it is on the last surfaces that still lied about it: on the web and Raspberry Pi versions, an unreachable backend or a download cut short partway through used to send all eight data tabs to the upload instructions for a backup the user plainly had, and they now name the file and where to fix it, with a later visit trying again rather than staying stuck. That message is also spoken now: on all eight tabs the failure sentence is announced to a screen reader, where six never announced it at all and the two that did announced unreliably and read the action button as part of the failure. Nothing about it looks or behaves differently on screen.
...
### iCloud Sync (complete -- September 2026, v1.0.12)

An opt-in, off-by-default iCloud Sync section in Settings on the Mac, iPhone and iPad apps keeps the eBird backup and the Macaulay Library export the same across the user's own Apple devices through their own iCloud account (the most recently uploaded copy wins, whole; each file row shows which device its copy came from, when, and one of eight plain-text sync states), and a second off-by-default Sync API keys switch, available only while file sync is on and turned off with it, shares the eBird and OpenWeather keys through one two-slot record in the same private container (the most recently changed key wins per key, a Clear reaches every sharing device, turning the switch off removes the copy from iCloud, and each key row shows where its key came from and its state), while settings and caches stay device-local and Windows, web and Pi are unchanged.
...
### County shading on two more maps, and eBird project contributions (complete — August 2026, v1.0.5)

The county overlay reaches two maps that carried none. On **Species Detail** a single **Counties** switch shades only the counties where the selected bird has been recorded, so a bird's own page answers "where have I found this one" with a map rather than a list of pins, and a county's popup names the county, how many checklists reported that bird there, and at which spots; switching species reshades without a reload. On **Statistics**, the Geographic Stats map shades every county birded, by species or by checklists, on the Map Explorer's own ramp, textures and contrast guarantees, and its numbers agree with the Map Explorer's for the same county and metric. Both are pure offline computation over the loaded export — no connection, no API key — and both behave exactly as before until the overlay is turned on. **Completeness is deliberately not offered on either**, because it is the one metric that costs a request per county, because on Statistics it would sit beside county tables using a different numerator rule, and because per species it has no meaning.

And the Statistics tab reports **which eBird projects a birder has contributed to** — the California Breeding Bird Atlas among them — with a checklist count, a date range, and its share of the checklists checked. The backup file does not record project membership at all, so this is a check the app runs against eBird with the user's own key, one request per checklist: nothing is sent until the user presses **Check projects**, the section states the exact cost in requests and minutes first, and it is stoppable at any moment. Every answer already paid for is kept, a stopped or interrupted check resumes by asking only about what is still unanswered, and because a checklist's project assignment never changes after submission, loading a newer export later asks only about the checklists it added. That is what turns an eight-minute cost into a one-time cost.

- **Every tally is shown with its denominator, and a never-run section shows no number at all** rather than a zero — "3 projects across 412 of 3,300 checklists checked" is honest, "3 projects" on a partial check is not. A partial answer therefore reads as a floor.
- **How a checklist was submitted** — the eBird website, the eBird app, or Merlin — is reported separately and labelled as such, because a submission portal is not a project. A project's name is never invented: known project codes and ids carry a bundled label, and anything unknown shows its raw identifier verbatim.
- **The card is dense and visual, with the chart as decoration only.** Each project reads as one line — name, count, then its share and date span — and with two or more projects a small bar chart beside the rows compares checklists per project (per portal, when only portals have rows), each charted row tied to its bar by a colored dot. The chart carries no figure of its own: everything it shows is in the rows as text, with its denominator, and with one project there is nothing to compare and no chart renders. Portal rows take the same one-line tightening and stay chartless while any project is listed.
- **The check never outpaces the app's eBird pacing contract, and it escalates instead of oscillating under sustained rate limiting.** It runs through the same key-global gate as every other eBird lookup, at one enforcement point, so a rate limit anywhere still slows everything on the key; a 429 is surfaced as a 429 on the checklist path. On top of the gate, each 429 wave during a run widens the sweep's own spacing for the rest of that run, and after the third the check pauses itself — session-only, every answer kept, resume asking only about what is still unanswered — suggesting about an hour as guidance, never an enforced lockout. The Map Explorer's own lookups never inherit the pause. Offline, missing-key, and provider-failure all degrade through the app's three honest states. The privacy policy's eBird bullet names the per-checklist project lookup.
- **County popups no longer open past the edge of a narrow map.** A popup wider than two thirds of its map cannot be anchored at all — a side anchor puts an edge at the click point — so at 320px and the largest text scale the county popup opened well past its map and took its close button with it. It is now capped to what the map can anchor, and on a map too narrow for a readable one it pins to the map instead. The same overflow was latent on Species Detail and the Map Explorer at ordinary text sizes; all three are fixed, and any map wider than about 380px is unchanged.
- Shipped to macOS, Windows, the website and TestFlight. **Not submitted to the App Store**: 1.0.5 cleared the standing bar for replacing an in-review submission and the user chose to let the in-review 1.0.4 finish review instead, so 1.0.5 reaches the store as a later submission.

### Colorblind-accessible hotspot pins — Use Tier Rings (complete — August 2026, v1.0.2)

The Map Explorer's "Color pins by" modes gain an opt-in **Use Tier Rings** switch: with a color mode active, each ramp pin carries a thin white five-segment ring whose filled segments equal its tier, so tiers read by structure — extent at map scale, count up close — without hue or luminance discrimination. The legend minis and the popup's tier badge draw the same ring from the one shared spec, and with the switch off every sprite is the shipped build byte for byte (the default coloring, non-value states, personal pins, and the 9px in-view dots never change in either state; guarded by structural-monotonicity, ring-contrast, and op-stream byte-identity tests). Off by default and persisted through the storage seam (`hotspotTierRings`) — the deliberate, logged deviation from the session-only Use-Textures precedent, because a vision-linked reading aid should not need re-enabling every launch. Shipped everywhere except the App Store, where the in-review 1.0.4 submission was deliberately kept; 1.0.2 follows as a store update after its approval.

### Mobile chart tip + escapee domestic-form repair (complete — August 2026, v1.0.1)

On phones, Statistics and Species Detail each show a one-time dismissible tip above their charts (rotate to landscape, or use the desktop app; neutral, never steering), persisted per page through the storage seam via a `chartTipDismissed` map; tablets and desktops never mount it (`components/ChartViewTip.tsx`, `useIsPhone` render-branch). And the escapee check finds domestic-form-only birds again: the Statistics taxonomy batch sends each raw name plus its normalized parent (the species-only `codes` maps cannot resolve a "(Domestic type)" name, so those species never entered the escapee cover and the tab read zero escapees), and a store poisoned by a broken pass heals itself — a fresh ledger entry never stands for a recordless cover species (`carriersNeedingRefetch` + explicit `refetch` intent at `dedupedFetchChecklist`). The App Store submission was replaced in review so the store debut carries the fix (submission b52fdc55, releasing on approval; Phase B keys off it).

### iOS App Store release (complete — August 2026, v1.0.0; live on the App Store)

SnowRaven is live on the public iOS App Store: the 1.0.0 debut (a deliberate one-time jump from 0.5.93; the incremental rhythm continues upward from it, never below) shipped to every platform at once, the store record that carried it was approved and is on sale, and later versions reach the store as ordinary updates. App behavior is unchanged on every platform — what this feature built is the store presence and the standing pipeline behind it.
...
- **My species and My checklists are fully offline joins** over the loaded backup by location id, zero network verified at the transport seam. My species counts by the shared countable rule (subspecies fold to species, non-countable forms excluded), so its number can honestly differ from the popup's raw "species recorded" line; both render, labeled. Escapee provenance is deliberately not applied.
- **Recent activity is one new dual-transport endpoint, `GET /map/hotspot-activity`** (`backend/routers/map.py` ↔ `lib/tauri/mapService.ts`, shared parity fixture), reducing eBird's per-location `data/obs/{locId}/recent?back=30` to one `(speciesCode, obsDt)` pair per species — one call answers both windows, so a Week / 30 days flip never refetches. Mode 3 counts eBird's taxa as returned (no form collapse), and the popup label says "species reported" accordingly.
- **Fetching is bounded and paced:** result-set-scoped (never viewport-driven), pool of 4, capped at 200 per search (on-screen hotspots first, then nearest the search center; cache hits exempt), deduped in flight, request starts spaced 150 ms, and any eBird 429 opens ONE key-global Retry-After-honoring cooldown with bounded per-hotspot retries — a rate limit is a brief visible slowdown, and a hotspot still limited through its retries lands in the honest "not checked yet" state with Retry. Answers persist in a durable 6h storage-seam cache (`hotspot-activity-v1`, FIFO at 2,000 fixed-shape counts-only entries, per-entry validation on load, errors never cached, offline stale-reads with as-of wording); the route is deliberately NOT in `CACHED_GET_PATHS`.
- **Values land on a five-class cyan-blue quantile ramp** (`--sr-hotspot-1..5`, theme-identical, map-anchored), contrast-guarded at the token in `hotspotContrast.test.ts`; the nodata state deviates from the uniform ≥3:1-vs-land clause by design (pale fill, ring-supplied boundary) with replacement guard clauses. Four honest non-value states never masquerade as low values: hollow means "asked, zero" (the personal-zero and community-quiet states share one fill token with distinct wording), dashed gray means "not checked yet", pale means "never birded by you". Personal locations keep their orange star and are excluded from all ramps.
- **Nothing depends on perceiving color:** an opt-in **Use Tier Rings** switch (persisted via the storage seam, `hotspotTierRings`, off by default) bakes a five-segment ring into every ramp pin — filled segments = tier — so the tiers read structurally without hue perception, with one shared `HOTSPOT_TIER_ARC` spec driving the pin sprites, the legend minis, and the popup's tier badge so they cannot drift; the legend auto-reveals with true class ranges and carries the kind hide/show filters as glyph chips (color no longer encodes kind while a mode is active); the popup and the keyboard "Hotspots in view" list state every pin's number or state in words, and the popup states the visited state in every mode. Mode 3's progress region is throttled and emits frozen snapshots so the status line genuinely updates at the stated rate.
- Offline, missing-key, and lookup-failure degrade through the app's three honest states, with a Retry that re-asks only the unanswered remainder without re-running the hotspot search. `PRIVACY_POLICY.md`'s eBird bullet names the per-hotspot recent-community-sightings call; HELP, README, website, and ACCESSIBILITY updated in the same change. Shipped as v0.5.92 to macOS, Windows, the website, and TestFlight.

### Search this area on the Map Explorer, plus a corrected map panel height (complete — August 2026, v0.5.91)

A **Search this area** control on the Map Explorer's three centre-based views (Hotspots, Nearby Lifers, Media Targets). Pan or zoom somewhere new and it appears on the map; one press re-runs whichever of the three you are on over what is on screen, with no trip through the Filters panel. It is one control rather than three, because the centre and radius are already shared state and only one of the three views is ever on screen. Shipped alongside it, at the user's choice, is the repair of a pre-existing defect the new control made visible: the map panel had been sizing itself from a fixed guess at the app's own chrome.

- **The centre is the middle of the view; the radius is the smallest of the existing 5, 10, 25 or 50 mile rungs that covers it, capped at 25.** The radius is measured centre-to-corner, so the searched circle circumscribes the rectangle on screen rather than fitting inside it, and the shape of the view matters and not only its width. Past the cap the circle is deliberately smaller than the view, which is the one case where "everything on screen was searched" stops being true and exactly the case the drawn indicator exists to make visible.
- **The press adopts the centre into the sidebar's coordinate boxes but leaves the Radius control alone.** The user's radius setting is theirs, so the circle drawn on the map is what reports the size actually searched. The stated consequence, accepted deliberately: after a press the circle and the Radius control show different sizes, and leaving a centre view and returning re-runs that view's search at the sidebar's radius.
- **The searched area is shown by dimming the ground outside it, not by drawing a ring.** A covering radius puts the circle's edge off screen, so a ring alone would be invisible at exactly the moment the feature is working. Nothing dims while the search covers what you can see; the dashed edge appears once the boundary comes into view; and the capped case explains itself without a word of copy.
- **Nothing is ever sent without a press, and one press costs one lookup.** Panning and zooming issue no request in any form, including debounced. A search run from this control does not re-frame the map onto its results, which is what stops a press moving the map out from under its own record and inviting a second, wider press.
- **Every search route now announces its outcome on the map**, not only this control: the sidebar Find button, place-name search, Use my location, a dropped or dragged centre pin, and a view-mode change all say what was found or what failed, in a live region that is the same node a sighted user reads. On a phone the sidebar is off screen, so this is a gain on the shipped routes.
- **Where the map is too short to hold another row of controls, the control withholds itself** rather than covering something that must stay pressable. Measured: at ordinary text size no supported viewport withholds it; withholding appears only at larger text scales and follows the map's available height. Everything it does stays reachable through Filters and Find.
- **The Map Explorer panel now measures the app chrome above and below it instead of assuming it.** The old fixed guess was smaller than the real header, navigation and footer at every width and text scale measured, so the map's corner buttons could sit below the bottom of the window until scrolled. Measuring also covers the two cases a constant could never get right: the navigation changing density, and the larger text sizes. Since v1.0.17 it also adds the phone bottom bar's measured height, which sits fixed rather than in `<main>`'s sibling flow and so is invisible to the above/below arithmetic. A map minimum height is retained, so the narrowest phone at the largest text size still scrolls a little by design.
- No new dependency, endpoint, backend route, desktop service function, tile provider, or stored setting, and no privacy-policy change.
...
- **The correction runs both ways, and most totals go up.** 88 names eBird counts were being excluded, folding into 59 parent species and ordinary for a North American birder (`Canada Goose (moffitti/maxima)`, `Redpoll (Common/Hoary)`, `Dark-eyed Junco (Slate-colored/cismontanus)`). 81 were being counted that eBird does not count: three named hybrids whose names give no hint (`Brewster's Warbler (hybrid)`, `Lawrence's Warbler (hybrid)`, `Bogota Sunangel (hybrid)`), 25 spuhs, and 53 undescribed or unrecognized forms. Excluded birds stay on the Life List; only the count changes.
- **The controls are `Count all forms`** (Statistics, Calendar) **and `Show all forms`** (Multimedia, Species Detail). The old labels named a set that no longer exists. The two `Show all forms` toggles now govern the whole non-countable set, so a tab's visible rows and its own "X of N species" figure agree by construction instead of leaving hybrid rows on screen that the count excluded.
- **Media documentation coverage and the Frivolous Lists always use countable species, whichever way the toggle is set**, and now say so on screen. Both are about the canonical life list rather than about what was recorded, so the toggle deliberately does not reach them; the help text had claimed otherwise.
- **Offline and instantaneous, with nothing new fetched.** The verdict ships as the naming convention plus its 169 corrections, adding 4% to the initial download rather than the 32% a full lookup would cost, with a test asserting the pair equals eBird's full answer name by name. A name eBird does not publish falls back to the old convention, so anything outside the taxonomy behaves exactly as before.

### Escapee-aware species count on Statistics (complete — August 2026, v0.5.87)

An off-by-default **Count escapees** toggle beneath the form-counting switch (now labeled "Count all forms"), so the Statistics species total follows eBird's own life-list rule: Exotic: Provisional and Exotic: Naturalized count, Exotic: Escapee does not. With the toggle on, the total is exactly the pre-v0.5.87 number, so the earlier behavior is preserved rather than approximated. On the reference export the default figure reads 264 rather than 267 (Graylag Goose, Swan Goose, Muscovy Duck), matching what eBird shows the same birder.
...
- The corrected rule reaches the surfaces that headline a life-list count: Statistics totals and milestones, media documentation coverage, county Completeness, Calendar species counts, and the Frivolous Lists. Of the surfaces that list species, Species Detail alone reads it too, as a passive reader behind an off-by-default Show escapees switch, so its selector and species count agree with the Statistics headline at both tabs' defaults; Multimedia, Breeding Codes, the List Comparer and the Map Explorer are unchanged. Two stated limits: county Completeness applies the rule to its numerator only, against eBird's unfiltered regional species list, which its caption now says in words; and Statistics' Geographic Stats per-county counts are out of scope.
...
- **Countable intergrades survive all four import families.** The eBird backup, Life List, Macaulay Library and both Breeding Codes paths converged on one classifier applied to the raw name, rescuing 36 bundled-taxonomy forms into 26 parent species with 0 additional names excluded. (v0.5.89 replaced the classifier itself with eBird's `reportAs` rule; the convergence and all 36 rescues stand.)
...
- **The Map Explorer's corner buttons are one family on every view.** Hotspots, Nearby Lifers and Media Targets gain the share button the My Sightings view already had, opening the copy popup belonging to the search-center pin already placed there rather than dropping a second pin of its own, and panning that pin back into view first if it has drifted off screen; the fullscreen button joins its neighbours at the same size, and every glyph in the row now grows with the Text Size setting instead of staying fixed.
- **Pressing Pin code labels no longer stretches the Breeding Codes table.** On a desktop it had roughly doubled every column, because the Unbounded view sized its card by the tier legend beneath the table rather than by the table itself; the legend now wraps to the table's width instead of setting it, and the phone layout and pinning itself are unchanged.
- **The in-app Help no longer has to be dragged sideways just above phone width.** At large text sizes and window widths a little wider than a phone, a long web address ran past the edge of the text column and pushed the whole panel sideways, because the phone fix had been applied at phone widths only; long addresses now wrap at every width.
- **The in-app Help makes a link clickable only when it points at an ordinary web address**, showing anything else as plain text. The only help content SnowRaven ships is its own file, so this is a precaution rather than a repair.
- **Trimming a trailing parenthetical from a bird name is now a single pass** instead of a pattern that slowed sharply on malformed input, producing exactly the same name for every real bird, so no list, count, or total moves.

### Four-build Spool bundle: a location button, pin-label parity, and two counting and layout repairs (complete — August 2026, v0.5.83)

Four queued builds shipped as one release: one new feature, one improvement, and two fixes. Frontend only throughout: no new provider, backend route, dependency, persisted setting, or Tauri grant, and location detection itself is unchanged on every platform.

- **A location button on the Map Explorer's map surface.** The floating control cluster gains a "Use my location" button on all four views, so centering on yourself no longer means opening the Filters panel, and is possible at all on My Sightings, where there had been no location control. The two round white buttons beside each other are now told apart by silhouette rather than by reading a tooltip: the share pin's glyph becomes a planted flag, matching the marker it actually plants, and the location button takes a target reticle. A location failure gets a home on the map itself, click-through and announced to a screen reader, carrying the existing platform-specific remediation wording. A press on My Sightings is a pure recenter that transmits nothing.
- **Pin column labels on the Multimedia tab, matching Breeding Codes.** Both tables now offer the same opt-in pill that keeps the header row visible while a long list scrolls, off by default, per-session, and offered in the "↔ Unbounded" view under one invariant: pinning implies Unbounded. Multimedia's header pin had been anchored in a way WKWebView does not honor, so it had very likely never worked in the macOS app or on iOS; it is repaired, cleared of the Dynamic Island, and keyboard focus can no longer land beneath it. Pinning freezes the code header row only: the species-name column's freeze remains a property of the Normal view.
- **Hybrids no longer count as species.** A `Mallard x American Black Duck` was still counting toward the headline species total on Statistics, toward per-county species counts on the Map Explorer, and on Calendar, while "sp." and slash forms were correctly excluded. All three moved onto one countable rule, so the county Species and Completeness metrics finally agree with each other, and Statistics' switch was brought into line with the Calendar tab's. Intraspecific intergrades such as `Yellow-rumped Warbler (Myrtle x Audubon's)` still count, which is what keeps the repair from erasing a species a birder has recorded only in that form. A user with no hybrid rows saw no change; a user with them saw a genuine downward correction. (v0.5.89 replaced the shared rule with eBird's own and relabeled both switches; the agreement between the metrics is what stands.)
- **The in-app Help no longer scrolls sideways on a phone.** Every line of prose had run off the right edge mid-word and had to be dragged left and right to read, at 320px and worse at large text sizes. The content column now fills the screen and wraps, while the one legitimately wide item, the coordinates example, keeps scrolling inside its own box.
...
- **One text size across every phone filter row.** `.sr-input-16`'s flat 16px iOS focus-zoom guard sat beside 12px pills and *inverted* at 200% text scale (pills 24px, controls still 16px). One shared formula in the ≤640 tier, plus a new `.sr-ctl-row` container hook, puts both sides on one size across Life List, Multimedia, Checklists, Breeding Codes, Species Detail, and Calendar. (That formula was a replacement rather than a floor, so it still cut a control declaring a larger size DOWN at 200%; corrected in v1.0.33.)
...
- **Coverage fix:** the "X of N life-list species documented with media" denominator excludes non-countable forms via the shared predicate in `lib/speciesUtils.ts` (`isNonCountableForm` since v0.5.89), correcting an overcount (it had counted every distinct observed name). Isolated to `computeMediaStats`; `backboneNames` (Species-Detail linking) untouched.
...
- **(v1.0.21)** Each expanded individual now also carries a selectable strip of
  its sightings in time, the tab carries a master strip of one lane per named
  bird, and the elapsed figure named above was rebuilt on a true day count --
  see the **Named Birds sightings in time** entry at the top of this file.
...
- Excludes the forms eBird does not count toward a species list (spuhs, slashes, hybrids, undescribed forms); a subspecies-level form counts as its parent species, and soundscape entries are included
...
- Excludes the forms eBird does not count toward a species list (spuhs, slashes, hybrids, undescribed forms); a subspecies-level form counts as its parent species, and soundscape entries are included as first-class rows
...
- **Three toolbar toggles:**
  ...
  - **Show all forms** (default OFF) — hides every form eBird does not count toward a species list, so the visible rows agree with the tab's own species count (labeled "Show sp./slash" and covering only spuhs and slashes before v0.5.89)
...
**Key files:**
- `frontend/src/lib/speciesUtils.ts` — shared module: `normalizeSpeciesName(name)` (strips trailing parentheticals) and `isNonCountableForm(name)` (eBird's countability rule; replaced `isSpuhOrSlash` in v0.5.89); used by `LifeList.tsx` and `SpeciesDetail.tsx`
...
no code, or with a code that fails the link shape gate (soundscapes, pre-fetch), show nothing.
A favicon that fails to LOAD is a different state: since v1.0.19 the mark reveals a bundled
stand-in glyph in the same slot rather than going invisible.
...
- Favicons loaded from `ebird.org/favicon.ico` and `birdsoftheworld.org/favicon.ico`; `onError` hides the `<img>` and reveals a bundled lucide glyph in the same reserved slot (per image, non-destructive, `onLoad` restores the favicon in place); the images carry `className="sr-favicon"` for dark-mode CSS filter treatment
...
- External favicons in `SpeciesLinks` carry `className="sr-favicon"`; `globals.css` applies `filter: brightness(0) invert(1); opacity: 0.65` in dark mode to keep them visible. That selector is `img.sr-favicon`-scoped and deliberately does NOT reach the fallback glyph, which is a token-coloured SVG stroke and would be destroyed by the filter.
...
- **Non-countable forms toggle** — second `ToggleSwitch` ("Show all forms", "Show sp./slash" before v0.5.89) defaults to OFF (hidden). Hides entries `isNonCountableForm` rejects.
- **Escapee toggle** -- third `ToggleSwitch` ("Show escapees") defaults to OFF (hidden). Hides a species that eBird tags Exotic: Escapee on every checklist carrying it, as published by the Statistics exotic-status check and confirmed against the loaded backup, so the tab's species count agrees with the Statistics headline at both tabs' defaults; a request for a species hidden by either reveal switch turns that switch on and selects it. Session-only, separate from the Statistics checkbox. Every boxed `ToggleSwitch` carries a hover state.
- **Subspecies Explorer** — a "Subspecies and forms" control below the species selector lists every species in the loaded backup with at least one countable subspecies or form entry (each with its forms and their share of that species' reports), and a per-species "Subspecies and Forms" section shows each form's report count and percentage plus a "No form noted" row for plain species-level reports — displayed shares sum to exactly 100%, a footnote reconciles against the Sightings figure whenever non-countable variant rows make that figure larger, and everything derives offline from the loaded backup in the default merged mode.
...
- On screens ≤640px, a circular fullscreen (maximize) button appears next to the floating Filters button in all three Map Explorer modes. Tapping it expands the map to fill the entire viewport — the app header, the navigation bar, and mode tabs are hidden — for maximum map area on a phone. Tapping the minimize icon restores the normal layout.
...
**What it does:**
- Drag to reorder tabs using a six-dot grip handle; the navigation updates immediately
...
- No first-paint flash — initial navigation order is derived synchronously before React's first render using a lazy `useState` initializer (NFR-04)
- These preferences also choose the phone bottom bar's four favourites (v1.0.17): they are the first four *visible* tabs in the saved order, so this section already decides them and the navigation stores nothing of its own
...
- **Main navigation — roving tabindex:** The `role="tablist"` uses roving tabindex: the active destination has `tabIndex={0}`, all others `tabIndex={-1}`. This is the correct ARIA tablist pattern — Tab exits the group entirely. Since v1.0.17 the list is vertical (`aria-orientation="vertical"`) and the arrow keys are **Up/Down**, not Left/Right; Home/End are unchanged. It is the app's only remaining roving group besides the Settings choice rows. The phone bottom bar and the More sheet are deliberately **not** roving — plain buttons at `tabIndex={0}`, because roving buys nothing across four cells and a tablist may not contain a non-tab button.
...
- `frontend/src/components/TabNav.tsx` — the navigation's roving tabindex + arrow key navigation (moved out of `App.tsx` in v1.0.17, which now builds the items and renders the panels)
- `frontend/src/App.tsx` — tabpanel ARIA, `<main>` wrapper, `role="contentinfo"` on footer, live regions
...
### Responsive Tab Navigation (complete — May 2026, v0.3.29) — SUPERSEDED

**Replaced by the responsive navigation at three densities (v1.0.17), at the top of this file.** The horizontal strip and the dropdown it collapsed into are both gone, so this entry's description of them no longer describes the app.

What this feature established and what survived it: navigation shape is chosen from **measured available width**, never a fixed pixel breakpoint, and the navigation reuses the saved tab order and visibility rather than storing anything of its own. Both rules carried straight into the replacement and were extended to its three density tiers. What did not survive: the dropdown itself, and this feature's expectation that a future mobile app would inherit it. See DECISIONS.md for the reversal and its reasoning.
...
**WKWebView (Tauri) requires explicit `tabIndex={0}` on every `<button>` AND every `<a href>`**
WebKit's default tab mode, which is what the shipped Mac, iPhone and iPad apps run, visits only elements carrying an explicit `tabindex`, native form controls (`input`, `select`, `textarea`) and `<summary>`. **A plain `<a href>` is skipped there too**, which this entry said the opposite of until v1.0.16 measured it. So every `<button>` and every `<a href>` added to any component must carry `tabIndex={0}`. The exceptions are the roving-tabindex groups, which hold one tab stop on purpose and must NOT get `tabIndex={0}`: the main navigation's vertical tablist and the three Settings choice rows. Two further roster rows are not roving at all — the species selector's chevron, whose text box opens the same list, and a base-map button the platform removes via native `disabled`. That is four rows; it was five until v1.0.17 retired the old strip's collapsed dropdown. There is no shared `<Button>` component in this app; the attribute is written at each call site, and `frontend/src/lib/tabOrderCoverage.test.ts` asserts the property over every shipped `.tsx` on every build, with a literal exclusion roster that binds each row to its own site by a declared count. Two earlier sweeps (v0.3.28, then the map corner controls) each believed they had closed this and had not; the guard is what makes the claim survive the next component.
...
is truthy AND matches `SPECIES_CODE_RE` (`/^[a-z0-9-]{2,16}$/`), or `null` otherwise. This means soundscape entries,
pre-fetch rows, species not found in taxonomy, and a name that resolves to an inherited `Object.prototype` member
all silently show no icons — no broken state, and no junk in an href. Favicons are loaded from the live sites; a
load that FAILS hides the `<img>` and reveals a bundled lucide glyph in the same reserved slot, per image, with
`onLoad` restoring the favicon in place. Both `<a>` elements carry `rel="noreferrer"` to prevent tab-napping.
...
`normalizeSpeciesName` and the countability predicate (then `isSpuhOrSlash`, now `isNonCountableForm`) are exported from `frontend/src/lib/speciesUtils.ts`
