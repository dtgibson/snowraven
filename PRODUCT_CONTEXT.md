...
- **The Map Explorer panel now measures the app chrome above and below it instead of assuming it.** The old fixed guess was smaller than the real header, tab strip and footer at every width and text scale measured, so the map's corner buttons could sit below the bottom of the window until scrolled. Measuring also covers the two cases a constant could never get right: the tab strip folding into a dropdown, and the larger text sizes. A map minimum height is retained, so the narrowest phone at the largest text size still scrolls a little by design.
