...
The Calendar's day details box and the Map Explorer's filters sidebar hold the keyboard in place by construction rather than by predicting the order the browser will visit their controls in, which is what the app's other overlays have done since 1.0.15; the same shared machinery is now behind the first-run welcome screen and the in-app help overlay, with no change to how either behaves. Pressing Tab in any of them behaves exactly as it did. The filters sidebar also stops holding the Tab key once a phone is turned sideways: rotating to landscape with it open turns it from a cover over the map into a column beside it, and it used to go on cycling the keyboard around its own controls with the map and the page plainly visible alongside. It now stands down the moment the screen is wide enough that it is no longer a cover, nothing on screen moves, and the keyboard is left on whatever control was already in use.
...
Two more surfaces stopped reporting a failure as an absence. At the bottom of the Weather tab, "List checklists with no weather blocks" used to show the same panel to a birder whose saved backup would not load as it shows to one who has never saved a backup at all, telling them to import a file Settings was still listing as saved; it now says it could not load the backup and points at the right slot in Settings, in the same words the tabs already use, and the failure is announced to screen readers every time the section is opened. On Multimedia, a saved Macaulay Library export that could not be turned into a species list rendered as though it had loaded, with every photo, recording and video count missing and nothing on screen saying anything had gone wrong, so a birder who has photographed hundreds of species saw the same screen as one who has photographed none; all four ways that can happen now say the export could not be loaded. Having saved nothing is unchanged on both surfaces, because the setup instructions are the right answer to it. The family is still not closed: when the app cannot read its own record of which files are saved, it reports that as having no files, and the surfaces that read a file downstream of it inherit that answer.
