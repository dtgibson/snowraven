...
### Weather on the Statistics tab (complete -- September 2026, v1.0.22)

SnowRaven has been writing weather blocks into eBird checklist comments since its first release and had never read one back; Statistics now does. A Weather section reports what the user's birding actually looked like across sky condition, temperature, wind and time of day, built entirely from the eBird backup already loaded, so it works offline, makes no lookup, needs no key and writes nothing. It opens by saying what fraction of their checklists it describes and how many blocks were found but could not be read, so the coverage figure is visible before any chart is. With no readable blocks the section is not there at all; with only a few it says so and points at the Weather Backlog rather than charting anything. Above that floor it shows their outings distributed across the eleven sky conditions, seven temperature bands, nine wind bands and day versus night, then species recorded per checklist by band with the same bands' average time out beside it, because weather changes how long a person stays out and a species count that ignores that is measuring the birder rather than the birds. A picker gives one species its own record: the conditions and temperature bands that bird has turned up in, each row scaled against the bird's own largest band, with the same band's share of all outings printed beside it in words. Nothing is predicted and nothing is ranked. A band with too few checklists to average says so and keeps its count, a band with none takes a visibly different shape from a thin one, every figure names the checklists behind it, and no figure divides one share by another. Blocks written by RainCrow count exactly as SnowRaven's own do.
