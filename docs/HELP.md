...
- **Plan** lets you choose a place (type a place name to search, or tap the map to drop a pin and drag it to fine-tune). **Get specific forecast** shows the forecast weather and the predicted tide for the date and time you enter; **See all upcoming weather and tide data** builds the plan described below.
...
The Weather/tide Planner, a second action in Plan, lists every sunrise and sunset from now to the end of the weather forecast, each with the predicted tide and the forecast weather at that moment, on one chart and in one list that carries every figure the chart draws. Press **See all upcoming weather and tide data** beneath Get specific forecast; the date and time fields apply only to Get specific forecast. Each listed sunrise and sunset carries its local time in the place's own clock, the predicted tide height in feet relative to MLLW and whether the tide is rising or falling at that moment, the high and low on either side with their times, and the weather in force: within the first couple of days the nearest hourly reading, and after that the day's summary, labeled as a daily summary exactly as Get specific forecast labels one. The chart draws the same window on one time axis: the tide curve, the night hours shaded, every sunrise and sunset marked, every high and low marked, the sun's height as a line, and a weather strip along the bottom, and it scrolls sideways on every screen. A tap on the timeline, or the arrow keys with it focused, reads the estimated tide, weather and sun height at any moment, from the plan already loaded rather than a fresh lookup. The sun's height is drawn across the whole window, with sunrise and sunset exactly where the plan lists them. Each day states its moon phase, from the same computation the checklist weather blocks use. The plan stops where the weather forecast stops. Tide predictions reach further ahead, so for any later moment use Get specific forecast with a date and time. It ranks and recommends nothing. Which mornings put low water at first light is yours to read. Where the nearest tide station is more than 25 miles away or the place is outside the US, the plan still shows sunrise, sunset and weather, says why the tide is missing, and offers the same one-tap option to show the nearest station anyway. A plan loaded once re-shows offline with a cue naming when it was fetched. On a wide window a Days in view choice above the chart fits one, three, seven or all of the plan's days into the chart at once, and two day buttons beside the legend move the chart a day at a time.
...
**Subspecies and forms.** Directly below the species selector, a "Subspecies and forms" control lists every species in your loaded data with at least one subspecies or form noted: each entry shows the species, its reported forms, and each form's share of your reports for that species. Picking an entry selects that species, exactly as choosing it in the selector would, and brings the Subspecies and Forms section into view. The list always reflects your whole backup, not the current county or date filter, and it says so. Included forms follow eBird's countability rule: subspecies groups (like `Dark-eyed Junco (Oregon)`), intergrades, and domestic types count as their parent species and appear here; hybrids, spuhs, slashes, and undescribed forms do not. Everything is computed from your already-loaded export, with no network and no API key. The control appears in the default merged view; with "Show subspecies" on, the exact-name view already lists forms individually, so the control and the section step aside until you switch back.

**Clicking bird names anywhere.** Throughout the app, a bird's common name is rendered in a consistent format: the name is a link that opens that species here on Species Detail (for any species in your data), followed by small icons linking to its eBird and Birds of the World pages, which become the app's own globe and shelf-of-books symbols whenever a site's icon cannot be fetched, with the scientific name shown where there's room. So you can click a species in the Statistics lists, the Map Explorer, the Multimedia, Breeding Codes, or the List Comparer to jump straight to its full history. For a bird you haven't recorded yet (such as a nearby lifer or map target species), the name appears with the eBird/Birds of the World icons but without a Species Detail link, since there's no entry to open.
...
- Subspecies and Forms: how your reports of this species divide between its reported forms and plain species-level reports. One row per countable form with its report count and percentage, plus a "No form noted" row for reports made at species level, always summing to exactly 100%; a headline states the total and how often you note a form at all. The section follows the same county and date filters as the rest of the page. A species with no form detail anywhere in your data says so in one line. When your merged total for the species includes names that are not countable forms (a hybrid, a slash), a footnote states how many and why the Sightings total above differs from this breakdown, so the two figures never quietly disagree. Shown in the default merged view only.
...
- Sighting Locations map: an interactive map with a pin at every unique observation coordinate. Click any pin to see the dates and checklist links for that location. Toggle between Pins and Heatmap view; in Heatmap mode a Heatmap Intensity slider (1–10) lets you dial the coverage from tighter to broader and hotter, the same control as the Map Explorer's My Sightings map. A **Counties** switch beside those controls shades every US county where you have recorded this species, tinted by how many of your checklists there reported it: the counties you have birded without finding it stay as a plain outline. It works in both Pins and Heatmap view, needs no connection and no API key (it is computed entirely from your loaded backup), and reshades the moment you pick a different species, which leaves your Pins or Heatmap choice and the Counties switch exactly as you set them. Any county or date filter you have set applies to the shading exactly as it applies to the pins. Clicking a county opens a popup with the county, the state, your checklist count for that bird there, and your top locations for it. A panel beneath the map carries a **Use Textures** switch (a distinct hatch density per level, so the shading reads without relying on color) and the legend. Both switches are per-session, resetting on relaunch.
- Comments: all your per-species field notes from the eBird backup, sortable by date and filterable by keyword.
- Weather: the skies and temperatures you have recorded this bird in, read back from the weather blocks SnowRaven and RainCrow write into your checklist comments, out of the same derivation the Statistics Weather section paints from, so for any bird both can offer the two print the same numbers. It has no picker of its own: you are already looking at a bird, so the card describes the one the page has selected. It opens by stating one number against two different wholes: how many of your weather-block checklists carry this bird, and how many of the checklists you have it on carry a weather block. Nothing divides one by the other, and there is no coverage percentage for the bird. Below that, two groups of rows, sky and temperature, each bar the bird's own record scaled so its biggest band fills the track, each row reading as a count and that count's share of the bird's records, then in smaller muted type the same band's share of all your outings. Those are two shares of two different wholes, each named, and nothing divides them either. The card has four states. With no weather block anywhere in your data it does not appear at all. With blocks but fewer than 5 readable ones it says so and points at the Weather tab, the same words the Statistics section uses for the same state. With your export above that floor but this bird on fewer than 10 weather-block checklists, it states the two counts and draws no chart: a distribution spread across eleven sky conditions on a handful of checklists would read as a fact about the bird, so the card names the floor instead and points nowhere, because it cannot know whether the backlog reaches this bird. It is the ordinary outcome for most species, and it is meant to read as a fact about coverage rather than as a missing chart. Only above both floors does the chart appear. The figures cover every checklist in your export and count every form as its parent species, so the county and date filters, Show subspecies, Show all forms and Show escapees change nothing here, and the card says so in a line whenever a filter is active or you have a form selected. Counts are checklists, not sightings. It is built from the backup you already loaded, so it makes no lookup and works offline, and it is correct whether or not you have ever opened the Statistics tab.
...
A **Named Individuals** section appears when this species has birds you've named in your checklist comments with a `[name:…]` tag (see the Named Birds tab). Each individual lists its sightings with checklist links and the comment, and the line under its dates says how long you followed it, from its first sighting to its last. The per-bird top-locations list, sightings timeline and map live on the Named Birds tab; this tab's own Top Locations and Sighting Locations map cover the whole species.
...
A **Counties** switch in the section header shades every US county you have birded. A panel beneath the map chooses what the shade means, **Species** or **Checklists**, and carries a **Use Textures** switch (a distinct hatch density per level, so the shading reads without relying on color) and the legend. The numbers match the county tables below it, because both are computed from the same loaded backup. They also match the Map Explorer's while Count all forms is off, which is how both start: the Map Explorer always applies the countable-species rule, so turning Count all forms on here raises the numbers on this map above the ones there. It needs no connection and no API key. Completeness, the Map Explorer's third county metric, is deliberately not offered here: it is the one metric that asks eBird a question, and it would sit inches from county tables that count species by a slightly different rule. Both switches and the metric choice are per-session, resetting on relaunch.
...
Which eBird projects your checklists were submitted to, and how many of them went to each. The California Breeding Bird Atlas is the one this was built for; any other project in your data appears the same way.

**Nothing is sent until you press Check projects.** Your eBird backup does not record project membership at all, so there is no way to work it out on your device: each checklist has to be asked about on its own, one request each. Before you have run it, the section shows no count of any kind, states exactly how many requests that would be, and gives a time estimate derived from the pace the app allows itself (about eight minutes for 3,252 checklists). That estimate is a floor: it takes longer if eBird asks the app to slow down, which it will do automatically and tell you about. If eBird keeps asking during one run, the check spreads its requests further apart each time, and after the third slow-down it pauses itself, keeps every answer already stored, and suggests trying again in about an hour; **Resume** then asks only about the checklists still unanswered.

While it runs you get a progress bar, an `N / M` readout, and a **Stop** button. Stopping keeps every answer already paid for. So does closing the tab or quitting the app: answers are stored on your device, so resuming asks only about the checklists that have not been answered yet, and loading a newer export asks only about the checklists it added. That is what turns an eight-minute cost into a one-time cost. Because a submitted checklist keeps the project it was submitted to, an answer already given is not re-checked for a year. A **Check again** control on a finished section re-asks about all of them anyway, for the rare case where you think an answer is wrong.

**Every figure comes with its denominator.** A tally always reads against the number of checklists actually checked and the number in your backup, so a partial answer reads as a floor rather than as a finished total. Each project lists its checklist count, its share of the checklists checked, and the span of dates you contributed over. With two or more projects, a small bar chart beside the list compares their checklist counts, each row linked to its bar by a colored dot; the chart is decoration, and every figure it shows is also in the rows as text. A project with no published name shows the identifier eBird reports, exactly as reported, with a line saying why: no public eBird endpoint gives a project a name, and SnowRaven does not invent one.

A separate, quieter **How you submitted** block below the projects reports the app or portal each checklist came in through (eBird, Merlin, and so on). That is not a project, and the section says so. A project with its own submission portal appears in both places.

This needs your eBird API key and a connection. Without either, the section says so plainly and offers nothing it cannot do. Checklists that could not be answered after the app's built-in retries are reported separately and are not counted; a **Try again** control re-asks only those.
...
**One species, its weather** lets you pick a bird and see the skies and temperatures you have recorded it in. Each bar is that bird's own record, scaled so its biggest band fills the track and the rest are legible against it; the two groups scale separately, so both are readable at once. Every row reads as a count and that count's share of the bird's records, then, in smaller muted type, the same band's share of all your outings. Those are two shares of two different wholes, each named, and nothing divides one by the other. The second one is there for a reason worth stating: a chart of where a bird turned up also reflects when you were out, and putting the two side by side on every row is what lets you tell the difference without scrolling back up the card. "0 and 0%" and "no outings" are different facts and are drawn differently: the first means outings in that band and never this bird, and it still carries the reference figure; the second means you have never birded that band at all, and carries no numeral and no reference. Counts are checklists, not sightings, so a bird listed three times on one checklist counts once.
...
This section and the Weather card on Species Detail come out of one derivation, so for any bird they can both offer they print the same numbers. Where they can differ is stated rather than left to be discovered: this section follows the tab's own **Count all forms** switch, while the card always counts every form, so with that switch off this section can show a slightly smaller readable-checklist total, by exactly the checklists made up entirely of forms that do not count toward a life list. Neither figure is wrong; they are two stated populations, and each surface names which one it is on. The card also covers birds this section's picker does not offer for the same reason.
...
**The buttons in the map's bottom-right corner.** Every Map Explorer view shows the same row of three round buttons, all the same size, each carrying a small picture of the thing it acts on. (On My Sightings before your eBird backup is loaded there is no map yet, only the setup instructions, so the row is not there either.) Left to right: a **share button**, a **location button** (a target reticle), and a **fullscreen button**. On small or mobile screens a **Filters** button sits beside them to open the controls sidebar. Three other maps carry a shorter row in the same corner, with a share button and a fullscreen button and no location button: Species Detail's Sighting Locations map, the Statistics Geographic Stats map, and a Named Birds card map (see "Fullscreen on the other maps" below). The small location-picker map in the Weather tab's Plan form has no row at all.
...
**Fullscreen on the other maps.** Three maps outside the Map Explorer expand the same way, with the same button in the same corner: Species Detail's **Sighting Locations** map (in both Pins and Heatmap view), the Statistics **Geographic Stats** map, and the per-individual map on a **Named Birds** card. Press it and the map you were already looking at fills the window. It is the same map, not a new one: the same pins, the same base map, the same centre and zoom, a popup you had open still open, a share pin you dropped still dropped, and the county shading exactly as you set it. Press the button again (it becomes a minimize icon) and the map goes back into its box at the size it was, keeping anything you changed while it was expanded. **Escape** closes it too, and puts the keyboard focus back on the button you pressed; while a map is expanded the Tab key stays inside it. Expanded, the map zooms with the scroll wheel and pans with one finger, because there is no page behind it to scroll; back in its box it returns to the two-finger pan these maps use so a thumb-scroll moves the page instead of the map. The controls that sit beside a map rather than on it stay on the page, so set them first: Species Detail's Heatmap Intensity slider and its Counties switch, and the Counties switch in the Statistics section header. Fullscreen is per-session, resetting on relaunch, and it is per map: expanding one leaves the others as they were.
...
**Use Tier Rings (colorblind aid).** While a color mode is active, a **Use Tier Rings** switch (off by default) appears below the mode buttons. It adds a thin white ring just inside each ramp pin's rim, split into five fixed segments: a pin's tier fills that many segments clockwise from the top, and the rest stays as a faint track, so a tier reads by extent from across the map (like a clock face) and by count up close, without relying on color or brightness. The legend's mini pins carry the same ring, and the popup's small color swatch becomes a round badge with the same segments, so the map, the legend, and the popup always agree. Hollow, dashed, and pale pins, personal locations, and the default Visited status coloring are unchanged either way. Unlike the mode choice, this switch is remembered between sessions: it is a reading aid, so you are not asked to re-enable it every launch.
...
**Tapping a widget** opens SnowRaven on Map Explorer, on the matching view, with the widget's time range, its Media choice as the Filter by Type chip (Any opens on All), a 25 mile radius for the session, and a search from your current location. Your saved Radius and Default Location stay as they were. While it loads, finds your location and looks for birds, a line at the top of the map says what it is looking for. Tapping a bird (a row, or the small widget's card) then shows only that species, centered on the sighting the widget listed with its details open; press **Show all**, in the pill at the bottom right of the map that names the bird, to see every bird again. Tapping the widget's title shows every bird. If the bird is no longer reported nearby, the map shows every bird and says so.
...
Each named bird shows its name, species, first- and last-seen dates, a small line noting how long you've followed it, and its total number of sightings, and expands to list every checklist it appears on: the date, the location (a link to the hotspot page on eBird when the location is a public hotspot), a link to that checklist on eBird, and the species comment, plus a small map of everywhere that individual has been seen. Cards open one at a time. Sort the list by **Name (Individual)**, **Alphabetical**, **Taxonomic**, or **Last Seen**.

**How long you've followed it.** The figure under the dates is the true number of days between the two dates it names, and it says which two those are: "50 days · first to last sighting". Up to 60 days it is an exact count of days. Past that it reads as months plus the days left over ("2 mos. 29 days"), and past a year as years plus months ("2 yrs. 6 mos."), so a number never stands for a range wider than the smallest unit it shows. A first and last sighting on the same date reads "Same day".

**Measure to.** Under Sort there is a switch with two settings: **Last sighting**, which is how the tab opens, and **Today**. It governs the whole tab at once, so pressing it changes the figure on every card, both timelines and the wording of the endpoint phrase together. On **Last sighting** a bird's span runs from its first sighting to its last; on **Today** it runs from its first sighting to today, so the gap since you last saw a bird becomes something you can see rather than work out. The same switch appears again inside an open card and above the timeline at the bottom of the tab, and all three are the one control. Your choice is per-session, resetting on relaunch.

**Sightings over time.** An expanded card carries a strip showing that individual's sightings in time, one mark per date it was recorded, running from its first sighting to whichever end the switch is set to. Marks sit exactly where the sightings fall, so they cluster where you saw the bird often and leave the gaps where you did not. A bird recorded on a single date gets no strip, since there would be nothing to plot; its figure still shows. The dates at each end of the strip are written out beneath it.

**Reading a mark.** Press anywhere along a strip and it selects the nearest sighting, and the line underneath names it: its date and where you saw it, with a second line saying which of the bird's dates it is, such as "8 of 9 dates". It counts dates rather than sightings, which is what explains a card reading nine sightings with eight marks on its strip: two checklists on one date are one mark, and the line names both places. On a keyboard, Tab to a strip and the left and right arrow keys step through the sightings one at a time, Home and End jump to the first and last, and Escape clears the selection. Hovering a mark with a mouse previews it without selecting. The card's own dated report list below the strip is still the full record, with the checklist link and your comment on every row.

**All named birds over time.** Below the card list, a second strip puts every named bird on one shared time axis, a labelled lane each, so you can see which birds you were following when without expanding anything. Every named bird gets a lane, with nothing left out and nothing hidden behind a "show more"; the lanes are in the same order as the cards above, so changing Sort reorders both together. A sentence above it names how many birds are shown and the two dates the axis runs between. Each lane draws a line across the bird's own span, first sighting to last, over the full axis, so a bird you followed for a fortnight and one you have followed for years are told apart at a glance. It works like the strip in a card: press or use the arrow keys to select a sighting, and the up and down arrow keys move between birds, landing on the sighting nearest in time. The strip appears once you have two or more named birds, since with one the card above it is already the picture.

**Top locations of a named bird.** Between the reports and the map, an expanded card ranks the places that individual has actually been recorded, most sightings first, so you can see where a bird you follow tends to turn up without counting rows yourself. The counts are that bird's own: they come only from checklists where its `[name:…]` tag appears, never from the species as a whole, and they add up to the card's sighting count (any sighting whose export carries no location name is left out). Five places are shown, with a **Show all N locations** control when there are more. A public hotspot name links to its hotspot page on eBird; a personal location stays plain text. A bird recorded in only one place gets a single line naming it rather than a ranking of one, and a bird whose export carries no location names shows no list at all.
...
A bird is identified by its name together with its species, so the same name used for two different species is tracked as two individuals, and name matching ignores case. This tab requires your eBird backup; the media section additionally requires your ML export. The same named-individual information for a single species also appears as a **Named Individuals** section on the Species Detail tab (without the top-locations list, the map, or the media section, since that tab already has its own Top Locations and sighting map for the whole species).
...
**Turning it on.** Flip the switch. A note first says what goes to iCloud (the two files, and for each its name, when it was uploaded, which device it came from (its name), its size and a checksum), whose account it goes to (your own iCloud account, on Apple's servers, with no SnowRaven server in the path and nothing the developer can see), what happens next, and how to turn it off. Press **Turn on** to proceed or **Cancel** to leave it off. Nothing is written to iCloud until you press Turn on. Your settings and caches are never synced, and your API keys are synced only if you also turn on **Sync API keys** (below).

**Sync API keys.** A second switch sits directly below the first, off by default. It keeps your eBird and OpenWeather API keys the same on every Mac, iPhone and iPad that also turns it on, so a key entered once on any device is used by the others. It can only be turned on while iCloud Sync is on and iCloud is available; until then it is visible but cannot be operated, and a one-line reason says why ("Turn on iCloud Sync first.", or the same note the iCloud Sync switch shows). Flipping it on shows its own note before anything is written: what goes to iCloud (your two keys exactly as you entered them, and for each when it was last changed and which device changed it), whose account (your own, in the same private SnowRaven folder as your synced files, with no SnowRaven server in the path and nothing the developer can see), how Apple protects it (encrypted in transit and at rest; end-to-end encrypted only if Advanced Data Protection is turned on for your iCloud account), which devices (every one signed in to this iCloud account that also turns the switch on), what happens next (a device with no key takes the shared one, the most recently changed key wins, and clearing a key on any sharing device clears it on the others at their next check), and how to stop. Press **Turn on** to proceed or **Cancel** to leave it off. The section's last-check time and **Check now** cover the keys as well as the files. Turning Sync API keys off needs no confirmation: the keys on this device stay put and keep working, and the copy in iCloud is removed. Turning iCloud Sync off turns Sync API keys off too, in the same way. If iCloud cannot be reached at that moment, the switch still goes off and the section says the key copy is waiting to be removed; SnowRaven tries again on its own when iCloud is reachable. **Remove synced keys from iCloud** deletes the key copy in your iCloud account at any time, without touching the keys on any device; it appears whenever iCloud holds a key copy, whether the switch is on or off. Remove synced files from iCloud never touches the key copy, and Remove synced keys from iCloud never touches the files.
...
**Clearing a file with sync on** removes it from this device and from iCloud, and every other device with sync on removes its copy at its next check, so the row asks you to confirm first. On every device that clears its copy, the saved answers worked out from that file go with it, exactly as described under Default Files above. Devices with sync off keep theirs. With sync off, Clear is the same instant local action as everywhere else.

**Turning it off** needs no confirmation: the files on this device stay put, and the copies in iCloud are left alone (the key copy, if Sync API keys was on, is removed, as described above). **Remove synced files from iCloud** deletes the file copies in your iCloud account without touching any device; it appears whenever iCloud holds copies, whether sync is on or off. A device with sync on uploads its copy again at its next check, so turn sync off on each device first if you want iCloud to stay empty. The same is true of **Remove synced keys from iCloud** and a device with Sync API keys on.
...
Reorder and show or hide individual tabs. Drag rows to reorder, or use the **Move up** / **Move down** buttons on each row for a keyboard-only alternative to dragging. Click the eye icon to toggle a tab's visibility. At least one tab must remain visible at all times. The Settings tab is always last and cannot be hidden. On phones the first four tabs shown here become the bottom bar; the rest live under More.

The navigation itself changes shape with the width of the window, and all three shapes follow the same order and visibility choices you set here. Which shape you get is decided from the space actually available to the app, never from what kind of device it is running on. On a wide window it is a vertical sidebar down the leading edge, listing every tab with Settings last under a thin dividing line. When the window is narrow enough that a full sidebar would leave the page itself cramped, the sidebar becomes a column of icons. Nothing is lost from the list itself: the tab you are on stays marked in green with a bar on its leading edge, and every icon keeps its name, which appears beside it when you hover the icon or reach it with the keyboard, and which a screen reader reads whether or not that label is showing. On a touch screen, where there is nothing to hover, press and hold an icon for a moment and its name appears the same way; lift your finger, or slide it away, to dismiss it. On a phone, and in any browser window 640 pixels wide or narrower, the navigation moves to a bar across the bottom of the screen holding your first four tabs plus a **More** button that opens the rest in a sheet. Every tab is reachable at every width.

On a wide window, a **Collapse** button at the foot of the sidebar narrows it to icons by hand and expands it again. It appears only when the window is wide enough for the full sidebar, and the choice is per-session, resetting on relaunch. Narrowing the window far enough switches to the icons on its own, so the button never has to be found in a window where it would not fit.

The rest of the interface is built to hold up on a phone too: controls and lists wrap rather than overflow, buttons and pills grow to a comfortable tap size, and a map popup that is taller than the screen (a shaded county's completeness card, a pin's sightings list, a target or nearby-lifer marker) scrolls inside itself so its content stays reachable.
...
- **Bird names stay complete from a cold start.** Even on a first-ever launch with no connection, bird names sort in taxonomic order and every name still carries its pair of marks linking out to eBird and to Birds of the World. The two sites' own icons are little pictures fetched from those sites, so with no connection the app draws its own stand-ins in their place: a globe for eBird and a shelf of books for Birds of the World. The marks sit in the same spot, are the same size, name the same destinations, and open the same pages once you have a connection again.
