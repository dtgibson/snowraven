...
A **Named Individuals** section appears when this species has birds you've named in your checklist comments with a `[name:…]` tag (see the Named Birds tab). Each individual lists its sightings with checklist links and the comment. The per-bird top-locations list and map live on the Named Birds tab; this tab's own Top Locations and Sighting Locations map cover the whole species.
