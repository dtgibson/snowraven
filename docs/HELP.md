...
The Weather/tide Planner, a second action in Predict, lists every sunrise and sunset from now to the end of the weather forecast, each with the predicted tide and the forecast weather at that moment, on one chart and in one list that carries every figure the chart draws. Press **Plan sunrises and sunsets** beneath Get forecast; the date and time fields apply only to Get forecast. Each listed sunrise and sunset carries its local time in the place's own clock, the predicted tide height in feet relative to MLLW and whether the tide is rising or falling at that moment, the high and low on either side with their times, and the weather in force: within the first couple of days the nearest hourly reading, and after that the day's summary, labeled as a daily summary exactly as Predict labels one. The chart draws the same window on one time axis: the tide curve, the night hours shaded, every sunrise and sunset marked, every high and low marked, and a weather strip along the bottom, and it scrolls sideways on every screen. The plan stops where the weather forecast stops. Tide predictions reach further ahead, so for any later moment use Get forecast with a date and time. It ranks and recommends nothing. Which mornings put low water at first light is yours to read. Where the nearest tide station is more than 25 miles away or the place is outside the US, the plan still shows sunrise, sunset and weather, says why the tide is missing, and offers the same one-tap option to show the nearest station anyway. A plan loaded once re-shows offline with a cue naming when it was fetched. On a wide window a Days in view choice above the chart fits one, three, seven or all of the plan's days into the chart at once, and two day buttons beside the legend move the chart a day at a time.
