...
**Every figure comes with its denominator.** A tally always reads against the number of checklists actually checked and the number in your backup, so a partial answer reads as a floor rather than as a finished total. Each project lists its checklist count, its share of the checklists checked, and the span of dates you contributed over. A project with no published name shows the identifier eBird reports, exactly as reported, with a line saying why: no public eBird endpoint gives a project a name, and SnowRaven does not invent one.
