...
- **Planner chart:** `--sr-plan-day` / `--sr-plan-night` (the plot ground and the sunset-to-sunrise band), `--sr-plan-tide` (curve, high/low marks, the station glyph), `--sr-plan-sun` (sunrise/sunset marks, leaders, event tiles), `--sr-plan-halo` (the ring behind every mark and the stroke behind every chart label) and `--sr-plan-grid-rgb` (gridlines at alpha 0.16 over both bands). Per-theme, not theme-identical: on light the night band is the shaded one; on dark the DAY is the lit band and night is the deep ground, and the halo is the night ground itself. Both bands are always drawn and both are in the legend, so which one is shaded carries no meaning; every mark is haloed so no hue is load-bearing over the night band, and shape plus label distinguish sunrise from sunset and high from low. The contrast invariant is stated per band and guarded by a parse-the-tokens test (`planContrast.test.ts`): over each band every mark's fill or its halo clears 3:1 and the label ink or its halo clears 4.5:1, in both themes. `--sr-plan-sun` also serves as the glyph on `--sr-surface-subtle` tiles (4.7 light, 11 dark). No text is painted on either band by the app's own text tokens, so no on-fill text pair is minted; the chart's labels carry their own halo instead.
