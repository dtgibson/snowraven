...
- **Planner chart:** `--sr-plan-day` / `--sr-plan-night` (the plot ground and the sunset-to-sunrise band: light `#FFFCF5` / `#CBD1DE`, dark `#3E3830` / `#121317`, repainted in 1.0.30 so the plot reads as daylight and night rather than one blue-grey field), `--sr-plan-tide` (curve, high/low marks, the station glyph), `--sr-plan-sun` (sunrise/sunset marks, leaders, event tiles), `--sr-plan-halo` (the ring behind every mark and the stroke behind every chart label; on dark it is the night ground, `#121317`), `--sr-plan-grid-rgb` (gridlines at alpha 0.16 over both bands), `--sr-plan-sunline` with its `-rgb` triplet (the sun-height track above the horizon, light `#703000`, dark `#FDBA74`, filled beneath at alpha 0.14), `--sr-plan-sunline-night` (the track below the horizon, a 1px receded line, light `#6B7080`, dark `#80848F`) and `--sr-plan-pick` (the picked-moment marker and the readout's glyph, light `#0F3D22`, dark `#34D399`). Per-theme, not theme-identical: on light the night band is the shaded one; on dark the DAY is the lit band above the surface and night recedes to just under it, and the halo is the night ground itself. Both bands are always drawn and both are in the legend, so which one is shaded carries no meaning; the day/night boundary is carried by the sunrise and sunset marks, their labels and the sun line's own crossing, so the bands need only differ by the 1.4:1 visible-boundary floor (the same reasoning as the sticky band's hairline; 1.49 light, 1.60 dark). Every mark is haloed so no hue is load-bearing over the night band, and shape plus label distinguish sunrise from sunset and high from low. The contrast invariant is stated per band and guarded by a parse-the-tokens test (`planContrast.test.ts`): over each band every mark's fill or its halo clears 3:1, the tide line, the sun line, its night portion and the pick marker each clear 3:1 against both bands, and the label ink or its halo clears 4.5:1, in both themes. `--sr-plan-sun` also serves as the glyph on `--sr-surface-subtle` tiles. No text is painted on either band by the app's own text tokens, so no on-fill text pair is minted; the chart's labels carry their own halo instead.
