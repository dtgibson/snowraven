...
A link is a **view link** (one of exactly fifteen strings) optionally followed by a **bird suffix** (Stage 8 re-entry: a bird tap carries which bird). The parser is a table over the fifteen view links plus one anchored, bounded pattern for the suffix; it is not a query-string reader. Nearby Lifers carries the window only; Media Targets carries the window and then the media setting, in that fixed order:

```
view link (fifteen strings):
  snowraven://map/lifers?window=<day|week|all>
  snowraven://map/targets?window=<day|week|all>&media=<photo|audio|video|any>
      (the window token `all` is the 30 days value and is unrelated to the media value)

bird link = view link + bird suffix, fixed order, no other parameter:
  &sp=<speciesCode>&loc=<locId>
      speciesCode: ^[a-z0-9-]{2,16}$    (corrected per decisions.md, Stage 8: the app's shared SPECIES_CODE_RE) eBird species codes are lowercase alphanumerics ("norcar", "reevir1", "x00001"); 16 is
                                        above every code in the taxonomy (longest observed 10) by a margin that costs nothing
      locId:       ^L[0-9]{1,15}$       the eBird location id; the same class security.md's ChecklistLink rule uses for an
                                        `L` id, written with explicit [0-9] (v0.5.54), bounded at 15 digits
```

**Identifiers, and why not coordinates.** The widget has `speciesCode`, `locId`, `lat` and `lng` for every row from the eBird response. The link carries `speciesCode` and `locId` only: the app re-runs the same eBird search from the user's current position (FR-36) and gets back records keyed by exactly those two ids, so the sighting is found by identity rather than by matching a rounded coordinate against a coordinate, and nothing in the URL is a number the app would have to interpret. Where `locId` is not in the fresh results (the sighting aged out of the window, or the user moved), the app falls back to the nearest listed sighting of that species (4.5), which is what coordinates would have bought and at no grammar cost.

`lib/links/deepLink.ts` (entry-safe, dependency-free):

```
export const LINK_SCHEME = 'snowraven'
export const LINK_MAX_LENGTH = 96            // longest view link 46 + longest suffix 41 ("&sp=" 4 + 16 + "&loc=" 5 + 16) = 87; 96 leaves no room for a payload
export const SPECIES_CODE_RE = /^[a-z0-9-]{2,16}$/   // lib/speciesCode.ts, shared with SpeciesLinks
export const LOC_ID_RE = /^L[0-9]{1,15}$/
export const BIRD_SUFFIX_RE = /^&sp=([a-z0-9-]{2,16})&loc=(L[0-9]{1,15})$/    // anchored at both ends; fixed classes; bounded quantifiers; no alternation
export type ViewLink =
  | { view: 'lifers'; window: 'day' | 'week' | 'all' }
  | { view: 'targets'; window: 'day' | 'week' | 'all'; media: 'photo' | 'audio' | 'video' | 'any' }
export type BirdRef = { speciesCode: string; locId: string }
export type WidgetLink = ViewLink & { bird?: BirdRef }
export function parseWidgetLink(raw: unknown): WidgetLink | null
export function buildWidgetLink(link: WidgetLink): string   // the inverse; refuses (throws) a bird whose ids fail the two patterns, so the TS side can never build an out-of-pattern link
```

`parseWidgetLink` rules, in order:

1. `typeof raw === 'string'` and `raw.length <= LINK_MAX_LENGTH`, else null (checked BEFORE any other operation, so every later step runs over at most 96 code units: the linearity argument in section 9).
2. `cut = raw.indexOf('&sp=')`; `head = cut < 0 ? raw : raw.slice(0, cut)`; `tail = cut < 0 ? '' : raw.slice(cut)`. `indexOf` is the first occurrence, so a `&sp=` anywhere in a malformed string only moves the cut earlier, and the head then fails step 3.
3. `head` is byte-equal to one of the fifteen view links, looked up through `Object.hasOwn(TABLE, head)` on a null-prototype table (security.md v0.5.81), else null: the whole link is rejected, sets nothing, renders nothing, logs nothing. No `URL` constructor, no query parsing, no case folding, no trailing-slash tolerance, no percent-decoding, no parameter reordering.
4. `tail === ''` gives the view link. Otherwise `BIRD_SUFFIX_RE.exec(tail)`: a match gives the view link with `bird = { speciesCode, locId }` from the two captures; **no match DEGRADES to the view link** (the bird is dropped, the view still opens, nothing is shown or logged about the suffix). A bird parameter is never a reason to refuse a link whose view part is exact, because the view-only landing is the shipped behavior and is always correct; the degradation is also what a user with an out-of-date widget build gets from a newer app, and the reverse.

Rejected whole (rule 1 or 3): `snowraven://map/lifers/?window=day`, `SNOWRAVEN://...`, `?window=day&x=1`, `map/lifers?window=day&media=photo` (media on the lifers path), `map/targets?window=day` (media missing), `map/targets?media=photo&window=day` (reordered), `media=both` and `media=all` (superseded values), `#fragment`, a `javascript:` payload, a 97-character string, and `...?window=day&loc=L1&sp=norcar` (suffix order reversed: the head then ends in `&loc=L1`, which is not a view link). Degraded to the view link (rule 4): `&sp=NORCAR&loc=L1` (uppercase), `&sp=n&loc=L1` (1 char), a 17-char code, `&sp=norcar&loc=1` (no `L`), `&loc=l1`, `&loc=L` (no digits), a 16-digit id, `&sp=norcar` alone, `&sp=norcar&loc=L1&x=1` (trailing parameter), `&sp=norcar&loc=L1#f`, `&sp=nor%63ar&loc=L1` (encoded), `&sp=norcar&loc=L1&sp=other&loc=L2` (repeated). Every one of these is a fixture row (7.2).

Because the widget is the only author of these URLs, an exact table for the view and one anchored pattern for the bird is not a limitation; it is the smallest parser that can be correct, and the bird ids are used by the app only as lookup keys against records it fetched itself (4.5), never rendered and never placed in a URL.
...
3. If `phase.tag === 'ready'`: mark the id applied, then run the location step: `setIsLocating(true)`, `getCurrentLocation()`, on success `setLat/setLng/setDetectedLocation/setPanTarget` exactly as `handleUseMyLocation` does, then `handleFindLifers(loc.lat, loc.lng, 25)` or `handleFindSightings(loc.lat, loc.lng, 25)` UNCONDITIONALLY (unlike `handleUseMyLocation`, which only searches when the center was empty: FR-36 says the search runs whether or not a center was set). On failure `setGeoError(describeLocationError(err))` and no search (FR-38).
   ...
   **Bird focus (Stage 8 re-entry):** when `linkRequest.bird` is present, set `linkFocus` (below) to `{ speciesCode, locId, searchId }` in the same batch as the search call, where `searchId` is a monotone counter the two search handlers increment on entry (a new `searchSeq` ref), so the focus is bound to exactly the search the link started and a later search cannot inherit it.
...
**`linkFocus`: the bird-focus state (Stage 8 re-entry).** `const [linkFocus, setLinkFocus] = useState<{ speciesCode: string; locId: string; searchId: number } | null>(null)` in `MapExplorer`, session component state and nothing else: never written to `storage`, never part of `map-defaults`, `searchRecords` or any replay document, and gone on relaunch. It is applied as a pure derivation over the search's results, never by mutating them:

- **Lifers:** `displayedLiferPins = linkFocus ? liferPins.filter(loc => loc.lifers.some(s => s.speciesCode === linkFocus.speciesCode)).map(loc => ({ ...loc, lifers: loc.lifers.filter(same), count: 1 })) : liferPins`. **Targets:** `displayedTargetPins` (which already exists for the media chips) additionally filters `pin.speciesCode === linkFocus.speciesCode`. The sidebar lists and the map layers read the displayed arrays, so both agree.
- **Center:** once the focused search resolves (the handler's `setLiferPins` / `setTargetPins` with `searchSeq.current === linkFocus.searchId`), pick the location with `locId === linkFocus.locId` among the displayed pins, else the displayed pin nearest the search center by `distanceMiles`, and `setPanTarget` to it. Comparisons are string equality on ids the app fetched itself; the link's ids are never rendered.
- **Species not in the results:** the displayed arrays would be empty, so instead the derivation returns the full arrays and the view shows all pins with a one-sentence note in the existing `searchOutcome` slot (copy by The Designer: the tapped bird is not in these results, showing all nearby). The note cannot name the bird without the name, and the link carries no name; the species name is looked up from the results, and there is none, so the sentence is generic by construction rather than by omission.
- **"Show all":** a control in the focused view (placed by The Designer) that calls `setLinkFocus(null)`; the displayed arrays fall back to the full results with no new request.
- **Clears** (every one is `setLinkFocus(null)`, wired at the existing setter call sites, and pinned by a table test): Show all; a view-mode change (the mode buttons at MapExplorer.tsx line 2944 and the link's own step 1 when the view changes); a window change (`setLiferWindow` / `setTargetViewMode` `onChange` at lines 2776 and 2625); a media chip change (`setTargetTypeFilter` from the chips, but NOT the link's own step 3 write, which runs in the same batch as the focus and must not clear it); any new search (the first statement of `handleFindLifers` and `handleFindSightings`, before `searchSeq` increments, so Use my location, Search this area, the pin drop and a second link all start clean). A second bird link sets a new focus after that clear, in the same batch as its search.
- **Never persisted, never synced, never in the entry chunk:** it is a `useState` in the lazy Map Explorer chunk, and the link parser that produces it is already entry-safe.
...
src-tauri/gen/apple/snowraven_widgets/
  ...
  Sources/
    Logic/                         PURE Swift, imports Foundation only. No WidgetKit, SwiftUI, CoreLocation or URLSession import,
      ...
      DeepLink.swift               viewURL(kind:window:media:) -> URL, the fifteen-string builder (QA-34); birdURL(view:speciesCode:locId:) -> URL,
                                   which appends the "&sp=<code>&loc=<locId>" suffix ONLY when both ids match SPECIES_CODE_RE / LOC_ID_RE
                                   (NSRegularExpression, anchored, explicit [0-9]) and otherwise returns viewURL, so the extension never
                                   emits an out-of-pattern link (the same refusal the TS builder makes, pinned by the parity guard)
    ...
    Widget/                        imports WidgetKit, SwiftUI, CoreLocation, AppIntents
      ...
      Views/                       the SwiftUI views The Designer specifies (small, medium, large; state views); system text styles.
                                   LINK WIRING (Stage 8 re-entry; independent of the visual design): the root view of every family carries
                                   `.widgetURL(viewURL)` so any tap outside a row (the header, a state sentence, the update time) opens
                                   the view-only link; on medium and large each row is wrapped in `Link(destination: birdURL(row))`, which
                                   WidgetKit resolves per region and which overrides the root `widgetURL` inside the row; on small,
                                   which allows no `Link` regions, the root `.widgetURL` is `birdURL(row)` when a row is shown and
                                   `viewURL` in every state view (S1 to S12), so the one-bird card IS the bird link. `row` supplies
                                   `speciesCode` and `locId` from the reduced record (6.6). The header is never a `Link`.
...
|---|---|---|---|---|
| App admits, widget refuses | `+2026-09-01`, `2026-9-1` (anything `Number()` coerces that the anchored class does not; corrected at the build: ` 2026-09-01` and `2026-09-01T00:00`, first listed here, are refused by the app too, and the corpus test lists what the app actually admits) | admitted | refused | None of these can leave eBird: `obsDt` is documented and observed as `YYYY-MM-DD HH:mm` (or `YYYY-MM-DD`); the widget refusing them costs a row that the app would show only for a body eBird does not send. Enumerated by the generated-corpus test in 7.3, which runs every single-position edit of one conforming string over a 96-character alphabet through BOTH predicates and asserts the widget-admits-and-app-refuses set is EMPTY |
...
`Distance.miles(lat1, lng1, lat2, lng2)`: R = 3958.8, haversine with `atan2(sqrt(a), sqrt(1 - a))`, radians via `* .pi / 180`, in `Double`. JavaScript and Swift both use IEEE 754 binary64 and platform `libm` (`sin`, `cos`, `atan2`, `sqrt`); the results agree to well within 0.01 mi (QA-06's tolerance) and the fixture asserts each row's distance to 1e-6 mi, tighter than the display. Display: one decimal plus ` mi` through `JSNumber.toFixed(d, 1)`, an exact Swift twin of JavaScript's `toFixed` (`Distance.swift`), never `String(format: "%.1f")`. Corrected by measurement at the build: this section first claimed `toFixed(1)` and `%.1f` both round half away from zero, and they do not agree on an exact binary tie, where C rounds to even and `toFixed` takes the larger value (0.25 prints `0.2` from `%.1f` and `0.3` from the app). The fixture's format rows carry those ties. Below 0.05 the format yields `0.0 mi` on both sides with no special case (FR-06).
...
| `lib/icloudPaths.parity.test.ts` pattern (new sibling `widgetPaths.parity.test.ts`) | `APP_GROUP_ID`, `WIDGETS_DIR`, `HANDOVER_FILE`, `HANDOVER_MAX_BYTES`, `LINK_SCHEME`, `LINK_MAX_BYTES` / `LINK_MAX_LENGTH`, and (Stage 8) the source text of `SPECIES_CODE_RE` and `LOC_ID_RE` in `deepLink.ts` and `DeepLink.swift` (one pattern string on each side, compared as text, since the two engines cannot share an object) pinned across `widgets.rs`, `widgetHandover.ts`, `deepLink.ts`, `AppGroup.swift`, `DeepLink.swift`, both entitlement files; the Rust `HandoverDoc` derives no `Debug` |
...
- **`lib/links/deepLink.test.ts`** (QA-35, QA-52): the fifteen view URLs and the fixture's `links` table (valid birds, bounds, every malformed shape with its degrade-or-reject expectation), the two structural rules (7.3 d and e), and a linear-time check over 10,000 generated inputs up to 4 KB (the length gate mutated out to prove it is the gate, and `BIRD_SUFFIX_RE` run against a 96-character adversarial tail to show it has no super-linear case: anchored, fixed classes, bounded quantifiers, no alternation, no nested quantifier).
- **`MapExplorer` link-focus tests** (the Stage 8 delta): the displayed arrays under a focus (lifers and targets), the center choice (matching `locId`, else nearest of the species), the species-absent fallback with its note, Show all, and a table over every clear (view change, window change, chip change, each of the four search entry points, a second link) asserting `linkFocus` is null afterwards and that the link's own chip write does NOT clear it; a `searchId` row where a stale search resolving after a newer one cannot center the map.
...
|---|---|---|---|
| `parseWidgetLink` (TS) | a URL string from native, ultimately from whatever process opened the scheme (any app can open `snowraven://...`) | `length <= 96` checked first | one length check, one `indexOf('&sp=')`, one `slice`, one `Object.hasOwn` on a fifteen-entry null-prototype table for the head, and one anchored regex (`BIRD_SUFFIX_RE`: two fixed character classes with bounded quantifiers, no alternation, no nesting, so linear in its input with no backtracking blowup) over a tail of at most 92 code units: O(n) with n <= 96; no `URL`, no decoding. Declared per security.md's "a build that adds a scan declares it" rule: this build turns a table lookup into a table lookup plus one bounded pattern |
| The bird ids inside the app | `speciesCode` and `locId` captured by the pattern above | 2 to 16 characters each | used only as `===` keys against records the app fetched from eBird itself (4.5); never rendered, never in a URL, never persisted |
...
Nothing from the deep link is reflected into the UI (NFR-04); the only things it changes are three enum-typed state values, one set-valued filter chosen from four fixed sets, one radius literal, and (Stage 8) a `linkFocus` pair of ids that select among fetched records and are themselves never displayed: the species name and location name the focused view shows come from the matched eBird record, not from the link.
...
7. **Exact-match parser for the view link** (4.1): fifteen strings in a table; the widget is the only author, so a free-form grammar is surface without benefit. The bird suffix added at Stage 8 (decision 15) is the one bounded pattern beside it, not a query-string reader.
...
15. **A bird tap carries `&sp=<speciesCode>&loc=<locId>` after an unchanged view link, and the parser stays an allowlist: a fifteen-entry table for the head plus one anchored, bounded pattern for the tail (Stage 8 re-entry).** Ids rather than coordinates, because the app re-fetches and matches by identity, with nearest-of-species as the fallback. A malformed suffix degrades to the view link rather than refusing, because the view-only landing is always correct and is what a mismatched widget and app version must produce in either direction. The focus is session state in Map Explorer, derived over the results, cleared by Show all, any view / window / chip change and any new search, and never persisted. The header keeps the view link; rows and the small card carry the bird link.
...
- **V8 (Stage 8)** WidgetKit honors a per-row `Link` inside medium and large and the root `widgetURL` for the header region on a device, and small's single `widgetURL` carries the bird link; verified in the 1.0.36 build 2 device pass by tapping a row, the header and a small card. If `Link` regions prove unreliable in the tinted or clear rendering modes, the fallback is the same grammar with rows as `Link` only in large and the whole medium card as the first row's bird link, stated as a limit rather than silently.
- **V9 (Stage 8)** `locId` values in eBird's `geo/recent` response match `^L[0-9]{1,15}$` for every record in the dense fixture and in one live response; a personal location id is also `L`-prefixed. If a shape outside the class appears, the widget's builder degrades that row to the view link (by design), and the class is widened here before it is widened in code.
