...
> **CORRECTED 2026-09-22 by the `plan-sun-sampling-bound` build (Improve lane).** Three of the rows below declared a bound the code did not have, and the paragraph under the table stated a cap that did not exist. `PLAN_DAYS_MAX` caps the day COUNT at the producer; it never capped the window SPAN, each day's own span, or anything at all in a REPLAYED document, because `composePlan` re-applied no producer cap. The 1,568-sample `sunTrack` figure was therefore an intention, not a ceiling, and the live path could exceed it (a One Call body whose 16 daily entries carry `dt` a year apart yields a 5,475-day window and 525,667 samples through the shipped builder on both transports). The `sunAltitudeAt` row also omitted the `sideAt` walk over `model.days` entirely, which is the term that makes `sunPeakByDay` times `sideAt` quadratic in the day count. The rows below now state the bound and the cost the code HAS, enforced at two chokepoints (`composePlan` in `lib/plan.ts` and `planGeometry` in `lib/planChartGeometry.ts`) and guarded by `frontend/src/lib/planSpanBound.test.ts`. Rationale and measurements: `pipeline/plan-sun-sampling-bound/decisions.md`.

The three constants the corrected rows cite are declared in `frontend/src/lib/plan.ts`: `PLAN_DAY_MAX_S = 26 * 3600` (26 h, from the measured 89,999 s `dst-fall` day, so a 24 h clamp would have refused a conforming day), `PLAN_COMPOSE_DAYS_MAX = 16` (pinned equal to `weatherPlan.ts`'s `PLAN_DAYS_MAX` by the guard, because the merge's entry-chunk closure may not import it), and `PLAN_SPAN_MAX_S = PLAN_COMPOSE_DAYS_MAX * PLAN_DAY_MAX_S = 1,497,600 s`. The fourth, `PLAN_Y_ABS_MAX_FT = 100`, is in `lib/planChartGeometry.ts` because it is a different unit. A fifth, `PLAN_TS_ABS_MAX = 8.64e12` s, is in `lib/plan.ts` and bounds the loop ANCHOR rather than a span: `composePlan` refuses a window, and `asDays` drops a day, whose own instants lie outside it, because from |t| >= 2^63 one ulp of a double exceeds the 900 s quarter step and the `sunTrack` and `sunPeakByDay` loop variables stop advancing altogether. Its value is the ECMAScript Date range (+/- 8.64e15 MILLISECONDS) converted into this document's unit of epoch SECONDS; the unconverted figure would admit the band in which `solarNoonTs`'s whole-day normalization loop hangs instead (the residual named at that function and in `pipeline/plan-sun-sampling-bound/decisions.md`). Without the guard the two sample ceilings below would hold for every document except the ones that hang.
...
| `buildSunModel` | `Plan.days` | per day: two bisections of at most 20 evaluations of a closed-form function | `<= PLAN_COMPOSE_DAYS_MAX` = 16 days, `<= 640` evaluations per plan, once per plan. The day count is the MERGE's now, not the producer's: `asDays` breaks at 16 admitted days, so a replayed document carrying twenty thousand costs sixteen admissions |
| `sunTrack` | the model | one pass over the quarter marks with a running anchor index | `<= PLAN_SPAN_MAX_S / 900 + 1 + 2 * PLAN_COMPOSE_DAYS_MAX = 1,497,600 / 900 + 1 + 32 = 1,697` samples per plan. The governing number is the WINDOW SPAN, which `composePlan` clamps to `axisStartTs + PLAN_SPAN_MAX_S`; `PLAN_DAYS_MAX` never bounded it. Each sample is `O(1)` in the anchors with the running index but carries one `sideAt` day walk (next row); once per plan, never per pick or per density |
| `sunAltitudeAt` | the model | a binary search over `<= 32` anchors, ONE LINEAR `sideAt` walk of `model.days`, and one evaluation | `O(log 32) + O(days)` per call. The day walk was omitted from this table before the correction above and is the term that matters: it is why `sunPeakByDay` times `sideAt` is quadratic in the day count, measured 34.6 / 85.3 / 273.6 / 1,011.5 ms at 500 / 1,000 / 2,000 / 4,000 days. Bounded at `<= PLAN_COMPOSE_DAYS_MAX` = 16 comparisons per call by the merge's day cap |
| `sunPeakByDay` | the model | per day, one pass over that DAY's quarter marks, plus a SCAN of ALL anchors filtered by `dayIndex` (<= 32 per day, <= 512 per plan, of which at most 2 are evaluated), plus the warped solar noon | `<= PLAN_COMPOSE_DAYS_MAX * (PLAN_DAY_MAX_S / 900 + 1 + 2 + 1) = 16 * 108 = 1,728` evaluations per plan. Bounded on TWO axes, and neither was declared before the correction above: each day's own span (clamped by `asDays` to `PLAN_DAY_MAX_S`, which the window clamp alone would not reach) and the day count. Runs in a `useMemo` in a render body, so this is the thread that paints |
...
| the gridline and y-tick loops (`PlanChart.tsx`) | `planGeometry`'s `yMin` / `yMax`, from `Plan.tide.curve[].v` | `for (v = ceil(yMin); v <= floor(yMax); v += 2)`, twice, each iteration building a React element | `<= 101` lines and `<= 101` ticks. Added by the correction above: `v` is `parseFloat` plus a finiteness check with no magnitude bound on the LIVE path (`lib/tide.ts`), measured at 100,000,001 iterations for `v = 1e8`. `planGeometry` clamps the DOMAIN to `+/- PLAN_Y_ABS_MAX_FT` (never the values, which would distort the drawn curve); a curve lying wholly beyond the ceiling falls back to the default domain and overflows, as `allowDataOverflow` already permits. `PlanChart.tsx` itself is unedited |
...
No new regular expression; no `includes`, `indexOf` or `find` over a provider-derived or document-derived string needle (the one `indexOf` is over a constant array with an in-house needle); no lookup table keyed by an external string. **No new value is interpolated into any URL**: the feature makes no request. The rename touches no identifier, storage key, replay key or route (FR-04). `Plan.lat` / `Plan.lng` reach `Math` functions only. The Auditor's doubling check is owed on `sunTrack` at 8 / 16 days and on one readout at the maximal family. **This sentence used to end "flat past `PLAN_DAYS_MAX` because the composer caps `days`", which was false in both halves: the composer capped nothing, and a day cap would not have flattened a curve driven by the window SPAN.** Since the correction above the composer does cap `days`, and the flattening claim is true of the SPAN clamp: `sunTrack` is flat past `PLAN_SPAN_MAX_S` because `composePlan` clamps `window.endTs`.
