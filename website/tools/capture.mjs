...
const weatherFailure = await runRequiredCapture('weather-light.png', async () => {
  ...
  try {
    ...
    for (const btn of await p.$$('main button')) {
      if ((await btn.getAttribute('role')) !== 'tab' && /get weather/i.test(await btn.textContent() || '')) {
        lookupButton = btn;
        break;
      }
    }
    if (!lookupButton) throw new Error('Get weather button was not available');
    await lookupButton.click();
    await waitForWeatherCaptureReady(
      () => p.locator('#panel-weather').innerText(),
      { sleep: ms => p.waitForTimeout(ms) },
    );
...
if (weatherFailure) requiredFailures.push(weatherFailure);

await browser.close();
if (requiredFailures.length) {
  console.error(`CAPTURE FAILED (${requiredFailures.length}):\n  ` + requiredFailures.join('\n  '));
} else {
  log('CAPTURE DONE — now run: node process-img.mjs');
}
applyCaptureExitCode(requiredFailures);
